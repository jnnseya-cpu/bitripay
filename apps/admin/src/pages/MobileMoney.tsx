...
export function MobileMoney() {
  ...
  return (
    <div>
      ...
      <Tabs tabs={[{ id: 'operators', label: 'Operators & collection numbers' }, { id: 'devices', label: 'Evidence devices (SMS forwarders)' }, { id: 'templates', label: 'SMS parsing templates' }, { id: 'routes', label: 'Any-to-any money routes' }]} value={tab} onChange={(v) => setTab(v as any)} />
      {tab === 'operators' && (
        <>
          <div className="card mb">
            <h4>How the direct rail works</h4>
            <p className="small muted">1. Register a merchant / collection number with each operator you want to accept (your normal business mobile money account). 2. Enter it below. 3. Customers authorise the intent with biometrics/PIN and pay from their own mobile money app or USSD to that number with the reference we show them. 4. The operator's receipt SMS is forwarded by a <b>registered, key-signed device</b> (Evidence devices tab); it is parsed, matched on reference / amount / currency / sender / time window, checked for replays and duplicates, and only then settled. Anything uncertain lands in the verification console for maker-checker approval. The legacy shared-secret webhook below is accepted as evidence but is not authoritative unless you enable it in Gateway controls. Payouts to any mobile money number are queued in Approvals → Withdrawals for your treasury team or an agent to pay from the operator float.</p>
            <div className="grid cols-2">
              <div><KVLine k="Signed evidence endpoint" v={<code>POST {config?.apiUrl}/api/evidence/sms</code>} /><KVLine k="Legacy shared-secret webhook" v={<code>POST {config?.apiUrl}/api/webhooks/manual_momo</code>} /><KVLine k="Body" v={<code>{'{"secret":"<smsSecret>","text":"<forwarded SMS>"}'}</code>} /><KVLine k="Gateway status" v={direct ? <StatusBadge status={direct.enabled ? 'active' : 'pending'} /> : '—'} /><KVLine k="Configured operators" v={`${configured} with a collection number`} /></div>
              <div>
                <Field label="SMS webhook shared secret" hint={direct?.configuredKeys?.includes('smsSecret') ? 'Configured – enter a new value to rotate. Not authoritative unless enabled in Gateway controls.' : 'Not configured – the legacy webhook is rejected until set'}>
      ...
      {tab === 'devices' && (
        <div className="grid cols-2">
          <div className="card">
            ...
            <p className="small muted">The forwarder app on the collection phone generates an Ed25519 key pair and keeps the private key in the device's secure storage. Paste its public key (PEM or raw base64). Every forwarded SMS is signed over <code>deviceId\nnonce\nreceivedAt\nfrom\noperatorId\ntext</code> and posted to <code>POST {config?.apiUrl}/api/evidence/sms</code>. Nonces are single-use; bad signatures raise the device's risk score.</p>
            <Field label="Device name"><Input value={dev.name} onChange={(e) => setDev({ ...dev, name: e.target.value })} placeholder="Collection phone – Nairobi" /></Field>
            <Field label="Public key"><Textarea rows={4} value={dev.publicKey} onChange={(e) => setDev({ ...dev, publicKey: e.target.value })} placeholder="-----BEGIN PUBLIC KEY----- …" /></Field>
            <Field label="Operators this device may confirm (comma-separated ids, blank = any)"><Input value={dev.operatorIds} onChange={(e) => setDev({ ...dev, operatorIds: e.target.value })} placeholder="mpesa_ke, airtel_ke" /></Field>
            <Field label="Device kind"><Select value={dev.kind} onChange={(e) => setDev({ ...dev, kind: e.target.value })}><option value="collection">collection – forwards receipts for money in</option><option value="payout">payout – approved Android device with a merchant SIM</option></Select></Field>
            {dev.kind === 'payout' && <div className="grid cols-2"><Field label="SIM MSISDN"><Input value={dev.simMsisdn} onChange={(e) => setDev({ ...dev, simMsisdn: e.target.value })} /></Field><Field label="SIM ICCID"><Input value={dev.simIccid} onChange={(e) => setDev({ ...dev, simIccid: e.target.value })} /></Field><Field label="Payout account id"><Input value={dev.payoutAccountId} onChange={(e) => setDev({ ...dev, payoutAccountId: e.target.value })} /></Field><Field label="Operating agent user id"><Input value={dev.agentUserId} onChange={(e) => setDev({ ...dev, agentUserId: e.target.value })} /></Field></div>}
            <Button disabled={!dev.name || dev.publicKey.length < 32} onClick={() => api.post('/api/admin/evidence/devices', { name: dev.name, publicKey: dev.publicKey, operatorIds: dev.operatorIds.split(',').map((s) => s.trim()).filter(Boolean), kind: dev.kind, simMsisdn: dev.simMsisdn || null, simIccid: dev.simIccid || null, payoutAccountId: dev.payoutAccountId || null, agentUserId: dev.agentUserId || null }).then(() => { toast('Device registered', 'success'); setDev({ name: '', publicKey: '', operatorIds: '', kind: 'collection', simMsisdn: '', simIccid: '', payoutAccountId: '', agentUserId: '' }); devices.reload(); }).catch((e) => toast(e.message, 'error'))}>Register device</Button>
          </div>
          <div className="card">
            <h4>Registered devices</h4>
            <Table head={['Name', 'Id', 'Kind / SIM', 'Operators', 'Status', 'Risk', 'Last seen', '']} rows={(devices.data?.items ?? []).map((d: any) => [<b>{d.name}</b>, <span className="mono tiny">{d.id}</span>, <span className="tiny">{d.kind}{d.simMsisdn ? ` · …${String(d.simMsisdn).slice(-4)}` : ''}</span>, d.operatorIds.length ? d.operatorIds.join(', ') : 'any', <StatusBadge status={d.status} />, <Chip kind={d.riskScore >= 50 ? 'danger' : d.riskScore > 0 ? 'warning' : 'success'}>{d.riskScore}</Chip>, fmtDate(d.lastSeenAt), d.status === 'active' ? <ConfirmButton size="sm" variant="danger" prompt="Reason" onConfirm={(r) => api.del(`/api/admin/evidence/devices/${d.id}`).then(() => { toast(`Revoked: ${r}`, 'success'); devices.reload(); })}>Revoke</ConfirmButton> : null])} empty="No devices registered – signed evidence cannot be received yet" />
          </div>
        </div>
      )}
      {tab === 'templates' && (
        <div className="grid cols-2">
          <div className="card">
            <div className="row mb"><h4 style={{ margin: 0 }}>Operator parsing templates</h4><Button size="sm" style={{ marginLeft: 'auto' }} onClick={() => setTpl({ id: 'new', operatorId: '*', name: '', priority: 0, enabled: true, patterns: { keywords: [], reference: '', amount: '', currency: '', sender: '', recipient: '', externalRef: '', timestamp: '', balance: '' } })}>+ New template</Button></div>
            <p className="small muted">Regular expressions (group 1 = value; sender uses group 1 = name, group 2 = phone). Templates for a specific operator take priority over the generic <code>*</code> templates. Confidence: reference 50, amount 25, currency 10, operator transaction id 10, sender 5.</p>
            <Table head={['Operator', 'Name', 'Priority', 'Enabled', '']} rows={(templates.data?.items ?? []).map((t: any) => [<span className="mono">{t.operatorId}</span>, t.name, t.priority, <StatusBadge status={t.enabled ? 'active' : 'disabled'} />, <div className="row"><Button size="sm" variant="secondary" onClick={() => setTpl({ ...t, patterns: { ...t.patterns } })}>Edit</Button><ConfirmButton size="sm" variant="ghost" onConfirm={() => api.del(`/api/admin/evidence/templates/${t.id}`).then(templates.reload)}>Delete</ConfirmButton></div>])} />
          </div>
          <div className="card">
            <h4>Test a message</h4>
            <Field label="Paste an operator SMS"><Textarea rows={4} value={testText} onChange={(e) => setTestText(e.target.value)} placeholder="QX7A1B2C3D Confirmed. You have received Ksh1,000.00 from JOHN DOE 254712345678 … Ref MMABC123" /></Field>
            <Button variant="secondary" disabled={!testText} onClick={() => api.post('/api/admin/evidence/parse-test', { text: testText }).then((r: any) => setTestOut(r.parsed)).catch((e) => toast(e.message, 'error'))}>Parse</Button>
            {testOut && <pre className="tiny mono mt" style={{ whiteSpace: 'pre-wrap' }}>{JSON.stringify(testOut, null, 2)}</pre>}
          </div>
        </div>
      )}
      <Modal open={!!tpl} onClose={() => setTpl(null)} title={tpl?.id === 'new' ? 'New parsing template' : `Edit ${tpl?.name}`} wide>
        {tpl && (
          <>
            <div className="grid cols-2">
              <Field label="Operator id (* = any)"><Input value={tpl.operatorId} onChange={(e) => setTpl({ ...tpl, operatorId: e.target.value })} /></Field>
              <Field label="Name"><Input value={tpl.name} onChange={(e) => setTpl({ ...tpl, name: e.target.value })} /></Field>
              <Field label="Priority"><Input type="number" value={tpl.priority} onChange={(e) => setTpl({ ...tpl, priority: Number(e.target.value) })} /></Field>
              <Field label="Keywords (comma-separated, message must contain one)"><Input value={(tpl.patterns.keywords ?? []).join(', ')} onChange={(e) => setTpl({ ...tpl, patterns: { ...tpl.patterns, keywords: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) } })} /></Field>
            </div>
            {['reference', 'amount', 'currency', 'sender', 'recipient', 'externalRef', 'timestamp', 'balance'].map((k) => <Field key={k} label={`${k} pattern`}><Input className="mono" value={tpl.patterns[k] ?? ''} onChange={(e) => setTpl({ ...tpl, patterns: { ...tpl.patterns, [k]: e.target.value } })} /></Field>)}
            <Switch on={!!tpl.enabled} onChange={(v) => setTpl({ ...tpl, enabled: v })} label="Enabled" />
            <div className="mt"><Button disabled={!tpl.name || !tpl.operatorId} onClick={() => { const patterns = Object.fromEntries(Object.entries(tpl.patterns).filter(([, v]) => (Array.isArray(v) ? v.length > 0 : !!v))); api.put(`/api/admin/evidence/templates/${tpl.id}`, { operatorId: tpl.operatorId, name: tpl.name, priority: tpl.priority, enabled: tpl.enabled, patterns }).then(() => { toast('Template saved', 'success'); setTpl(null); templates.reload(); }).catch((e) => toast(e.message, 'error')); }}>Save</Button></div>
          </>
        )}
      </Modal>
      {tab === 'routes' && (
