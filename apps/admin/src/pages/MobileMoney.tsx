...
export function MobileMoney() {
  ...
  const [dev, setDev] = useState({ name: '', publicKey: '', operatorIds: '' });
      ...
      {tab === 'devices' && (
        <div className="grid cols-2">
          <div className="card">
            ...
            <Button disabled={!dev.name || dev.publicKey.length < 32} onClick={() => api.post('/api/admin/evidence/devices', { name: dev.name, publicKey: dev.publicKey, operatorIds: dev.operatorIds.split(',').map((s) => s.trim()).filter(Boolean) }).then(() => { toast('Device registered', 'success'); setDev({ name: '', publicKey: '', operatorIds: '' }); devices.reload(); }).catch((e) => toast(e.message, 'error'))}>Register device</Button>
          </div>
          <div className="card">
            <h4>Registered devices</h4>
            <Table head={['Name', 'Id', 'Operators', 'Status', 'Risk', 'Last seen', '']} rows={(devices.data?.items ?? []).map((d: any) => [<b>{d.name}</b>, <span className="mono tiny">{d.id}</span>, d.operatorIds.length ? d.operatorIds.join(', ') : 'any', <StatusBadge status={d.status} />, <Chip kind={d.riskScore >= 50 ? 'danger' : d.riskScore > 0 ? 'warning' : 'success'}>{d.riskScore}</Chip>, fmtDate(d.lastSeenAt), d.status === 'active' ? <ConfirmButton size="sm" variant="danger" prompt="Reason" onConfirm={(r) => api.del(`/api/admin/evidence/devices/${d.id}`).then(() => { toast(`Revoked: ${r}`, 'success'); devices.reload(); })}>Revoke</ConfirmButton> : null])} empty="No devices registered – signed evidence cannot be received yet" />
          </div>
        </div>
      )}
      {tab === 'templates' && (
