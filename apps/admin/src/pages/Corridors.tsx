      ...
      {tab === 'corridors' && (
        <div className="card">
          <Table head={['Corridor', 'Rail / operator', 'Status', 'Arrangements', 'ETA', 'Max', '']} rows={(corridors.data?.items ?? []).map((c: any) => [
            ...
            <span className="tiny">{c.collectionPartner ? `in: ${c.collectionPartner}` : 'no collection partner'}<br />{c.payoutPartner ? `out: ${c.payoutPartner}` : 'no payout partner'}<br />{c.licenceRef ? `licence: ${c.licenceRef}` : 'no licence ref'}{c.approvedBy ? <><br />live since {fmtDate(c.approvedAt)}</> : null}</span>, `${c.estimatedPayoutMinutes} min`, c.maxAmount ? c.maxAmount : '—',
