...
export function Chip({ children, kind, onClick, selected }: { children: ReactNode; kind?: 'success' | 'warning' | 'danger' | 'primary'; onClick?: () => void; selected?: boolean }) {
  return <span className={`chip ${kind ?? ''} ${onClick ? 'clickable' : ''} ${selected ? 'selected' : ''}`} onClick={onClick}>{children}</span>;
}
const KIND: Record<string, 'success' | 'warning' | 'danger' | 'primary' | undefined> = { completed: 'success', paid: 'success', succeeded: 'success', verified: 'success', active: 'success', open: 'primary', answered: 'primary', negotiating: 'primary', pending: 'warning', escrowed: 'warning', ready_for_pickup: 'warning', processing: 'warning', initiated: 'warning', new: 'warning', failed: 'danger', rejected: 'danger', declined: 'danger', reversed: 'danger', disputed: 'danger', suspended: 'danger', frozen: 'warning', replied: 'success' };
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className={`modal ${wide ? 'wide' : ''}`}>
        <div className="modal-title"><h3 style={{ margin: 0 }}>{title}</h3><button className="btn ghost sm" onClick={onClose}>✕</button></div>
        {children}
      </div>
    </div>
  );
}
export const Empty = ({ icon = '🗂️', text = 'Nothing here yet' }: { icon?: string; text?: string }) => <div className="empty"><div className="ico">{icon}</div><div>{text}</div></div>;
export const Loading = () => <div className="loading-page"><span className="spinner" /></div>;
export function Tabs({ tabs, value, onChange, pills }: { tabs: { id: string; label: string }[]; value: string; onChange: (id: string) => void; pills?: boolean }) {
  return <div className={pills ? 'pill-tabs' : 'tabs'}>{tabs.map((t) => <button key={t.id} type="button" className={`tab ${value === t.id ? 'active' : ''}`} onClick={() => onChange(t.id)}>{t.label}</button>)}</div>;
}
export const KV = ({ k, v }: { k: ReactNode; v: ReactNode }) => <div className="kv"><span className="k">{k}</span><span className="v">{v}</span></div>;
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  return <div className="page-header"><div><h1>{title}</h1>{subtitle && <p>{subtitle}</p>}</div>{actions && <div className="row wrap">{actions}</div>}</div>;
}
  ...
  return (
    <div className="table-wrap">
      <table>
        ...
        <tbody>{rows.length === 0 ? <tr><td colSpan={head.length}><Empty text={empty} /></td></tr> : rows.map((r, i) => <tr key={i}>{r.map((c, j) => <td key={j}>{c}</td>)}</tr>)}</tbody>
      </table>
    </div>
  );
}
export function Pager({ page, total, pageSize, onPage }: { page: number; total: number; pageSize: number; onPage: (p: number) => void }) {
  ...
  return <div className="row between mt"><span className="small muted">{total} total</span><div className="row"><Button size="sm" variant="secondary" disabled={page <= 1} onClick={() => onPage(page - 1)}>←</Button><span className="small">{page} / {pages}</span><Button size="sm" variant="secondary" disabled={page >= pages} onClick={() => onPage(page + 1)}>→</Button></div></div>;
}
...
export function Switch({ on, onChange, label }: { on: boolean; onChange: (v: boolean) => void; label?: ReactNode }) {
  return <label className="row" style={{ cursor: 'pointer' }}><button type="button" className={`switch ${on ? 'on' : ''}`} onClick={() => onChange(!on)} />{label && <span>{label}</span>}</label>;
}
export function UserCell({ user }: { user?: PublicUser | null }) {
  if (!user) return <span className="muted">—</span>;
  return <div className="row"><Avatar user={user} size="sm" /><div><div className="bold small">{user.businessName || user.fullName}</div><div className="tiny muted">@{user.tag}</div></div></div>;
}
export function Money({ minor, currency }: { minor: number; currency: string }) {
  const { money } = useStore();
  return <>{money(minor, currency)}</>;
}
export function ConfirmButton({ onConfirm, children, prompt, variant, size }: { onConfirm: (reason?: string) => void; children: ReactNode; prompt?: string; variant?: 'danger' | 'secondary' | 'success' | 'ghost'; size?: 'sm' }) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Please confirm">
        {prompt ? <Field label={prompt}><Input value={reason} onChange={(e) => setReason(e.target.value)} autoFocus /></Field> : <p>Are you sure?</p>}
        <div className="row"><Button variant={variant === 'danger' ? 'danger' : undefined} disabled={!!prompt && reason.length < 2} onClick={() => { onConfirm(reason); setOpen(false); setReason(''); }}>Confirm</Button><Button variant="ghost" onClick={() => setOpen(false)}>Cancel</Button></div>
      </Modal>
    </>
  );
}
