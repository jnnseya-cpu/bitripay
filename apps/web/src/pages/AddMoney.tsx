...
export function AddMoney() {
  ...
  return (
    <div>
      <PageHeader title={t('addMoney.title')} subtitle="Fund your wallet with a card, mobile money, bank transfer or cash at an agent. External payments are credited only after they are independently confirmed." />
      <div className="grid cols-3">
        <div className="card" style={{ gridColumn: 'span 2' }}>
          {payment ? (
            <>
              {error && <Alert kind="error">{error}</Alert>}
              <PaymentStatus payment={payment} declaration={declaration} onDone={() => { setPayment(null); setDeclaration(null); setError(null); }} proof={proof} setProof={setProof} onSent={markSent} onAuthenticate={needsAuth ? () => setPinOpen(true) : undefined} />
            </>
          ) : (
            <>
              {error && <Alert kind="error">{error}</Alert>}
              ...
              {method === 'card' && (
                <>
                  ...
                  {!savedCardId && (
                    <label className="checkbox mb"><input type="checkbox" checked={saveCard} onChange={(e) => setSaveCard(e.target.checked)} /> Save this card for next time (tokenised by the processor – we never store the number or CVC)</label>
                  )}
                  {gw?.provider === 'sandbox' && <div className="alert info small">Sandbox processor: use 4242 4242 4242 4242 (any future expiry, any CVC). Cards ending 0002 are declined. No real money moves.</div>}
                </>
              )}
              {method === 'mobile_money' && (
                <>
              ...
              {method === 'bank' && <Alert kind="info">You'll receive bank details and a reference. Your wallet is credited only after the transfer is independently confirmed (bank notification or a verifier checking the statement).</Alert>}
              {amount && opt && (
      ...
      <PinModal open={pinOpen} onClose={() => setPinOpen(false)} onSubmit={needsAuth ? authenticate : submit} loading={loading} title="Authorise this payment" summary={<KV k={`Add money via ${methodLabels[method]}`} v={payment ? money(payment.amount, payment.currency) : `${amount} ${cur}`} />} />
    </div>
  );
}
  ...
  return (
    <>
      <div className="grid cols-2">
        ...
        <Field label="Mobile money operator" hint={value && list.find((o) => o.id === value)?.directRail ? 'Direct rail: you pay from your own mobile money app with a reference; confirmed from the operator receipt' : value ? 'Routed through a connected gateway or the sandbox' : undefined}>
      ...
      {value && (() => { const o = list.find((x) => x.id === value); return o ? <div className="row wrap mb"><span className="chip" style={{ background: o.color, color: '#fff' }}>{o.brand}</span>{o.ussd && <span className="chip">USSD {o.ussd}</span>}<span className="chip">{o.currency}</span>{o.directRail && <span className="chip success">direct rail · verified from receipt</span>}</div> : null; })()}
    </>
  );
}
...
export function PaymentStatus({ payment, declaration, onDone, proof, setProof, onSent, onAuthenticate }: { payment: PaymentView; declaration?: any; onDone: () => void; proof?: string; setProof?: (v: string) => void; onSent?: () => void; onAuthenticate?: () => void }) {
  const { money } = useStore();
  ...
  return (
    <div>
      <div className="center">
        ...
        <p className="muted small mt-sm">{payment.gatewayName} · {payment.method.replace('_', ' ')}{payment.authMethod ? ` · authorised by ${payment.authMethod}` : ''}</p>
      </div>
      ...
      {payment.stage === 'AUTHENTICATION_REQUIRED' && onAuthenticate && <Button block className="mt" onClick={onAuthenticate}>🔐 Confirm with biometrics or PIN</Button>}
      {open && payment.next?.type === 'prompt' && <Alert kind="info">{payment.next.message} <span className="spinner" style={{ verticalAlign: 'middle', marginLeft: 8 }} /></Alert>}
      ...
      {open && payment.next?.type === 'redirect' && payment.next.url && <a className="btn block" href={payment.next.url}>Continue to {payment.gatewayName}</a>}
      {open && payment.next?.type === 'bank_instructions' && (
        <div>
          ...
          {payment.stage === 'INSTRUCTION_ISSUED' && setProof && onSent && (
            <div className="mt">
              <Field label={payment.method === 'mobile_money' ? 'Transaction ID from your mobile money receipt (optional)' : 'Bank reference (optional)'} hint="Supporting note only – settlement is based on the operator/bank confirmation, never on a typed reference or screenshot">
                <div className="row">
                  ...
                  <Button variant="secondary" onClick={onSent}>I have sent the money</Button>
                </div>
              </Field>
            </div>
          )}
          {payment.stage === 'PAYMENT_SENT' && <Alert kind="warning">Waiting for independent confirmation. Nothing has been credited yet.</Alert>}
          {['MANUAL_REVIEW', 'MISMATCHED', 'DUPLICATE'].includes(payment.stage ?? '') && <Alert kind="warning">A verifier is reviewing this payment. You will be notified once it is confirmed or rejected; nothing is credited until then.</Alert>}
          {payment.expiresAt && payment.stage === 'INSTRUCTION_ISSUED' && <div className="tiny muted mt-sm">Pay before {new Date(payment.expiresAt).toLocaleString()} – unconfirmed intents expire automatically.</div>}
        </div>
      )}
      <RouteDisclosure declaration={declaration} />
      <Button block variant="secondary" className="mt" onClick={onDone}>{open ? 'Back' : 'Done'}</Button>
    </div>
  );
}
