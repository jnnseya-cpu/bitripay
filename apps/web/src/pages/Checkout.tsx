...
export function Checkout() {
  ...
  if (error) return <div className="auth-page"><div className="card"><Alert kind="error">{error}</Alert><Link to="/">Go to BitriPay</Link></div></div>;
      ...
      <div style={{ width: '100%', maxWidth: 900 }}>
        <div className="grid cols-2">
          <div className="card" style={{ borderTop: `6px solid ${merchant.brandColor}` }}>
            <div className="list-item">{merchant.logoUrl ? <img src={merchant.logoUrl} alt="" style={{ height: 40, borderRadius: 8 }} /> : <Avatar user={merchant} size="lg" />}<div><div className="main-text" style={{ fontSize: '1.15rem' }}>{merchant.businessName || merchant.fullName}</div><div className="sub-text">@{merchant.tag} {merchant.testMode && <span className="chip warning">test mode</span>}</div></div></div>
            <h1 className="mt" style={{ fontSize: '2.4rem' }}>{fixed ? money(pr.amount!) : `Any amount (${cur.code})`}</h1>
            ...
            <div className="center mt"><QrImage value={pr.link!} size={160} /><div className="tiny muted mt-sm">Scan with the BitriPay app</div></div>
          </div>
          <div className="card">
            {done || pr.status === 'paid' ? (
              <div className="center">
                ...
                {successUrl && <a className="btn block" href={successUrl}>Return to merchant</a>}
                {!successUrl && user && <Button block variant="secondary" onClick={() => nav('/app')}>Back to BitriPay</Button>}
              </div>
            ) : pr.status !== 'open' ? (
