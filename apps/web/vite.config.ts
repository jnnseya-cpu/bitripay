...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      ...Object.fromEntries(['/brand', '/lite', '/blog', '/legal', '/about', '/contact', '/sitemap.xml', '/feed.xml', '/robots.txt', '/llms.txt', '/llms-full.txt'].map((p) => [p, { target: process.env.VITE_API_PROXY || 'http://localhost:4000', changeOrigin: true }])),
    },
  },
