...
import { OperatorPicker } from './Money';
...
export function Move() {
  ...
  useEffect(() => {
    if (!route || route.status !== 'funding') return;
    const id = setInterval(() => api.get<{ route: any }>(`/api/money/${route.id}`).then((r) => { setRoute(r.route); if (r.route.status !== 'funding') refreshWallets(); }), 3000);
  ...
  const submit = async (p?: string) => {
    ...
    try {
      ...
      const r = await api.post<{ route: any }>('/api/money', { source: s, destination: destination(), amount, currency: cur, targetCurrency: target || null, pin: p });
      setRoute(r.route);
      setPin(false);
      refreshWallets();
  ...
  return (
    <Screen>
      ...
      <T muted size={13}>From any bank, card, QR code or mobile money to any other – end to end.</T>
      {route ? (
        <Card style={{ alignItems: 'center' }}>
          ...
          {route.status === 'funding' && route.payment?.next?.type === 'bank_instructions' && <View style={{ alignSelf: 'stretch' }}><Alert text={route.payment.next.message} />{Object.entries(route.payment.next.instructions ?? {}).map(([k, v]) => <KV key={k} k={k} v={String(v)} />)}</View>}
          {route.status === 'funding' && route.payment?.next?.type === 'prompt' && <Alert text={route.payment.next.message} />}
          {route.status === 'pending' && <Alert text="Money arrived; the payout is queued and completed by our team or a local agent." />}
        ...
        <Card>
          ...
          {preview && <Card soft><Row>{preview.destination.user && <Avatar user={preview.destination.user} size={30} />}<T bold>{preview.destination.label}</T></Row>{preview.quote.rate !== 1 && <KV k="Rate" v={preview.quote.rate.toFixed(4)} />}<KV k="Recipient gets" v={money(preview.quote.targetAmount, preview.quote.targetCurrency)} /></Card>}
          <Button title={source === 'wallet' ? 'Review & confirm' : 'Pay and deliver'} loading={loading} disabled={!preview} onPress={() => (source === 'wallet' ? setPin(true) : submit())} />
        </Card>
      )}
