...
export function Move() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => api.get<{ route: any }>(`/api/money/${route.id}`).then((r) => { if (r.route.stage !== route.stage || r.route.payment?.stage !== route.payment?.stage || r.route.payout?.stage !== route.payout?.stage) setRoute(r.route); if (['SETTLED', 'EXPIRED', 'FAILED', 'REVERSED', 'REFUNDED'].includes(r.route.stage)) refreshWallets(); }), 8000);
    ...
    <Screen>
      ...
      {route ? (
        <Card style={{ alignItems: 'center' }}>
          ...
          {route.stage === 'AWAITING_CONFIRMATION' && route.consent && <><Alert kind="warning" text={`The recipient must confirm receiving ${route.targetCurrency} before the payout is executed. Share the confirmation link.`} /><Button title="Share confirmation link" variant="secondary" onPress={() => Share.share({ message: `Please confirm the currency for the money I am sending you: ${route.consent.url}` })} /></>}
          {route.confirmationMethod && <T size={11} muted center>Settled on {String(route.confirmationMethod).replace(/_/g, ' ').toLowerCase()}</T>}
          ...
          {['FUNDING_PENDING', 'BIOMETRIC_APPROVAL_REQUIRED'].includes(route.stage) && route.payment?.stage && <StageBar stage={route.payment.stage} label={route.payment.stageLabel} description={route.payment.stageDescription} />}
        ...
        <Card>
          ...
          {preview?.quote?.receivingCurrencies ? (
            <View>
              <T size={12} muted>Recipient receives in (default: local currency {preview.quote.receivingCurrencies.defaultCurrency})</T>
              <Row style={{ flexWrap: 'wrap' }}>
                {preview.quote.receivingCurrencies.options.map((o: any) => <Chip key={o.currency} label={`${o.currency}${o.isLocal ? ' · local' : ''}${o.consentRequired ? ' · recipient confirms' : ''}${o.available ? '' : ' · unavailable'}`} kind={o.available ? ((target || preview.quote.targetCurrency) === o.currency ? 'primary' : undefined) : 'danger'} selected={(target || preview.quote.targetCurrency) === o.currency} onPress={o.available ? () => setTarget(o.currency) : undefined} />)}
              </Row>
              {preview.quote.receivingCurrencies.options.filter((o: any) => !o.available).map((o: any) => <T key={o.currency} size={11} muted>{o.currency}: {o.reasons.join('; ')}</T>)}
            </View>
          ) : (
          ...
          {preview && <Card soft><Row>{preview.destination.user && <Avatar user={preview.destination.user} size={30} />}<T bold>{preview.destination.label}</T></Row>{preview.quote.rate !== 1 && <KV k="Rate" v={`${preview.quote.rate.toFixed(4)} · ${preview.fx?.guaranteed ? 'guaranteed' : 'indicative'}`} />}{preview.fx && preview.fx.sourceCurrency !== preview.fx.targetCurrency && <T size={11} muted>Reference {preview.fx.midRate.toFixed(4)} · {preview.fx.providerLabel} · markup {(preview.fx.markupBps / 100).toFixed(2)}%</T>}<KV k="Recipient gets" v={money(preview.quote.targetAmount, preview.quote.targetCurrency)} /><KV k="Guaranteed" v={preview.quote.guaranteedRecipientAmount != null ? money(preview.quote.guaranteedRecipientAmount, preview.quote.targetCurrency) : 'indicative only'} /><KV k="Estimated payout" v={preview.quote.estimatedPayoutTime} />{preview.quote.confirmation && <T size={11} muted>Confirmed by: funding {preview.quote.confirmation.funding.replace(/_/g, ' ').toLowerCase()} · payout {preview.quote.confirmation.payout.replace(/_/g, ' ').toLowerCase()}</T>}{preview.quote.recipientConsentRequired && <Alert kind="warning" text={`The recipient must confirm receiving ${preview.quote.targetCurrency} before the payout is executed.`} />}{preview.quote.corridor && <T size={11} muted>{preview.quote.corridor.status === 'live' ? 'Authorised corridor' : 'Sandbox corridor – no real funds'} · {preview.quote.corridor.destCountry}</T>}<T size={11} muted>Refunds: {preview.quote.refundConditions}</T>{preview.declaration && <T size={11} muted>{preview.declaration.processing} · {preview.declaration.expectedCompletion} · {preview.declaration.funding.confirmation}</T>}</Card>}
          <Button title="🔐 Confirm and send" loading={loading} disabled={!preview} onPress={() => { setPinFor('send'); setPin(true); }} />
        </Card>
      )}
      ...
      <PinSheet open={pin} onClose={() => setPin(false)} onSubmit={(p) => (pinFor === 'cancel' && route ? api.post<{ route: any }>(`/api/money/${route.id}/cancel`, { pin: p || undefined }).then((r) => { setPin(false); setRoute(r.route); refreshWallets(); toast('Transfer cancelled', 'success'); }).catch((e) => { setPin(false); toast(e.message, 'error'); }) : submit(p))} loading={loading} title={pinFor === 'cancel' ? 'Confirm cancellation' : undefined} summary={pinFor === 'cancel' && route ? <KV k="Cancel transfer" v={money(route.amount, route.currency)} /> : preview && <KV k={`Send to ${preview.destination.label}`} v={money(preview.quote.targetAmount, preview.quote.targetCurrency)} />} />
    </Screen>
  );
}
