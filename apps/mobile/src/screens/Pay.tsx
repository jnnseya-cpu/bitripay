...
export function PayTarget({ route }: ScreenProps<'PayTarget'> | ScreenProps<'Checkout'>) {
  ...
  let minor = 0;
  try { minor = amount ? toMinor(amount, currency(cur).decimals) : 0; } catch { minor = -1; }
  useEffect(() => {
  ...
  if (resolved.kind === 'agent') {
    return (
      <Screen><Header title="Agent" />
        <Card><Row><Avatar user={target} /><View><T bold>{target.businessName || target.fullName}</T><T muted>@{target.tag}</T></View></Row><T muted size={13}>Agents let you deposit or withdraw cash.</T>
  ...
  return (
    <Screen>
      ...
      <Card>
        ...
        {minor > 0 && <Card soft><KV k={t('common.fee')} v={feeOnMe ? money(fee, cur) : 'Paid by merchant'} /><KV k={t('common.total')} v={money(total, cur)} /><KV k={t('common.balance')} v={wallet ? money(wallet.balance, wallet.currency) : `No ${cur} wallet`} /></Card>}
        <Button title={`Pay ${minor > 0 ? money(total, cur) : ''}`} onPress={() => setPin(true)} disabled={minor <= 0 || !wallet || wallet.balance < total || (!!pr && pr.status !== 'open') || target.id === user?.id} />
      </Card>
...
export function Send({ route }: ScreenProps<'Send'>) {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => api.get<{ user: PublicUser }>(`/api/account/lookup?q=${encodeURIComponent(to)}`).then((r) => setRecipient(r.user)).catch(() => setRecipient(null)), 350);
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => api.get<{ fee: number }>(`/api/transfers/fee?amount=${amount}&currency=${cur}&type=${recipient?.role === 'merchant' ? 'merchant_payment' : 'transfer'}`).then((r) => setFee(r.fee)).catch(() => setFee(0)), 300);
    return () => clearTimeout(id);
  ...
  return (
    <Screen>
      ...
      <Card>
        ...
        {recipient && <Card soft><Row><Avatar user={recipient} /><View><T bold>{recipient.businessName || recipient.fullName}</T><T muted>@{recipient.tag}</T></View></Row></Card>}
        <AmountInput label={t('common.amount')} amount={amount} currency={cur} onAmount={setAmount} onCurrency={setCur} />
        {wallet && <T muted size={12}>{t('common.balance')}: {money(wallet.balance, wallet.currency)}</T>}
        <Input label={t('common.note')} value={note} onChangeText={setNote} />
        {minor > 0 && <Card soft><KV k={t('common.fee')} v={feeOnMe ? money(fee, cur) : 'Paid by merchant'} /><KV k={t('common.total')} v={money(total, cur)} /></Card>}
        <Button title={t('send.review')} onPress={() => setPin(true)} disabled={!recipient || minor <= 0 || !wallet || wallet.balance < total} />
      </Card>
      <PinSheet open={pin} onClose={() => setPin(false)} onSubmit={submit} loading={loading} summary={<><KV k="To" v={`${recipient?.fullName} (@${recipient?.tag})`} /><KV k={t('common.total')} v={money(total, cur)} /></>} />
    </Screen>
  );
}
...
export function Receive() {
  ...
  useEffect(() => {
    ...
    api.get<{ content: string }>(`/api/qr/me${q}`).then(setData).catch(() => setData(null));
  }, [amount, cur, note]);
  return (
    <Screen title={t('receive.title')}>
      ...
      <Card style={{ alignItems: 'center' }}>
        ...
        <T bold size={20}>@{user?.tag}</T>
        <T muted>{user?.businessName || user?.fullName}</T>
        {amount ? <T bold>{amount} {cur}{note ? ` · ${note}` : ''}</T> : null}
        <Row>
...
export function Requests() {
  ...
  const [loading, setLoading] = useState(false);
  const list = useAsync(() => api.get<{ items: PaymentRequest[] }>(tab === 'incoming' ? '/api/payment-requests?role=payer&status=open' : `/api/payment-requests?role=requester&pageSize=50${tab === 'mine' ? '&kind=request' : ''}`), [tab]);
  ...
  return (
    <Screen>
      ...
      <Row><Button title="🙋 Request money" small variant="secondary" onPress={() => setCreate('request')} /><Button title="🔗 Payment link" small onPress={() => setCreate('link')} /></Row>
      <Tabs tabs={[{ id: 'incoming', label: 'To pay' }, { id: 'mine', label: 'My requests' }, { id: 'links', label: 'My links' }]} value={tab} onChange={(v) => setTab(v as any)} />
      {items.length === 0 && <Empty icon="🔗" />}
      {items.map((r) => (
        <Card key={r.id}>
          <Row between><T bold>{r.amount != null ? money(r.amount, r.currency) : `Any amount (${r.currency})`}</T><Status status={r.status} /></Row>
          <T muted size={13}>{r.description ?? ''} {tab === 'incoming' ? `· from @${r.requester?.tag}` : r.payer ? `· to @${r.payer.tag}` : ''}</T>
          <Row>
            {tab === 'incoming' && r.status === 'open' && <><Button title="Pay" small onPress={() => setPayCode(r)} /><Button title="Decline" small variant="secondary" onPress={() => api.post(`/api/payment-requests/${r.code}/decline`).then(list.reload)} /></>}
            {tab !== 'incoming' && <><Button title="View QR" small variant="secondary" onPress={() => setCreated(r)} />{r.status === 'open' && <Button title={t('common.cancel')} small variant="ghost" onPress={() => api.post(`/api/payment-requests/${r.code}/cancel`).then(list.reload)} />}</>}
          </Row>
        </Card>
      ))}
      <Sheet open={!!create} onClose={() => setCreate(null)} title={create === 'link' ? 'New payment link' : 'Request money'}>
        ...
        <AmountInput label={t('common.amount')} amount={form.amount} currency={form.currency} onAmount={(a) => setForm({ ...form, amount: a })} onCurrency={(c) => setForm({ ...form, currency: c })} currencies={(config?.currencies ?? []).map((c: any) => c.code)} />
      ...
      <Sheet open={!!created} onClose={() => setCreated(null)} title="Payment link">
        {created && <View style={{ alignItems: 'center', gap: 10 }}><Qr value={created.link!} size={200} /><T bold size={20}>{created.amount != null ? money(created.amount, created.currency) : 'Any amount'}</T><T muted size={12}>{created.link}</T><Row><Button title="Copy link" small variant="secondary" onPress={() => Clipboard.setStringAsync(created.link!)} /><Button title={t('common.share')} small onPress={() => Share.share({ message: created.link! })} /></Row></View>}
      </Sheet>
      <PinSheet open={!!payCode} onClose={() => setPayCode(null)} onSubmit={pay} loading={loading} summary={payCode && <KV k={`Pay @${payCode.requester?.tag}`} v={money(payCode.amount ?? 0, payCode.currency)} />} />
    </Screen>
  );
}
