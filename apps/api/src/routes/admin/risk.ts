...
import { listCases, getCase, openCase, assignCase, escalateCase, updateSarDraft, decideCase, closeCase, complianceOverview, runAmlScan, getAmlSettings, listSources, upsertSource, importSanctionsRows, parseSanctionsCsv, refreshSource, CASE_DECISIONS } from '../../services/risk/compliance';
...
import { floatForecast, computeTrustScore, latestTrustScore, runTrustScores, runFloatAlerts, listFloatRequests, fulfilFloatRequest, rejectFloatRequest, getAgentIntelSettings, dynamicCommissionBps } from '../../services/risk/agentIntel';
...
// ---------------------------------------------------------------- policies
const ruleSchema = z.object({ id: z.string(), description: z.string().max(200), when: z.object({ kinds: z.array(z.string()).optional(), minBase: z.number().optional().nullable(), maxBase: z.number().optional().nullable(), minScore: z.number().optional().nullable(), maxScore: z.number().optional().nullable(), kycTiers: z.array(z.number().int()).optional(), countries: z.array(z.string().length(2)).optional(), methods: z.array(z.string()).optional(), newBeneficiary: z.boolean().optional().nullable(), flags: z.array(z.string()).optional() }), action: z.enum(['allow', 'step_up', 'review', 'block']), reason: z.string().max(60) });
...
r.post('/policies/simulate', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ policyId: z.string().optional().nullable(), kind: z.string(), baseMinor: z.number().int().min(0), score: z.number().int().min(0).max(100), kycTier: z.number().int().optional().nullable(), country: z.string().optional().nullable(), method: z.string().optional().nullable(), newBeneficiary: z.boolean().optional().nullable(), flags: z.array(z.string()).optional() }), req.body);
  res.json(simulatePolicy({ ...b, flags: b.flags ?? [] }, b.policyId));
});

// ---------------------------------------------------------------- fraud scores
r.get('/fraud', requirePermission('compliance'), (req, res) => res.json({ ...fraudOverview(Number(req.query.days) || 7), items: listFraudScores({ userId: req.query.user ? String(req.query.user) : null, band: req.query.band ? String(req.query.band) : null, action: req.query.action ? String(req.query.action) : null, limit: Number(req.query.limit) || 100 }) }));
r.put('/fraud/settings', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ bands: z.object({ stepUp: z.number().int().min(1).max(100), review: z.number().int().min(1).max(100), block: z.number().int().min(1).max(100) }).partial().optional(), methodRisk: z.record(z.string(), z.number().min(0).max(100)).optional(), velocityThresholds: z.object({ hour: z.number().int(), day: z.number().int(), week: z.number().int() }).partial().optional(), velocityPoints: z.object({ hour: z.number().int(), day: z.number().int(), week: z.number().int() }).partial().optional(), amountDeviationPoints: z.number().int().optional(), newBeneficiaryPoints: z.number().int().optional(), recipientRiskPoints: z.number().int().optional(), kycMismatchPoints: z.number().int().optional(), nightHours: z.tuple([z.number().int().min(0).max(23), z.number().int().min(0).max(23)]).optional(), nightPoints: z.number().int().optional(), newDevicePoints: z.number().int().optional(), geoMismatchPoints: z.number().int().optional(), structuringPoints: z.number().int().optional(), minScoredBase: z.number().int().min(0).optional() }), req.body);
...
// ---------------------------------------------------------------- compliance cases
r.get('/cases', requirePermission('compliance'), (req, res) => res.json({ ...complianceOverview(), items: listCases({ status: req.query.status ? String(req.query.status) : null, kind: req.query.kind ? String(req.query.kind) : null, userId: req.query.user ? String(req.query.user) : null, assignedTo: req.query.assigned ? String(req.query.assigned) : null, severity: req.query.severity ? String(req.query.severity) : null, limit: Number(req.query.limit) || 100 }), decisions: CASE_DECISIONS }));
r.post('/cases', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ userId: z.string().optional().nullable(), subjectType: z.string().max(40).optional().nullable(), subjectId: z.string().max(80).optional().nullable(), severity: z.enum(['low', 'medium', 'high', 'critical']).optional(), title: z.string().min(3).max(200), summary: z.string().min(3).max(4000), indicators: z.array(z.string().max(200)).max(50).optional(), sar: z.boolean().optional() }), req.body);
...
r.put('/aml/settings', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ highRiskCountries: z.array(z.string().length(2)).optional(), structuring: z.object({ window24hCount: z.number().int().min(2), nearLimitPct: z.number().int().min(50).max(100) }).partial().optional(), passThrough: z.object({ distinctSenders24h: z.number().int().min(2), forwardedPct: z.number().int().min(10).max(100) }).partial().optional(), dormantDays: z.number().int().min(7).optional(), burstMultiplier: z.number().min(1).optional(), pepPoints: z.number().int().min(0).max(100).optional() }), req.body);
...
// ---------------------------------------------------------------- sanctions sources
r.get('/sanctions/sources', requirePermission('compliance'), (req, res) => res.json({ items: listSources(), entries: listSanctions({ source: req.query.source ? String(req.query.source) : null, kind: req.query.kind ? String(req.query.kind) : null, limit: Number(req.query.limit) || 200 }) }));
r.put('/sanctions/sources/:id', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ name: z.string().min(2).max(120), url: z.string().url().optional().nullable(), format: z.enum(['csv', 'json']).optional(), kind: z.enum(['sanctions', 'pep']).optional(), enabled: z.boolean().optional() }), req.body);
...
r.post('/sanctions/sources/:id/import', requirePermission('compliance'), (req, res) => {
  const b = validate(z.object({ version: z.string().min(1).max(80), rows: z.array(z.object({ kind: z.enum(['name', 'phone', 'email', 'country', 'pep']), value: z.string().min(1).max(200), externalId: z.string().max(80).optional().nullable(), note: z.string().max(300).optional().nullable() })).optional(), csv: z.string().max(20_000_000).optional() }), req.body);
...
r.post('/sanctions/sources/:id/refresh', requirePermission('compliance'), wrap(async (req, res) => {
...
r.put('/kyc/tiers', requirePermission('settings'), (req, res) => {
  ...
  const b = validate(z.object({ default: z.record(z.string(), limits).optional(), countries: z.record(z.string(), z.record(z.string(), limits)).optional(), addressDocMaxAgeDays: z.number().int().min(1).max(365).optional(), kybMonthlyVolumeThreshold: z.number().int().min(0).optional() }), req.body);
...
// ---------------------------------------------------------------- destination changes
r.get('/destination-changes', requirePermission('compliance'), (req, res) => res.json({ items: listDestinationChanges({ userId: req.query.user ? String(req.query.user) : null, status: req.query.status ? String(req.query.status) : null, limit: Number(req.query.limit) || 100 }), settings: getAccountProtectionSettings() }));
...
r.get('/agents/overview', requirePermission('agents'), (_req, res) => {
  ...
  res.json({ settings: getAgentIntelSettings(), agents: agents.map((a) => ({ id: a.id, name: a.business_name || a.full_name, tag: a.tag, country: a.country, trustScore: a.trust_score, band: latestTrustScore(a.id)?.band ?? null, float: floatForecast(a.id), commission: dynamicCommissionBps({ ...a, agent_commission_bps: null } as any, 'cash_in') })), requests: listFloatRequests({ status: 'REQUESTED', limit: 100 }) });
});
r.get('/agents/:id/trust', requirePermission('agents'), (req, res) => res.json({ latest: latestTrustScore(String(req.params.id)), history: (getDb().prepare('SELECT score, band, commission_bonus_bps, computed_at FROM agent_scores WHERE agent_user_id = ? ORDER BY computed_at DESC LIMIT 60').all(String(req.params.id)) as any[]).map((h) => ({ score: h.score, band: h.band, bonusBps: h.commission_bonus_bps, at: h.computed_at })) }));
...
r.get('/agents/float-requests', requirePermission('agents'), (req, res) => res.json({ items: listFloatRequests({ status: req.query.status ? String(req.query.status) : null, agentId: req.query.agent ? String(req.query.agent) : null, limit: Number(req.query.limit) || 100 }) }));
...
r.put('/agents/settings', requirePermission('settings'), (req, res) => {
  const b = validate(z.object({ targetDays: z.number().min(1).max(30).optional(), alertDays: z.number().min(0).max(30).optional(), bonusByBand: z.record(z.string(), z.number().int().min(0).max(500)).optional(), liquidityBonusBps: z.number().int().min(0).max(500).optional(), minTenureDays: z.number().int().min(0).optional(), onboardingCommissionMinor: z.number().int().min(0).optional() }), req.body);
