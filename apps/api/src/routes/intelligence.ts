...
// ---------------------------------------------------------------- offline protocol
r.get('/offline/settings', requireAuth, (_req, res) => res.json({ ...getOfflineSettings(), promiseVersion: 'v1', canonical: 'BITRIQR-OFFLINE|v1|merchantId|payerId|amountMinor|CURRENCY|nonce|expiresAt|counter|reference' }));
...
r.post('/offline/qr', requireAuth, requireRole('merchant', 'admin'), requireScope('qr_codes:write', 'qr:create'), writeLimit, (req, res) => {
  const b = validate(z.object({ amount: z.string().optional(), amount_minor: z.number().int().positive().optional(), currency: z.string().length(3), reference: z.string().max(40).optional().nullable(), ttl_seconds: z.number().int().min(60).max(3600).optional().nullable() }), req.body);
...
r.get('/offline/promises', requireAuth, (req, res) => res.json({ data: listPromises(req.user!.id, { state: req.query.state ? String(req.query.state) : null, limit: Number(req.query.limit) || 100 }) }));

// ---------------------------------------------------------------- Diaspora-Direct
r.get('/diaspora/rate-cards', (_req, res) => res.json({ data: listRateCards().map((c) => ({ id: c.id, sourceCurrency: c.sourceCurrency, destCurrency: c.destCurrency, customerRate: c.customerRate, validFrom: c.validFrom, validUntil: c.validUntil, keyId: c.keyId, signature: c.signature })), purposes: purposeCatalogue(), restricted: RESTRICTED_PURPOSES }));
r.get('/institutions', (req, res) => res.json({ data: listInstitutions({ status: 'verified', country: req.query.country ? String(req.query.country) : null, purposeCode: req.query.purpose ? String(req.query.purpose) : null, q: req.query.q ? String(req.query.q) : null }).map((i) => ({ userId: i.userId, kind: i.kind, name: i.name, purposeCodes: i.purposeCodes, country: i.country, tag: i.user?.tag ?? null })) }));
r.post('/institutions', requireAuth, requireRole('merchant', 'admin'), writeLimit, (req, res) => {
  const b = validate(z.object({ kind: z.string(), name: z.string().min(2).max(160), registryRef: z.string().max(80).optional().nullable(), purposeCodes: z.array(z.string()).min(1).max(12), country: z.string().length(2).optional().nullable() }), req.body);
...
r.post('/institutions/me/qr', requireAuth, requireRole('merchant', 'admin'), writeLimit, (req, res) => {
  const b = validate(z.object({ purposeCode: z.string(), currency: z.string().length(3), reference: z.string().max(40).optional().nullable(), amount_minor: z.number().int().positive().optional().nullable() }), req.body);
...
r.post('/diaspora/quotes', requireAuth, writeLimit, (req, res) => {
  const b = validate(z.object({ beneficiary: z.string().min(2), sourceCurrency: z.string().length(3), destCurrency: z.string().length(3).optional().nullable(), sourceMinor: z.number().int().positive().optional().nullable(), destMinor: z.number().int().positive().optional().nullable(), purposeCode: z.string(), reference: z.string().max(80).optional().nullable() }), req.body);
...
r.post('/diaspora/quotes/:id/pay', requireAuth, writeLimit, wrap(async (req, res) => {
