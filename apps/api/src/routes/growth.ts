...
import { fxOverview, createAlert, cancelAlert, createAutoRule, setAutoRuleStatus, deleteAutoRule, quoteForward, lockForward, listForwards, getForward, settleForward, cancelForward } from '../services/fxTools';
...
fxToolsRouter.post('/alerts', (req, res) => {
  const b = validate(z.object({ baseCurrency: z.string().length(3), quoteCurrency: z.string().length(3), direction: z.enum(['above', 'below']), targetRate: z.number().positive(), note: z.string().max(120).optional().nullable() }), req.body);
...
fxToolsRouter.post('/rules', (req, res) => {
  const b = validate(z.object({ fromCurrency: z.string().length(3), toCurrency: z.string().length(3), kind: z.enum(['on_receipt', 'sweep']), shareBps: z.number().int().min(1).max(10_000).optional().nullable(), keep: z.string().optional().nullable(), minRate: z.number().positive().optional().nullable(), pin: z.string().optional() }), req.body);
  ...
  res.status(201).json({ rule: createAutoRule(req.user!, { fromCurrency: b.fromCurrency, toCurrency: b.toCurrency, kind: b.kind, shareBps: b.shareBps ?? null, keepMinor, minRate: b.minRate ?? null }) });
});
...
fxToolsRouter.post('/forwards', wrap(async (req, res) => {
  ...
  res.status(201).json({ forward: lockForward(req.user!, { fromCurrency: from.code, toCurrency: b.toCurrency, amountMinor: toMinor(b.amount, from.decimals), settleOn: b.settleOn }, { type: 'user', id: req.user!.id }) });
}));
...
fxToolsRouter.post('/forwards/:id/settle', (req, res) => { getForward(req.user!.id, String(req.params.id)); res.json({ forward: settleForward(String(req.params.id), { type: 'user', id: req.user!.id }) }); });
...
creditRouter.post('/consents', (req, res) => {
  const b = validate(z.object({ lenderName: z.string().min(2).max(120), purpose: z.string().max(200).optional().nullable(), days: z.number().int().min(1).max(365).optional().nullable(), pin: z.string().optional() }), req.body);
