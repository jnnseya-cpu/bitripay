...
recipientsRouter.post(
  '/',
  wrap(async (req, res) => {
    const body = validate(recipientSchema.extend({ payoutMethod: z.enum(['wallet', 'bank', 'cash_pickup']).default('wallet'), currency: z.string().length(3).optional().nullable() }), req.body);
    res.status(201).json({ recipient: saveRecipient(req.user!.id, body) });
  }),
);
