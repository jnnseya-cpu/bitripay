...
const OPS: Op[] = [
  ...
  { method: 'post', path: '/plans', tag: 'Subscriptions', summary: 'Create a plan (interval, trial, tax, metered usage)', scope: 'subscriptions:write', body: { name: { type: 'string' }, currency: { type: 'string' }, amount_minor: { type: 'integer' }, interval: { enum: ['day', 'week', 'month', 'year'] }, interval_count: { type: 'integer' }, trial_days: { type: 'integer' }, tax_bps: { type: 'integer' }, tax_label: { type: 'string' }, usage_unit: { type: 'string' }, usage_price_minor: { type: 'integer' }, code: { type: 'string' } } },
  ...
  { method: 'get', path: '/subscriptions', tag: 'Subscriptions', summary: 'List subscriptions with a billing overview (recurring revenue, collections, dunning)', scope: 'subscriptions:read', query: ['status', 'limit'] },
  ...
  { method: 'post', path: '/subscriptions/{id}/usage', tag: 'Subscriptions', summary: 'Record metered usage for the current period', scope: 'subscriptions:write', body: { quantity: { type: 'integer' }, note: { type: 'string' } } },
  { method: 'post', path: '/subscriptions/{id}/cancel', tag: 'Subscriptions', summary: 'Cancel at period end (or immediately)', scope: 'subscriptions:write', body: { immediately: { type: 'boolean' } } },
  { method: 'get', path: '/invoices', tag: 'Subscriptions', summary: 'List invoices', scope: 'subscriptions:read', query: ['limit'] },
  { method: 'get', path: '/credit_readiness/{code}', tag: 'Credit readiness', summary: 'Read a consented credit-readiness signal (score, band, factors); never the transactions, never a lending decision', scope: 'credit:read' },
