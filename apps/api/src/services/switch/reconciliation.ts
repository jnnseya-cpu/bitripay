...
export const CASE_CLASSES = ['LOCAL_ONLY', 'EXTERNAL_ONLY', 'STATUS_CONFLICT', 'AMOUNT_MISMATCH', 'CURRENCY_MISMATCH', 'DUPLICATE_EXTERNAL', 'FEES_MISMATCH', 'SETTLEMENT_NOT_OBSERVED', 'LATE_RECORD', 'MISSING_REPORT', 'INTEGRITY'] as const;
...
const toCase = (r: any): ReconciliationCase => ({ id: r.id, connectionId: r.connection_id, class: r.class, paymentId: r.payment_id, lineId: r.line_id, runId: r.run_id, cycleRef: r.cycle_ref, exposure: { valueMinor: r.exposure_minor, currency: r.currency }, ageHours: Math.floor((Date.now() - Date.parse(r.created_at)) / 3600_000), references: parseJson(r.references_json, {}), sources: parseJson(r.sources, []), status: r.status, priority: r.priority, ownerId: r.owner_id, nextAction: r.next_action, dueAt: r.due_at, documents: parseJson(r.documents, []), resolution: r.resolution, resolvedBy: r.resolved_by, closureApprovedBy: r.closure_approved_by, merchantUserId: r.merchant_user_id, createdAt: r.created_at, updatedAt: r.updated_at });

/** Open a case unless an identical open one already exists (class + payment/line). Returns the case id. */
export function openCase(input: { connectionId: string; class: CaseClass; paymentId?: string | null; lineId?: string | null; runId?: string | null; cycleRef?: string | null; exposureMinor?: number; currency?: string | null; references?: Record<string, unknown>; sources?: string[]; merchantUserId?: string | null; dueHours?: number }): string {
  const db = getDb();
  const dup = db.prepare("SELECT id FROM reconciliation_cases WHERE connection_id = ? AND class = ? AND payment_id IS ? AND line_id IS ? AND status != 'CLOSED'").get(input.connectionId, input.class, input.paymentId ?? null, input.lineId ?? null) as any;
  ...
  db.prepare('INSERT INTO reconciliation_cases (id, connection_id, class, payment_id, line_id, run_id, cycle_ref, exposure_minor, currency, references_json, sources, status, priority, next_action, due_at, merchant_user_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, input.connectionId, input.class, input.paymentId ?? null, input.lineId ?? null, input.runId ?? null, input.cycleRef ?? null, input.exposureMinor ?? 0, input.currency ?? null, JSON.stringify(input.references ?? {}), JSON.stringify(input.sources ?? []), 'OPEN', priority, NEXT_ACTION[input.class], due, input.merchantUserId ?? null, now(), now());
  recordEvent('reconciliation', id, 'case.opened', { type: 'system' }, { class: input.class, paymentId: input.paymentId ?? null, exposure: input.exposureMinor ?? 0, currency: input.currency ?? null });
  ...
  if (priority === 'CRITICAL') openIncident(input.class === 'INTEGRITY' ? 'P1' : 'P1', `${input.class} on ${input.paymentId ?? input.lineId ?? input.cycleRef ?? 'cycle'}`, NEXT_ACTION[input.class], 'reconciliation_case', id);
  if (input.merchantUserId) emitEvent(input.merchantUserId, 'reconciliation.exception', { case: { id, class: input.class, paymentId: input.paymentId ?? null, exposure: { valueMinor: input.exposureMinor ?? 0, currency: input.currency ?? null }, openedAt: now() } }, { resource: { type: 'reconciliation_case', id } });
  return id;
}
...
export function listCases(filter: { connectionId?: string | null; status?: string | null; class?: string | null; merchantUserId?: string | null; paymentId?: string | null; limit?: number; cursor?: string | null } = {}): { data: ReconciliationCase[]; nextCursor: string | null } {
  const where: string[] = [];
  ...
  const rows = getDb().prepare(`SELECT * FROM reconciliation_cases ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY created_at DESC LIMIT ?`).all(...params, limit + 1) as any[];
...
export function proposeResolution(id: string, resolution: string, documents: string[], adminId: string): ReconciliationCase {
  ...
  getDb().prepare("UPDATE reconciliation_cases SET resolution = ?, documents = ?, resolved_by = ?, status = 'RESOLUTION_PROPOSED', updated_at = ? WHERE id = ?").run(resolution, JSON.stringify([...c.documents, ...documents]), adminId, now(), id);
...
const toImport = (r: any): ImportView => ({ id: r.id, connectionId: r.connection_id, source: r.source, cycleRef: r.cycle_ref, periodFrom: r.period_from, periodTo: r.period_to, currency: r.currency, checksum: r.checksum, lineCount: r.line_count, controlTotal: r.control_total, replacesImportId: r.replaces_import_id, importedBy: r.imported_by, createdAt: r.created_at });
...
export async function importReport(connectionId: string, input: ImportInput, importerId: string | null): Promise<ImportView> {
  ...
  const report: VerifiedReport = { source: input.source, cycleRef: input.cycleRef, currency: input.currency.toUpperCase(), periodFrom: input.periodFrom, periodTo: input.periodTo, raw: rawBase64, signatureValid: input.signatureValid ?? true };
  ...
  const checksum = sha256(`${input.source}|${input.cycleRef}|${report.currency}|${JSON.stringify(result.lines.map((l) => [l.externalReference, l.amountMinor, l.currency, l.status, l.settlementRef ?? '']))}`);
  const db = getDb();
  ...
  for (const l of result.lines) if (l.currency.toUpperCase() !== report.currency) throw badRequest(`Line ${l.externalReference} is in ${l.currency}; a report carries one currency`, 'mixed_currency_report');
  ...
  db.transaction(() => {
    db.prepare('INSERT INTO reconciliation_imports (id, connection_id, source, period_from, period_to, cycle_ref, currency, checksum, line_count, control_total, replaces_import_id, imported_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, connectionId, input.source, input.periodFrom, input.periodTo, input.cycleRef, report.currency, checksum, result.lineCount, total, input.replacesImportId ?? null, importerId, now());
    const ins = db.prepare('INSERT INTO reconciliation_lines (id, import_id, external_reference, correlation_id, debtor_id, creditor_id, amount_minor, currency, status, fee_minor, settlement_ref, business_date, occurred_at, raw) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    for (const l of result.lines) ins.run(`rl_${shortCode(14).toLowerCase()}`, id, l.externalReference, l.correlationId, l.debtorId, l.creditorId, Number(l.amountMinor), l.currency.toUpperCase(), l.status, l.feeMinor != null ? Number(l.feeMinor) : null, l.settlementRef, l.businessDate, l.occurredAt, JSON.stringify(l.raw));
  })();
  recordEvent('reconciliation', id, 'report.imported', { type: importerId ? 'admin' : 'system', id: importerId }, { source: input.source, cycleRef: input.cycleRef, lines: result.lineCount, total, currency: report.currency, proofRef });
  // a file arriving after the cycle was closed reopens it as a new version
  const closedRun = db.prepare('SELECT id FROM reconciliation_runs WHERE connection_id = ? AND cycle_ref = ? AND complete = 1 ORDER BY created_at DESC LIMIT 1').get(connectionId, input.cycleRef) as any;
  if (closedRun) openCase({ connectionId, class: 'LATE_RECORD', cycleRef: input.cycleRef, references: { importId: id, closedRunId: closedRun.id }, sources: [input.source], currency: report.currency });
...
export function listImports(connectionId: string, cycleRef?: string | null): ImportView[] {
  return (getDb().prepare(`SELECT * FROM reconciliation_imports WHERE connection_id = ? ${cycleRef ? 'AND cycle_ref = ?' : ''} ORDER BY created_at DESC LIMIT 200`).all(...(cycleRef ? [connectionId, cycleRef] : [connectionId])) as any[]).map(toImport);
}
...
const toRun = (r: any): RunView => ({ id: r.id, connectionId: r.connection_id, cycleRef: r.cycle_ref, coverage: parseJson(r.coverage, { expected: [], present: [], missing: [], complete: false }), totals: parseJson(r.totals, {}), matched: r.matched, casesOpened: r.cases_opened, complete: !!r.complete, runBy: r.run_by, createdAt: r.created_at });
...
export function runReconciliation(connectionId: string, cycleRef: string, runBy: string | null): RunView {
  ...
  const imports = (db.prepare('SELECT * FROM reconciliation_imports WHERE connection_id = ? AND cycle_ref = ? ORDER BY created_at').all(connectionId, cycleRef) as any[]).filter((i) => !(db.prepare('SELECT 1 FROM reconciliation_imports WHERE replaces_import_id = ?').get(i.id) as any));
  ...
  for (const imp of imports) {
    ...
    for (const l of lines) {
      ...
      const p = (db.prepare('SELECT * FROM switch_payments WHERE connection_id = ? AND (external_reference = ? OR switch_correlation_id = ? OR external_message_id = ?)').get(connectionId, l.external_reference, l.correlation_id ?? '', l.external_reference) as any) ?? null;
      if (!p) {
        openCase({ connectionId, class: 'EXTERNAL_ONLY', lineId: l.id, runId, cycleRef, exposureMinor: l.amount_minor, currency: l.currency, references: { externalReference: l.external_reference, source: imp.source }, sources: [imp.source] });
        cases += 1;
        continue;
      }
      ...
      if (l.currency !== p.currency) {
        openCase({ connectionId, class: 'CURRENCY_MISMATCH', paymentId: p.id, lineId: l.id, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { externalReference: l.external_reference, local: p.currency, external: l.currency, source: imp.source }, sources: ['BITRIPAY', imp.source], merchantUserId: p.merchant_user_id });
        cases += 1;
        clean = false;
      } else if (l.amount_minor !== p.amount_minor) {
        openCase({ connectionId, class: 'AMOUNT_MISMATCH', paymentId: p.id, lineId: l.id, runId, cycleRef, exposureMinor: Math.abs(l.amount_minor - p.amount_minor), currency: p.currency, references: { externalReference: l.external_reference, local: p.amount_minor, external: l.amount_minor, source: imp.source }, sources: ['BITRIPAY', imp.source], merchantUserId: p.merchant_user_id });
        cases += 1;
        clean = false;
      }
      ...
      if ((externalCompleted && ['REJECTED', 'UNKNOWN'].includes(p.status)) || (externalRejected && p.status === 'COMPLETED')) {
        openCase({ connectionId, class: 'STATUS_CONFLICT', paymentId: p.id, lineId: l.id, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { externalReference: l.external_reference, local: p.status, external: l.status, source: imp.source }, sources: ['BITRIPAY', imp.source], merchantUserId: p.merchant_user_id });
      ...
      if (l.fee_minor != null) {
        ...
        if (l.fee_minor !== expectedFee) {
          openCase({ connectionId, class: 'FEES_MISMATCH', paymentId: p.id, lineId: l.id, runId, cycleRef, exposureMinor: Math.abs(l.fee_minor - expectedFee), currency: p.currency, references: { expectedFee, observedFee: l.fee_minor, tariffBps: settings.feeBps, source: imp.source }, sources: ['BITRIPAY', imp.source] });
          cases += 1;
        }
        db.prepare("INSERT INTO switch_journal (id, payment_id, fact, amount_minor, currency, source, reference, occurred_at, created_at) VALUES (?, ?, 'FEE_OBSERVED', ?, ?, ?, ?, ?, ?)").run(`sj_${shortCode(14).toLowerCase()}`, p.id, l.fee_minor, p.currency, imp.source, l.external_reference, l.occurred_at ?? now(), now());
      }
      if (l.settlement_ref && imp.source === 'SWITCH') {
        ...
        db.prepare("INSERT INTO switch_journal (id, payment_id, fact, amount_minor, currency, source, reference, occurred_at, created_at) VALUES (?, ?, 'SETTLEMENT_REFERENCE', ?, ?, ?, ?, ?, ?)").run(`sj_${shortCode(14).toLowerCase()}`, p.id, p.amount_minor, p.currency, imp.source, l.settlement_ref, l.occurred_at ?? now(), now());
      }
      if (clean) {
        matched += 1;
  ...
  for (const [pid, n] of seenPayment) {
    if (n > 1) {
      ...
      openCase({ connectionId, class: 'DUPLICATE_EXTERNAL', paymentId: pid, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { occurrences: n }, sources: ['BITRIPAY', ...present], merchantUserId: p.merchant_user_id });
      cases += 1;
    }
  }
  // local payments in the period that no report mentions
  const locals = db.prepare("SELECT * FROM switch_payments WHERE connection_id = ? AND status IN ('COMPLETED', 'UNKNOWN', 'AUTHORIZED', 'PENDING') AND created_at >= ? AND created_at <= ?").all(connectionId, periodFrom, periodTo) as any[];
  for (const p of locals) {
    bump(p.currency, 'local', p.amount_minor);
    if (seenPayment.has(p.id) || !present.length) continue;
    if (p.status === 'COMPLETED') {
      openCase({ connectionId, class: present.includes('SWITCH') ? 'LOCAL_ONLY' : 'SETTLEMENT_NOT_OBSERVED', paymentId: p.id, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { externalReference: p.external_reference, status: p.status }, sources: ['BITRIPAY'], merchantUserId: p.merchant_user_id });
    } else openCase({ connectionId, class: 'LOCAL_ONLY', paymentId: p.id, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { externalReference: p.external_reference, status: p.status }, sources: ['BITRIPAY'], merchantUserId: p.merchant_user_id });
    cases += 1;
  }
  for (const p of db.prepare("SELECT * FROM switch_payments WHERE connection_id = ? AND status = 'COMPLETED' AND settlement_status = 'NOT_OBSERVED' AND created_at >= ? AND created_at <= ?").all(connectionId, periodFrom, periodTo) as any[]) {
    if (!seenPayment.has(p.id)) continue;
    openCase({ connectionId, class: 'SETTLEMENT_NOT_OBSERVED', paymentId: p.id, runId, cycleRef, exposureMinor: p.amount_minor, currency: p.currency, references: { externalReference: p.external_reference }, sources: ['BITRIPAY', ...present] });
    cases += 1;
  }
  ...
  if (!complete) for (const a of db.prepare("SELECT id FROM users WHERE role = 'admin' AND is_system = 0 AND status = 'active'").all() as { id: string }[]) notify(a.id, 'Reconciliation coverage incomplete', `Cycle ${cycleRef} on ${conn.name}: missing ${missing.join(', ')}. The cycle is not reconciled until every expected report is in.`, { kind: 'reconciliation', runId });
...
export function reconciliationOverview(connectionId: string) {
  const db = getDb();
  const cases = db.prepare("SELECT class, currency, status, COUNT(*) n, COALESCE(SUM(exposure_minor), 0) exposure, MIN(created_at) oldest FROM reconciliation_cases WHERE connection_id = ? AND status != 'CLOSED' GROUP BY class, currency, status").all(connectionId) as any[];
  const cycles = db.prepare('SELECT cycle_ref, source, COUNT(*) files, MAX(created_at) last FROM reconciliation_imports WHERE connection_id = ? GROUP BY cycle_ref, source ORDER BY cycle_ref DESC LIMIT 60').all(connectionId) as any[];
  const runs = listRuns(connectionId, 30);
  return { cases: cases.map((c) => ({ class: c.class, currency: c.currency, status: c.status, count: c.n, exposureMinor: c.exposure, oldestAgeHours: Math.floor((Date.now() - Date.parse(c.oldest)) / 3600_000) })), cycles, runs, coverageComplete: runs[0]?.complete ?? false };
}
