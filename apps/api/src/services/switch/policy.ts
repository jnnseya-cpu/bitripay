...
import { getConnection, connectionForCountry, emissionGate, type SwitchConnection } from './connections';
...
const toPolicy = (r: any): RoutePolicy => ({ version: r.version, country: r.country, rules: parseJson(r.rules, DEFAULT_RULES), status: r.status, authorId: r.author_id, approvedBy: r.approved_by, approvedAt: r.approved_at, activatedAt: r.activated_at, retiredAt: r.retired_at, notes: r.notes, createdAt: r.created_at });

const DEFAULT_RULES: PolicyRules = {
  onUs: 'REVIEW_REQUIRED',
  products: { MERCHANT_PAYMENT: { enabled: true, channels: ['api', 'qr', 'ussd'] }, REFUND: { enabled: true, channels: ['api'] }, REVERSAL: { enabled: true, channels: ['ops'] }, INQUIRY: { enabled: true, channels: ['api', 'ops'] }, P2P: { enabled: false, channels: [] } },
  currencies: ['CDF', 'USD'],
  failover: 'SAME_SYSTEM_ONLY',
  bilateralFallback: false,
};
...
export function activePolicy(country: string): RoutePolicy {
  ...
  return { version: 0, country: country.toUpperCase(), rules: DEFAULT_RULES, status: 'ACTIVE', authorId: null, approvedBy: null, approvedAt: null, activatedAt: null, retiredAt: null, notes: 'built-in default policy (version 0)', createdAt: now() };
}
export function listPolicies(country?: string | null): RoutePolicy[] {
  return (getDb().prepare(`SELECT * FROM route_policies ${country ? 'WHERE country = ?' : ''} ORDER BY version DESC`).all(...(country ? [country.toUpperCase()] : [])) as any[]).map(toPolicy);
}
export function createPolicyDraft(country: string, rules: Partial<PolicyRules>, authorId: string, notes?: string | null): RoutePolicy {
  ...
  const r = getDb().prepare('INSERT INTO route_policies (country, rules, status, author_id, notes, created_at) VALUES (?, ?, ?, ?, ?, ?) RETURNING *').get(country.toUpperCase(), JSON.stringify(merged), 'DRAFT', authorId, notes ?? null, now());
  return toPolicy(r);
}
...
const toException = (r: any): RoutingException => ({ id: r.id, country: r.country, scope: r.scope, products: parseJson(r.products, []), participants: parseJson(r.participants, []), currency: r.currency, documentRef: r.document_ref, documentSha256: r.document_sha256, validFrom: r.valid_from, validTo: r.valid_to, status: r.status, authorId: r.author_id, firstApproverId: r.first_approver_id, secondApproverId: r.second_approver_id, signature: r.signature });

export function createException(input: { country: string; scope: string; products: string[]; participants: string[]; currency?: string | null; documentRef: string; documentSha256?: string | null; validFrom: string; validTo: string }, authorId: string): RoutingException {
  ...
  getDb().prepare('INSERT INTO routing_exceptions (id, country, scope, products, participants, currency, document_ref, document_sha256, valid_from, valid_to, status, author_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, input.country.toUpperCase(), input.scope, JSON.stringify(input.products), JSON.stringify(input.participants), input.currency?.toUpperCase() ?? null, input.documentRef, input.documentSha256 ?? null, input.validFrom, input.validTo, 'DRAFT', authorId, now(), now());
...
export function approveException(id: string, approverId: string): RoutingException {
  ...
  if (r.status === 'DRAFT') getDb().prepare("UPDATE routing_exceptions SET status = 'FIRST_APPROVAL', first_approver_id = ?, updated_at = ? WHERE id = ?").run(approverId, now(), id);
    ...
    const signature = sha256(`${r.id}|${r.country}|${r.scope}|${r.products}|${r.participants}|${r.currency ?? ''}|${r.document_ref}|${r.valid_from}|${r.valid_to}|${r.first_approver_id}|${approverId}`);
...
export function listExceptions(country?: string | null): RoutingException[] {
  return (getDb().prepare(`SELECT * FROM routing_exceptions ${country ? 'WHERE country = ?' : ''} ORDER BY created_at DESC`).all(...(country ? [country.toUpperCase()] : [])) as any[]).map(toException);
}
function applicableException(country: string, product: string, participants: string[], currency: string): RoutingException | null {
  ...
  for (const e of listExceptions(country)) {
    ...
    const expected = sha256(`${e.id}|${e.country}|${e.scope}|${JSON.stringify(e.products)}|${JSON.stringify(e.participants)}|${e.currency ?? ''}|${e.documentRef}|${e.validFrom}|${e.validTo}|${e.firstApproverId}|${e.secondApproverId}`);
...
export function decideRoute(input: DecideInput): RouteDecision {
  ...
  const base = { accessMode: conn?.accessMode ?? null, connectionId: conn?.id ?? null, schemeId: conn?.schemeId ?? null, participantId: conn?.participantId ?? null, sponsorId: conn?.sponsorId ?? null, profileVersion: conn?.profileVersion ?? null, policyVersion: policy.version, configVersion, institutions: { debtor: input.debtorId, creditor: input.creditorId }, currency: input.currency.toUpperCase(), product: input.product, channel: input.channel, exceptionId: null as string | null, decidedAt: now() };
  const deny = (cls: RouteClass, rule: string, code: string, message: string, reasons: string[] = []): RouteDecision => ({ ...base, allowed: false, class: cls, rail: 'NONE', rule, reasons: [message, ...reasons], rejection: { code, message } });
  ...
  if (cls === 'UNSUPPORTED') return deny(cls, 'RTE-005', 'UNSUPPORTED_PARTICIPANT_PAIR', !debtor || !creditor ? 'A participant is not in the trusted registry' : 'Unsupported participant kinds for this product');
  ...
  if (cls === 'CLOSED_LOOP') return { ...base, allowed: true, class: cls, rail: 'INTERNAL', rule: 'CLOSED_LOOP', reasons: ['both parties on the aggregator: internal ledger, no interbank movement'], rejection: null };
  ...
  if (!policy.rules.currencies.includes(input.currency.toUpperCase())) return deny(cls, `POLICY-${policy.version}`, 'CURRENCY_NOT_ENABLED', `${input.currency.toUpperCase()} is not enabled by route policy v${policy.version}`);
  ...
  if (cls === 'ON_US_REVIEW_REQUIRED') {
    ...
    if (policy.rules.onUs === 'REVIEW_REQUIRED') return deny(cls, `POLICY-${policy.version}/ON_US`, 'ON_US_REVIEW_REQUIRED', 'Same-institution payments are classified ON_US_REVIEW_REQUIRED until a documented rule exists (no automatic exemption)');
  }
  // RTE-001: domestic interoperable movements must use a route containing the national switch
  if (!caps.nationalSwitch.required) return { ...base, allowed: true, class: cls, rail: 'NATIONAL_SWITCH', rule: 'RTE-001/optional', reasons: ['national switch not mandatory in this country'], rejection: null };
  ...
  const availability = serviceAvailability({ connectionId: conn.id, connectionEnabled: conn.enabled, environmentCertified: envCertified, country, debtorId: input.debtorId, creditorId: input.creditorId, currency: input.currency, product: input.product, channel: input.channel });
  ...
  return { ...base, allowed: true, class: cls, rail: 'NATIONAL_SWITCH', rule: exception ? `RTE-004 exception ${exception.id}` : 'RTE-001', reasons: [`${conn.accessMode === 'SPONSORED' ? 'sponsored' : 'direct'} participation via ${conn.schemeId}${conn.simulation ? ' (SIMULATION)' : ''}`, ...(gate.inquiryOnly ? ['link in inquiry-only mode: emission will wait'] : [])], rejection: null, exceptionId: exception?.id ?? null };
}
...
export function connectionOf(decision: RouteDecision): SwitchConnection | null {
  return decision.connectionId ? getConnection(decision.connectionId) : null;
}
