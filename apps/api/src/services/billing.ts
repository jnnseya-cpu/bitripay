...
export interface Plan { id: string; code: string; name: string; description: string | null; currency: string; amountMinor: number; interval: PlanInterval; intervalCount: number; trialDays: number; taxBps: number; taxLabel: string | null; usageUnit: string | null; usagePriceMinor: number; status: 'ACTIVE' | 'ARCHIVED'; merchantId: string; createdAt: string }
const planView = (r: any): Plan => ({ id: r.id, code: r.code, name: r.name, description: r.description, currency: r.currency, amountMinor: r.amount_minor, interval: r.interval, intervalCount: r.interval_count, trialDays: r.trial_days, taxBps: r.tax_bps, taxLabel: r.tax_label, usageUnit: r.usage_unit, usagePriceMinor: r.usage_price_minor, status: r.status, merchantId: r.merchant_user_id, createdAt: r.created_at });
export interface Subscription { id: string; planId: string; plan: Plan; merchantId: string; customerId: string; status: 'TRIALING' | 'ACTIVE' | 'PAST_DUE' | 'CANCELLED' | 'ENDED'; currentPeriodStart: string; currentPeriodEnd: string; nextChargeAt: string; usageQty: number; dunningAttempts: number; lastError: string | null; cancelAtPeriodEnd: boolean; reference: string | null; createdAt: string }
export interface Invoice { id: string; number: string; subscriptionId: string; merchantId: string; customerId: string; periodStart: string; periodEnd: string; currency: string; subtotalMinor: number; usageQty: number; usageMinor: number; taxMinor: number; totalMinor: number; status: 'OPEN' | 'PAID' | 'FAILED' | 'VOID'; attempts: number; nextAttemptAt: string | null; lastError: string | null; transactionId: string | null; paidAt: string | null; createdAt: string }
const invoiceView = (r: any): Invoice => ({ id: r.id, number: r.number, subscriptionId: r.subscription_id, merchantId: r.merchant_user_id, customerId: r.customer_user_id, periodStart: r.period_start, periodEnd: r.period_end, currency: r.currency, subtotalMinor: r.subtotal_minor, usageQty: r.usage_qty, usageMinor: r.usage_minor, taxMinor: r.tax_minor, totalMinor: r.total_minor, status: r.status, attempts: r.attempts, nextAttemptAt: r.next_attempt_at, lastError: r.last_error, transactionId: r.transaction_id, paidAt: r.paid_at, createdAt: r.created_at });
...
// ---------------------------------------------------------------- plans
export function createPlan(merchant: UserRow, input: { name: string; description?: string | null; currency: string; amountMinor: number; interval: PlanInterval; intervalCount?: number | null; trialDays?: number | null; taxBps?: number | null; taxLabel?: string | null; usageUnit?: string | null; usagePriceMinor?: number | null; code?: string | null }): Plan {
  ...
  const code = (input.code ?? `${input.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 24)}-${shortCode(4).toLowerCase()}`).trim();
  ...
  getDb().prepare('INSERT INTO merchant_plans (id, merchant_user_id, code, name, description, currency, amount_minor, interval, interval_count, trial_days, tax_bps, tax_label, usage_unit, usage_price_minor, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, merchant.id, code, input.name.trim(), input.description ?? null, cur.code, input.amountMinor, input.interval, input.intervalCount ?? 1, input.trialDays ?? 0, input.taxBps ?? 0, input.taxLabel ?? null, input.usageUnit ?? null, input.usagePriceMinor ?? 0, 'ACTIVE', now(), now());
...
export function listPlans(merchantId: string, includeArchived = false): Plan[] { return (getDb().prepare(`SELECT * FROM merchant_plans WHERE merchant_user_id = ? ${includeArchived ? '' : "AND status = 'ACTIVE'"} ORDER BY created_at DESC`).all(merchantId) as any[]).map(planView); }
...
export async function subscribe(customer: UserRow, planIdOrCode: string, opts: { reference?: string | null; mandateConfirmed: boolean }): Promise<{ subscription: Subscription; invoice: Invoice | null }> {
  ...
  const existing = getDb().prepare("SELECT id FROM merchant_subscriptions WHERE plan_id = ? AND customer_user_id = ? AND status IN ('TRIALING', 'ACTIVE', 'PAST_DUE')").get(plan.id, customer.id) as any;
  ...
  getDb().prepare('INSERT INTO merchant_subscriptions (id, plan_id, merchant_user_id, customer_user_id, status, current_period_start, current_period_end, next_charge_at, mandate_confirmed_at, reference, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, plan.id, plan.merchantId, customer.id, trialEnd ? 'TRIALING' : 'ACTIVE', start, periodEnd, trialEnd ?? start, start, opts.reference ?? null, start, start);
...
export function listSubscriptions(filter: { merchantId?: string | null; customerId?: string | null; status?: string | null; limit?: number }): Subscription[] {
  ...
  if (filter.merchantId) { where.push('merchant_user_id = ?'); params.push(filter.merchantId); }
  if (filter.customerId) { where.push('customer_user_id = ?'); params.push(filter.customerId); }
  if (filter.status) { where.push('status = ?'); params.push(filter.status); }
  return (getDb().prepare(`SELECT * FROM merchant_subscriptions ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY created_at DESC LIMIT ?`).all(...params, Math.min(500, filter.limit ?? 100)) as any[]).map(subView);
}
export function listInvoices(filter: { subscriptionId?: string | null; merchantId?: string | null; customerId?: string | null; limit?: number }): Invoice[] {
  const where: string[] = [];
  const params: unknown[] = [];
  if (filter.subscriptionId) { where.push('subscription_id = ?'); params.push(filter.subscriptionId); }
  if (filter.merchantId) { where.push('merchant_user_id = ?'); params.push(filter.merchantId); }
  if (filter.customerId) { where.push('customer_user_id = ?'); params.push(filter.customerId); }
  return (getDb().prepare(`SELECT * FROM merchant_invoices ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY created_at DESC LIMIT ?`).all(...params, Math.min(500, filter.limit ?? 100)) as any[]).map(invoiceView);
}
...
export function cancelSubscription(actor: UserRow, id: string, opts: { immediately?: boolean } = {}): Subscription {
  ...
  notify(s.customerId, 'Subscription cancelled', after.status === 'CANCELLED' ? `${s.plan.name} ended today.` : `${s.plan.name} stays active until ${s.currentPeriodEnd.slice(0, 10)} and then stops.`, { kind: 'wallet', subscriptionId: id });
  return after;
}
...
function raiseInvoice(s: any): any {
  ...
  getDb().prepare('INSERT INTO merchant_invoices (id, subscription_id, merchant_user_id, customer_user_id, number, period_start, period_end, currency, subtotal_minor, usage_qty, usage_minor, tax_minor, total_minor, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, s.id, s.merchant_user_id, s.customer_user_id, number, s.current_period_start, s.current_period_end, plan.currency, subtotal, s.usage_qty, usageMinor, tax, total, 'OPEN', now(), now());
...
function collectInner(subscriptionId: string, actor: Actor): Invoice {
  ...
  try {
    ...
    if (inv.total_minor > 0) {
      const tx = sendMoney(customer, { to: merchant.tag, amount: inv.total_minor, currency: plan.currency, type: 'merchant_payment', note: `${plan.name} · ${inv.number}`, idempotencyKey: `inv:${inv.id}`, stepUpVerified: true });
      txId = tx.id;
    }
    const nextStart = s.current_period_end;
    const nextEnd = addInterval(nextStart, plan.interval, plan.intervalCount);
    db.transaction(() => {
      db.prepare("UPDATE merchant_invoices SET status = 'PAID', transaction_id = ?, paid_at = ?, attempts = attempts + 1, last_error = NULL, next_attempt_at = NULL, updated_at = ? WHERE id = ?").run(txId, now(), now(), inv.id);
      // the paid period is the current one; the next charge falls at its end
      db.prepare("UPDATE merchant_subscriptions SET status = CASE WHEN cancel_at_period_end = 1 THEN 'CANCELLED' ELSE 'ACTIVE' END, ended_at = CASE WHEN cancel_at_period_end = 1 THEN ? ELSE NULL END, next_charge_at = ?, usage_qty = 0, dunning_attempts = 0, last_error = NULL, updated_at = ? WHERE id = ?").run(s.current_period_end, s.current_period_end, now(), s.id);
      void nextStart; void nextEnd;
    })();
    ...
    notify(s.customer_user_id, `${plan.name} paid`, `${formatMoney(inv.total_minor, cur)} for ${inv.period_start.slice(0, 10)} → ${inv.period_end.slice(0, 10)}${inv.tax_minor ? ` (includes ${formatMoney(inv.tax_minor, cur)} ${plan.taxLabel ?? 'tax'})` : ''}. Invoice ${inv.number}.`, { kind: 'wallet', invoiceId: inv.id, transactionId: txId });
    publish('subscription.charged', { subscriptionId: s.id, invoiceId: inv.id, merchantId: s.merchant_user_id, customerId: s.customer_user_id, amountMinor: inv.total_minor, currency: plan.currency }, { aggregateId: s.id, tenantId: s.merchant_user_id });
    return paid;
  } catch (err) {
    ...
    if (nextAttempt) {
      db.prepare("UPDATE merchant_subscriptions SET status = 'PAST_DUE', dunning_attempts = ?, last_error = ?, next_charge_at = ?, updated_at = ? WHERE id = ?").run(attempts, message, nextAttempt, now(), s.id);
      notify(s.customer_user_id, `${plan.name}: payment failed`, `${formatMoney(inv.total_minor, cur)} could not be collected (${message}). We will try again on ${nextAttempt.slice(0, 10)}; add money to keep the service.`, { kind: 'wallet', invoiceId: inv.id });
    } else {
      db.prepare("UPDATE merchant_subscriptions SET status = 'CANCELLED', dunning_attempts = ?, last_error = ?, cancelled_at = ?, ended_at = ?, updated_at = ? WHERE id = ?").run(attempts, message, now(), now(), now(), s.id);
      db.prepare("UPDATE merchant_invoices SET status = 'VOID', updated_at = ? WHERE id = ?").run(now(), inv.id);
      notify(s.customer_user_id, `${plan.name} cancelled`, `After ${attempts} failed collection attempts the subscription was cancelled. Subscribe again once your balance allows.`, { kind: 'wallet', subscriptionId: s.id });
...
export function billingOverview(merchantId: string) {
  ...
  const mrr = db.prepare("SELECT p.currency, SUM(CASE p.interval WHEN 'day' THEN p.amount_minor * 30 / p.interval_count WHEN 'week' THEN p.amount_minor * 4 / p.interval_count WHEN 'month' THEN p.amount_minor / p.interval_count ELSE p.amount_minor / (12 * p.interval_count) END) m FROM merchant_subscriptions s JOIN merchant_plans p ON p.id = s.plan_id WHERE s.merchant_user_id = ? AND s.status IN ('ACTIVE', 'PAST_DUE') GROUP BY p.currency").all(merchantId) as any[];
  const collected = db.prepare("SELECT currency, COALESCE(SUM(total_minor), 0) s, COUNT(*) n FROM merchant_invoices WHERE merchant_user_id = ? AND status = 'PAID' AND paid_at >= ? GROUP BY currency").all(merchantId, new Date(Date.now() - 30 * 86_400_000).toISOString()) as any[];
  return { byStatus: Object.fromEntries(subs.map((s) => [s.status, s.c])), monthlyRecurring: mrr.map((m) => ({ currency: m.currency, minor: Math.round(m.m) })), collected30d: collected.map((c) => ({ currency: c.currency, minor: c.s, invoices: c.n })), dunningDays: DUNNING_DAYS };
}
export const resolveCustomer = findUserByIdentifier;
