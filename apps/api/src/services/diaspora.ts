...
const toPolicy = (r: any): RatePolicy => ({ id: r.id, sourceCurrency: r.source_currency, destCurrency: r.dest_currency, markupBps: r.markup_bps, maxValidityHours: r.max_validity_hours, feeBps: r.fee_bps, feeFixedSourceMinor: r.fee_fixed_source_minor, minSourceMinor: r.min_source_minor, maxSourceMinor: r.max_source_minor, status: r.status, signedBy: r.signed_by, signature: r.signature, version: r.version, createdAt: r.created_at, retiredAt: r.retired_at });
export const policyCanonical = (p: { sourceCurrency: string; destCurrency: string; markupBps: number; maxValidityHours: number; feeBps: number; feeFixedSourceMinor: number; minSourceMinor: number; maxSourceMinor: number; version: number; signedBy: string }) => ['DD-RATE-POLICY', p.sourceCurrency, p.destCurrency, p.markupBps, p.maxValidityHours, p.feeBps, p.feeFixedSourceMinor, p.minSourceMinor, p.maxSourceMinor, p.version, p.signedBy].join('|');
...
export function signRatePolicy(admin: UserRow, input: { sourceCurrency: string; destCurrency: string; markupBps: number; maxValidityHours?: number; feeBps?: number; feeFixedSourceMinor?: number; minSourceMinor?: number; maxSourceMinor?: number }): RatePolicy {
  ...
  const body = { sourceCurrency: src.code, destCurrency: dst.code, markupBps: input.markupBps, maxValidityHours: hours, feeBps: input.feeBps ?? 0, feeFixedSourceMinor: input.feeFixedSourceMinor ?? 0, minSourceMinor: input.minSourceMinor ?? 0, maxSourceMinor: input.maxSourceMinor ?? 0, version, signedBy: admin.id };
  ...
  db.transaction(() => {
    ...
    db.prepare('INSERT INTO fx_rate_policies (id, source_currency, dest_currency, markup_bps, max_validity_hours, fee_bps, fee_fixed_source_minor, min_source_minor, max_source_minor, status, signed_by, signature, version, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, body.sourceCurrency, body.destCurrency, body.markupBps, body.maxValidityHours, body.feeBps, body.feeFixedSourceMinor, body.minSourceMinor, body.maxSourceMinor, 'ACTIVE', admin.id, signature, version, now());
  })();
...
export function listRatePolicies(includeRetired = false): RatePolicy[] {
  return (getDb().prepare(`SELECT * FROM fx_rate_policies ${includeRetired ? '' : "WHERE status = 'ACTIVE'"} ORDER BY source_currency, dest_currency, version DESC`).all() as any[]).map(toPolicy);
}
...
const toCard = (r: any): RateCard => ({ id: r.id, policyId: r.policy_id, sourceCurrency: r.source_currency, destCurrency: r.dest_currency, midRate: r.mid_rate, customerRate: r.customer_rate, markupBps: r.markup_bps, provider: r.provider, rateTimestamp: r.rate_timestamp, validFrom: r.valid_from, validUntil: r.valid_until, signature: r.signature, keyId: r.key_id, createdAt: r.created_at });
export const cardCanonical = (c: { id: string; sourceCurrency: string; destCurrency: string; customerRate: number; validFrom: string; validUntil: string; policyId: string }) => ['DD-RATE-CARD', c.id, c.sourceCurrency, c.destCurrency, c.customerRate.toFixed(8), c.validFrom, c.validUntil, c.policyId].join('|');
...
export function issueRateCard(sourceCurrency: string, destCurrency: string, actor: Actor = { type: 'system' }): RateCard {
  const db = getDb();
  const policy = db.prepare("SELECT * FROM fx_rate_policies WHERE source_currency = ? AND dest_currency = ? AND status = 'ACTIVE'").get(sourceCurrency.toUpperCase(), destCurrency.toUpperCase()) as any;
  ...
  const key = platformSigningKey();
  const signature = Buffer.from(signWithKey(key.keyId, cardCanonical({ id, sourceCurrency: p.sourceCurrency, destCurrency: p.destCurrency, customerRate, validFrom, validUntil, policyId: p.id }))).toString('base64');
  db.prepare('INSERT INTO fx_rate_cards (id, policy_id, source_currency, dest_currency, mid_rate, customer_rate, markup_bps, provider, rate_timestamp, valid_from, valid_until, signature, key_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, p.id, p.sourceCurrency, p.destCurrency, fx.midRate, customerRate, p.markupBps, fx.provider, fx.rateTimestamp, validFrom, validUntil, signature, key.keyId, now());
...
export function activeRateCard(sourceCurrency: string, destCurrency: string, issueIfMissing = true): RateCard | null {
  const r = getDb().prepare('SELECT * FROM fx_rate_cards WHERE source_currency = ? AND dest_currency = ? AND valid_until > ? ORDER BY valid_until DESC LIMIT 1').get(sourceCurrency.toUpperCase(), destCurrency.toUpperCase(), now()) as any;
...
export function listRateCards(): RateCard[] {
  return listRatePolicies().map((p) => activeRateCard(p.sourceCurrency, p.destCurrency)).filter(Boolean) as RateCard[];
}
...
const toInstitution = (r: any): Institution => {
  ...
  return { userId: r.user_id, kind: r.kind, name: r.name, registryRef: r.registry_ref, purposeCodes: parseJson(r.purpose_codes, []), country: r.country, status: r.status, verifiedBy: r.verified_by, verifiedAt: r.verified_at, createdAt: r.created_at, updatedAt: r.updated_at, user: u ? toPublicUser(u) : null };
};
export function registerInstitution(merchant: UserRow, input: { kind: string; name: string; registryRef?: string | null; purposeCodes: string[]; country?: string | null }): Institution {
  ...
  if (existing) db.prepare("UPDATE institutions SET kind = ?, name = ?, registry_ref = ?, purpose_codes = ?, country = ?, status = CASE WHEN status = 'verified' THEN 'pending' ELSE status END, updated_at = ? WHERE user_id = ?").run(input.kind, input.name.trim(), input.registryRef ?? null, JSON.stringify(codes), country, now(), merchant.id);
  else db.prepare('INSERT INTO institutions (user_id, kind, name, registry_ref, purpose_codes, country, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(merchant.id, input.kind, input.name.trim(), input.registryRef ?? null, JSON.stringify(codes), country, 'pending', now(), now());
...
export function listInstitutions(filter: { status?: string | null; country?: string | null; purposeCode?: string | null; q?: string | null } = {}): Institution[] {
  const rows = (getDb().prepare(`SELECT * FROM institutions ${filter.status ? 'WHERE status = ?' : ''} ORDER BY name`).all(...(filter.status ? [filter.status] : [])) as any[]).map(toInstitution);
  return rows.filter((i) => (!filter.country || i.country === filter.country.toUpperCase()) && (!filter.purposeCode || i.purposeCodes.includes(filter.purposeCode.toUpperCase())) && (!filter.q || i.name.toLowerCase().includes(filter.q.toLowerCase())));
}
export function reviewInstitution(userId: string, admin: UserRow, decision: 'verified' | 'suspended' | 'pending', note?: string | null): Institution {
  ...
  getDb().prepare('UPDATE institutions SET status = ?, verified_by = ?, verified_at = ?, updated_at = ? WHERE user_id = ?').run(decision, decision === 'verified' ? admin.id : i.verifiedBy, decision === 'verified' ? now() : i.verifiedAt, now(), userId);
  recordEvent('risk', userId, `institution.${decision}`, { type: 'admin', id: admin.id }, { note: note ?? null });
  notify(userId, decision === 'verified' ? 'Institution verified' : `Institution ${decision}`, decision === 'verified' ? 'Diaspora-Direct payments for your registered purposes are now enabled.' : note ?? 'Contact support for details.', { kind: 'kyc' });
...
export function institutionQr(merchant: UserRow, input: { purposeCode: string; currency: string; reference?: string | null; amountMinor?: number | null }) {
  ...
  return createStaticQr(merchant, { currency: input.currency, purposeCode: code, reference: input.reference ?? null, kind: 'institution', sign: true, corridorFlag: 'DD', amount: input.amountMinor ?? null, rails: ['wallet', 'card', 'bank'] });
}
...
const toQuote = (r: any): DiasporaQuote => {
  ...
  return { id: r.id, payerId: r.payer_user_id, beneficiaryId: r.beneficiary_user_id, beneficiary: b ? toPublicUser(b) : null, rateCardId: r.rate_card_id, sourceCurrency: r.source_currency, destCurrency: r.dest_currency, sourceMinor: r.source_minor, feeMinor: r.fee_minor, destMinor: r.dest_minor, customerRate: r.customer_rate, purposeCode: r.purpose_code, reference: r.reference, status: r.status === 'QUOTED' && r.expires_at < now() ? 'EXPIRED' : r.status, transactionId: r.transaction_id, expiresAt: r.expires_at, paidAt: r.paid_at, createdAt: r.created_at, disclosure: { rateCard: card ? toCard(card) : null, totalSourceMinor: r.source_minor + r.fee_minor } };
};
export function assertPurposeAllowed(beneficiary: UserRow, purposeCode: string) {
  ...
  if (!inst || inst.status !== 'verified' || !inst.purposeCodes.includes(code)) throw unprocessable(`${code} payments can only go to an institution registered and verified for that purpose`, 'purpose_not_allowed', { purposeCode: code });
}
export function createQuote(payer: UserRow, input: { beneficiary: string; sourceCurrency: string; destCurrency?: string | null; sourceMinor?: number | null; destMinor?: number | null; purposeCode: string; reference?: string | null }): DiasporaQuote {
  ...
  getDb().prepare('INSERT INTO diaspora_quotes (id, payer_user_id, beneficiary_user_id, rate_card_id, source_currency, dest_currency, source_minor, fee_minor, dest_minor, customer_rate, purpose_code, reference, status, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, payer.id, beneficiary.id, card.id, src.code, dst.code, sourceMinor, fee, destMinor, card.customerRate, input.purposeCode.toUpperCase(), input.reference ?? null, 'QUOTED', expiresAt, now());
  publish('diaspora.quote_created', { quoteId: id, payerId: payer.id, beneficiaryId: beneficiary.id, pair: `${src.code}/${dst.code}`, sourceMinor, destMinor, purposeCode: input.purposeCode.toUpperCase() }, { aggregateId: id });
...
export function payQuote(payer: UserRow, id: string, ctx: { stepUpVerified?: boolean; deviceHash?: string | null; ipCountry?: string | null } = {}): DiasporaQuote {
  ...
  enforceOutboundRisk({ userId: payer.id, kind: 'remittance', amount: q.sourceMinor, currency: q.sourceCurrency, subjectType: 'diaspora_quote', subjectId: q.id, counterparty: { name: beneficiary.full_name, phone: beneficiary.phone, email: beneficiary.email, country: beneficiary.country }, method: 'wallet', recipientUserId: beneficiary.id, stepUpVerified: ctx.stepUpVerified ?? false, deviceHash: ctx.deviceHash ?? null, ipCountry: ctx.ipCountry ?? null });
  const from = getUserWallet(payer.id, q.sourceCurrency);
  const to = ensureWallet(beneficiary.id, q.destCurrency);
  const tx = postTransaction({ type: 'remittance', amount: q.sourceMinor, fee: q.feeMinor, currency: q.sourceCurrency, fromWalletId: from.id, toWalletId: to.id, receiveAmount: q.destMinor, receiveCurrency: q.destCurrency, senderUserId: payer.id, receiverUserId: beneficiary.id, note: q.reference ?? `Diaspora-Direct ${q.purposeCode.toLowerCase().replace('_', ' ')} payment`, metadata: { method: 'wallet', diasporaDirect: true, quoteId: q.id, rateCardId: q.rateCardId, customerRate: q.customerRate, purposeCode: q.purposeCode, corridor: `${q.sourceCurrency}/${q.destCurrency}` }, idempotencyKey: `dd:${q.id}` });
  ...
  notify(beneficiary.id, `${q.purposeCode.replace('_', ' ')} payment received`, `${q.destMinor / 10 ** getCurrency(q.destCurrency, false).decimals} ${q.destCurrency} from ${payer.full_name}${q.reference ? ` (${q.reference})` : ''}.`, { kind: 'payment_received', transactionId: tx.id });
  emitEvent(beneficiary.id, 'payment.completed', { transaction: { id: tx.id, amount: q.destMinor, currency: q.destCurrency, method: 'diaspora_direct', purposeCode: q.purposeCode, quoteId: q.id } }, { resource: { type: 'transaction', id: tx.id } });
...
export const baseOf = (amount: number, currency: string) => toBase(amount, currency);
