...
const L = {
  en: { welcome: 'Welcome to BitriPay', menu: '1 Balance\n2 Send money\n3 My code\n4 Cash out at agent\n5 Pay merchant\n6 Mini statement\n7 Language\n0 Help', register: '1 Register\n2 Help', name: 'Your full name:', pin: 'Choose a 4-digit PIN:', pin2: 'Repeat PIN:', enterPin: 'Enter PIN:', recipient: 'Recipient (@tag or phone):', amount: 'Amount', agent: 'Agent (@tag or phone):', merchant: 'Merchant (@tag or phone):', confirm: 'Confirm', fee: 'fee', wrongPin: 'Wrong PIN.', unknown: 'Unknown option.', bye: 'Thank you for using BitriPay.', code: 'Your BitriPay code is', codeHelp: 'Others can send to this code or your phone number. Agents cash in to it.', sent: 'Sent', cashCode: 'Cash-out code', showAgent: 'Show it to the agent within 30 minutes.', lang: '1 English\n2 Français\n3 Kiswahili', langSet: 'Language set.', help: 'Send money to any @code or phone. Cash in and out at any BitriPay agent. Help:', balance: 'Balance', noWallet: 'No wallet yet. Cash in at an agent to start.', currency: 'Currency', registered: 'Welcome', tooMuch: 'Amount above the USSD limit', notFound: 'Not found', closed: 'Registration is closed. Use the app.', off: 'This service is not available right now.', invalidAmount: 'Invalid amount.', pinMismatch: 'PINs do not match.', last: 'Last transactions' },
  fr: { welcome: 'Bienvenue sur BitriPay', menu: '1 Solde\n2 Envoyer\n3 Mon code\n4 Retrait agent\n5 Payer marchand\n6 Mini relevé\n7 Langue\n0 Aide', register: '1 S\'inscrire\n2 Aide', name: 'Votre nom complet:', pin: 'Choisissez un PIN à 4 chiffres:', pin2: 'Répétez le PIN:', enterPin: 'Entrez le PIN:', recipient: 'Destinataire (@code ou téléphone):', amount: 'Montant', agent: 'Agent (@code ou téléphone):', merchant: 'Marchand (@code ou téléphone):', confirm: 'Confirmer', fee: 'frais', wrongPin: 'PIN incorrect.', unknown: 'Option inconnue.', bye: 'Merci d\'utiliser BitriPay.', code: 'Votre code BitriPay est', codeHelp: 'On peut vous envoyer de l\'argent avec ce code ou votre numéro. Les agents déposent dessus.', sent: 'Envoyé', cashCode: 'Code de retrait', showAgent: 'Montrez-le à l\'agent dans les 30 minutes.', lang: '1 English\n2 Français\n3 Kiswahili', langSet: 'Langue définie.', help: 'Envoyez à tout @code ou téléphone. Dépôt et retrait chez tout agent BitriPay. Aide:', balance: 'Solde', noWallet: 'Pas encore de portefeuille. Déposez chez un agent.', currency: 'Devise', registered: 'Bienvenue', tooMuch: 'Montant au-dessus de la limite USSD', notFound: 'Introuvable', closed: 'Inscription fermée. Utilisez l\'application.', off: 'Service indisponible pour le moment.', invalidAmount: 'Montant invalide.', pinMismatch: 'Les PIN ne correspondent pas.', last: 'Dernières transactions' },
  sw: { welcome: 'Karibu BitriPay', menu: '1 Salio\n2 Tuma pesa\n3 Kodi yangu\n4 Toa pesa kwa wakala\n5 Lipa mfanyabiashara\n6 Taarifa fupi\n7 Lugha\n0 Msaada', register: '1 Jisajili\n2 Msaada', name: 'Jina lako kamili:', pin: 'Chagua PIN ya tarakimu 4:', pin2: 'Rudia PIN:', enterPin: 'Weka PIN:', recipient: 'Mpokeaji (@kodi au simu):', amount: 'Kiasi', agent: 'Wakala (@kodi au simu):', merchant: 'Mfanyabiashara (@kodi au simu):', confirm: 'Thibitisha', fee: 'ada', wrongPin: 'PIN si sahihi.', unknown: 'Chaguo halijulikani.', bye: 'Asante kwa kutumia BitriPay.', code: 'Kodi yako ya BitriPay ni', codeHelp: 'Wengine wanaweza kutuma kwa kodi hii au namba yako ya simu. Mawakala huweka pesa humo.', sent: 'Imetumwa', cashCode: 'Kodi ya kutoa pesa', showAgent: 'Mwonyeshe wakala ndani ya dakika 30.', lang: '1 English\n2 Français\n3 Kiswahili', langSet: 'Lugha imewekwa.', help: 'Tuma pesa kwa @kodi au simu yoyote. Weka na toa pesa kwa wakala yeyote wa BitriPay. Msaada:', balance: 'Salio', noWallet: 'Hakuna pochi bado. Weka pesa kwa wakala kuanza.', currency: 'Sarafu', registered: 'Karibu', tooMuch: 'Kiasi kinazidi kikomo cha USSD', notFound: 'Haipatikani', closed: 'Usajili umefungwa. Tumia programu.', off: 'Huduma haipatikani kwa sasa.', invalidAmount: 'Kiasi si sahihi.', pinMismatch: 'PIN hazilingani.', last: 'Miamala ya mwisho' },
} as const;
...
export function ussdHandle(phoneRaw: string, inputs: string[]): UssdReply {
  ...
  switch (a) {
    ...
    case '5': {
      ...
      if (d === undefined) return CON(`${t.confirm}: ${money(minor, currency)} → ${recipient.business_name || recipient.full_name} (@${recipient.tag}), ${t.fee} ${money(fee, currency)}.\n${t.enterPin}`);
      if (!pinOk(user, d)) return END(t.wrongPin);
      try {
        const tx = sendMoney(user, { to: `@${recipient.tag}`, amount: minor, currency, note: a === '5' ? 'USSD merchant payment' : 'USSD transfer', type: a === '5' ? 'merchant_payment' : 'transfer', idempotencyKey: `ussd:${phone}:${inputs.join('*')}:${now().slice(0, 13)}` });
...
export function ussdRequest(input: { sessionId: string; phone: string; text: string; provider?: string; fullPath?: boolean }): UssdReply {
  ...
  if (fresh) db.prepare('INSERT OR REPLACE INTO ussd_sessions (id, phone, provider, inputs, last_response, ended, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(input.sessionId, phone, provider, JSON.stringify(inputs), reply.text, reply.end ? 1 : 0, now(), now());
...
export function recentUssdSessions(limit = 30) {
  return (getDb().prepare('SELECT * FROM ussd_sessions ORDER BY updated_at DESC LIMIT ?').all(limit) as any[]).map((r) => ({ id: r.id, phone: r.phone, provider: r.provider, inputs: parseJson<string[]>(r.inputs, []), lastResponse: r.last_response, ended: !!r.ended, createdAt: r.created_at, updatedAt: r.updated_at }));
}
