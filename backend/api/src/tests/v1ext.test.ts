...
beforeAll(() => {
  app = setupApp();
  getDb().prepare("UPDATE currencies SET enabled = 1 WHERE code IN ('CDF', 'KES', 'GBP')").run();
});
...
describe('gateway v1 account endpoints', () => {
  it('parses CSV with quotes, CRLF and blank lines', () => {
    ...
    expect(rows).toEqual([
      { method: 'wallet', amount: '25.00', wallet: '@amina', name: 'Kalala, Amina' },
      { method: 'wallet', amount: '40', wallet: '@jean', name: 'He said "hi"' },
    ]);
  });
  ...
  it('reads wallets, quotes and ranks routes, and sends idempotent transfers with an API key', async () => {
    ...
    const key = await request(app)
      ...
      .set(m.auth)
      .send({ label: 'erp', mode: 'test', kind: 'restricted', scopes: ['wallets:read', 'transfers:write', 'payouts:write', 'payouts:read', 'ai:run', 'remittances:write'] });
    ...
    expect(
      (
        await request(app)
          .get('/api/v1/wallets')
          .set({ Authorization: `Bearer ${ro.body.secret}` })
      ).status,
    ).toBe(403);
    ...
    const q1 = await request(app)
      ...
      .set(k)
      .send({ amount_minor: 2500, currency: 'USD', destination: { method: 'wallet', to: a.user.tag } });
    ...
    const q2 = await request(app)
      .post('/api/v1/transfers/quote')
      .set(k)
      .send({ amount_minor: 2500, currency: 'USD', target_currency: 'KES', destination: { method: 'mobile_money', operator_id: 'mpesa_ke', phone: '+254712345678' }, policy: 'cheapest' });
    ...
    const t1 = await request(app)
      ...
      .set({ ...k, 'Idempotency-Key': 'sal-001' })
      .send({ to: a.user.tag, amount_minor: 2500, currency: 'USD', note: 'Salary' });
    ...
    const r = await request(app)
      ...
      .set({ ...k, 'Idempotency-Key': 'rem-1' })
      .send({ amount_minor: 1000, currency: 'USD', target_currency: 'USD', payout_method: 'wallet', recipient: { name: 'Family', tag: b.user.tag } });
    expect(r.status, JSON.stringify(r.body)).toBe(201);
    expect(r.body.remittance.status).toBe('completed');
    const rAgain = await request(app)
      .post('/api/v1/remittances')
      .set({ ...k, 'Idempotency-Key': 'rem-1' })
      .send({ amount_minor: 1000, currency: 'USD', target_currency: 'USD', payout_method: 'wallet', recipient: { name: 'Family', tag: b.user.tag } });
  ...
  it('validates a bulk payout batch row by row, enforces four-eyes or step-up on approval, executes in order and reports per-row outcomes', async () => {
    ...
    const key = await request(app)
      .post('/api/v1/api_keys')
      .set(m.auth)
      .send({ label: 'payroll', mode: 'test', kind: 'restricted', scopes: ['payouts:write', 'payouts:read', 'payouts:approve'] });
    const k = { Authorization: `Bearer ${key.body.secret}` };
    const csv = [
      'method,amount,wallet,operator_id,phone,name,reference',
      `wallet,25.00,${amina.user.tag},,,Amina,SAL-1`,
      `wallet,40.00,${jean.user.tag},,,Jean,SAL-2`,
      'wallet,10.00,@nobody-here,,,Ghost,SAL-3',
      `wallet,abc,${amina.user.tag},,,Amina,SAL-4`,
      'mobile_money,30.00,,mpesa_ke,+254712345678,Wanjiru,SAL-5',
    ].join('\n');
    const created = await request(app)
      .post('/api/v1/payouts/batches')
      .set({ ...k, 'Idempotency-Key': 'payroll-sept' })
      .send({ currency: 'USD', csv, reference: 'PAYROLL-09' });
    ...
    const big = await request(app)
      .post('/api/v1/payouts/batches')
      .set(k)
      .send({ currency: 'USD', rows: [{ amount_minor: 50_000, destination: { method: 'wallet', to: amina.user.tag } }] });
    ...
    const third = await request(app)
      .post('/api/v1/payouts/batches')
      .set(k)
      .send({ currency: 'USD', rows: [{ amount_minor: 500, destination: { method: 'wallet', to: jean.user.tag }, reference: 'BONUS' }] });
...
describe('cross-border routes on the partner API', () => {
  it('quotes and creates any → any routes in the recipient currency, funds remittances from any card, and follows every stage', async () => {
    ...
    const key = await request(app)
      .post('/api/v1/api_keys')
      .set(m.auth)
      .send({ label: 'payroll-routes', mode: 'test', kind: 'restricted', scopes: ['routes:read', 'routes:write', 'remittances:write'] });
    const k = { Authorization: `Bearer ${key.body.secret}` };
    ...
    const currencies = await request(app)
      .post('/api/v1/routes/payout-currencies')
      .set(k)
      .send({ amount_minor: 5000, currency: 'USD', destination: { method: 'mobile_money', operator_id: 'mpesa_ke', phone: '+254712345678', name: 'Wanjiru' } });
    ...
    const r1 = await request(app)
      .post('/api/v1/routes')
      .set({ ...k, 'Idempotency-Key': 'pay-wanjiru-1' })
      .send({
        amount_minor: 5000,
        currency: 'USD',
        target_currency: 'KES',
    ...
    const rem = await request(app)
      ...
      .send({
        ...
        recipient: { name: 'Marie Kabila', phone: '+243990000123', country: 'CD', operator_id: 'orange_cd' },
    ...
    const declined = await request(app)
      ...
      .send({
        ...
        currency: 'USD',
        target_currency: 'CDF',
        source: { method: 'card', card: { ...card, number: '4000000000000002' } },
        destination: { method: 'mobile_money', operator_id: 'orange_cd', phone: '+243990000123', name: 'Marie Kabila' },
      });
    ...
    expect(
      (
        await request(app)
          .post('/api/v1/routes/quote')
          .set({ Authorization: `Bearer ${ro.body.secret}` })
          .send({ amount_minor: 100, currency: 'USD', destination: { method: 'keep' } })
      ).status,
    ).toBe(403);
    expect(reconcileLedger().ok).toBe(true);
  }, 30_000);
