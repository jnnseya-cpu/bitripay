...
r.get('/gateway/models', requirePermission('agents'), (_req, res) =>
  res.json({ models: modelHealth().map((m) => ({ ...m, economics: projectEconomics(m.model, getAcuPolicy().expectedTokensPerRun) })), floor: MIN_GROSS_MARGIN }),
);
r.put('/gateway/acu-policy', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({
      acuPriceMicros: z.number().int().min(1).optional(),
      acuPerKiloToken: z.number().min(0.001).optional(),
      minGrossMargin: z.number().min(0).max(0.99).optional(),
      monthlyBudgetPerUser: z.number().min(0).optional(),
      monthlyBudgetPerTenant: z.number().min(0).optional(),
      overage: z.enum(['block', 'bill']).optional(),
      expectedTokensPerRun: z.number().int().min(100).optional(),
      expectedTokensStandard: z.number().int().min(100).optional(),
      expectedTokensDeep: z.number().int().min(100).optional(),
      alertBelowMargin: z.number().min(0).max(1).optional(),
    }),
    req.body,
  );
  if (b.minGrossMargin !== undefined && b.minGrossMargin < MIN_GROSS_MARGIN)
    throw unprocessable(`The margin floor cannot be set below ${MIN_GROSS_MARGIN}`, 'margin_protection_violation', { floor: MIN_GROSS_MARGIN });
...
r.put('/gateway/routing', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({
      taskTypes: z.record(z.string(), z.array(z.string().min(3)).min(1)).optional(),
      timeoutMs: z.number().int().min(1000).max(60_000).optional(),
      circuitFailures: z.number().int().min(1).optional(),
      circuitWindowMs: z.number().int().min(10_000).optional(),
      providers: z.record(z.string(), z.object({ enabled: z.boolean(), apiKey: z.string().optional().nullable(), baseUrl: z.string().url().optional().nullable() })).optional(),
      rateLimits: z
        .object({ perUserPerMinute: z.number().int().min(0), perTenantPerMinute: z.number().int().min(0), perAgentPerMinute: z.number().int().min(0) })
        .partial()
        .optional(),
    }),
    req.body,
  ...
  setSetting('aiRouting', {
    ...current,
    ...b,
    taskTypes: { ...(current.taskTypes ?? {}), ...(b.taskTypes ?? {}) },
    providers: { ...(current.providers ?? {}), ...(b.providers ?? {}) },
    rateLimits: { ...(current.rateLimits ?? {}), ...(b.rateLimits ?? {}) },
  });
...
r.get('/gateway/ledger', requirePermission('agents'), (req, res) => {
  const rows = getDb()
    .prepare('SELECT * FROM ai_usage_ledger ORDER BY created_at DESC LIMIT ?')
    .all(Math.min(500, Number(req.query.limit) || 100)) as any[];
  res.json({
    items: rows.map((x) => ({
      id: x.id,
      ...
      provider: x.provider,
      model: x.model,
      tokensIn: x.tokens_in,
      tokensOut: x.tokens_out,
      rawCostMicros: x.raw_cost_micros,
      acuUsed: x.acu_used,
      acuRevenueMicros: x.acu_revenue_micros,
      margin: x.margin,
      latencyMs: x.latency_ms,
      outcome: x.outcome,
      errorCode: x.error_code,
      billedTo: x.billed_to,
...
// ---------------------------------------------------------------- agent mesh
r.get('/mesh', requirePermission('agents'), (_req, res) =>
  res.json({
    bindings: listBindings(),
    agents: MESH_AGENTS.map((a) => ({ key: a.key, registryId: a.registryId, name: a.name, aliases: a.aliases ?? [], bindings: a.bindings ?? [], tools: a.tools })),
    aliases: OS_AGENT_ALIASES,
...
r.get('/events', requirePermission('agents'), (req, res) =>
  res.json({
    items: listDomainEvents({
      type: req.query.type ? String(req.query.type) : null,
      aggregateId: req.query.aggregate ? String(req.query.aggregate) : null,
...
r.post('/diaspora/policies', requirePermission('treasury'), (req, res) => {
  const b = validate(
    z.object({
      ...
      maxValidityHours: z.number().int().min(1).max(4).optional(),
      feeBps: z.number().int().min(0).max(1000).optional(),
      feeFixedSourceMinor: z.number().int().min(0).optional(),
      minSourceMinor: z.number().int().min(0).optional(),
      maxSourceMinor: z.number().int().min(0).optional(),
...
r.get('/offline', requirePermission('transactions'), (req, res) => {
  ...
  const promises = (
    db
      .prepare(`SELECT * FROM offline_promises ${req.query.state ? 'WHERE sync_state = ?' : ''} ORDER BY created_at DESC LIMIT ?`)
      .all(...(req.query.state ? [String(req.query.state)] : []), Number(req.query.limit) || 100) as any[]
  ).map((x) => ({
    hash: x.intent_hash,
    merchantId: x.merchant_user_id,
    payerId: x.payer_user_id,
    deviceId: x.payer_device_id,
  ...
  res.json({
    ...
    stats: stats.map((s) => ({ state: s.sync_state, count: s.n, amountMinor: s.s })),
    devices: (db.prepare('SELECT COUNT(*) c FROM offline_devices').get() as any).c,
    promises,
  });
});
r.put('/offline/settings', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({
      ...
      maxPerPromiseBase: z.number().int().min(0).optional(),
      maxOutstandingPerDeviceBase: z.number().int().min(0).optional(),
      promiseValidityHours: z.number().int().min(1).max(72).optional(),
      qrTtlSeconds: z.number().int().min(60).max(3600).optional(),
    }),
    req.body,
  );
