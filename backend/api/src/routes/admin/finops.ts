...
import {
  ...
  withdrawDispute,
  disputeChronology,
  sweepDisputeDeadlines,
  getDisputeSettings,
} from '../../services/finops/disputes';
...
/** Processor CSV → statement lines. Column names are matched case-insensitively; missing currency falls back to the statement's. */
export function statementLinesFromCsv(csv: string, currency: string) {
  return parseCsv(csv)
    .map((r) => {
      const col = (name: string) => r[name] ?? r[name.toLowerCase()] ?? r[name.replace(/([A-Z])/g, '_$1').toLowerCase()] ?? '';
      return {
        reference: col('reference').trim(),
        amountMinor: col('amountMinor').trim(),
        currency: (col('currency').trim() || currency).toUpperCase(),
        status: col('status').trim() || 'SETTLED',
        feeMinor: col('feeMinor').trim() || null,
        settlementRef: col('settlementRef').trim() || null,
        occurredAt: col('occurredAt').trim() || null,
      };
    })
    .filter((l) => l.reference && l.amountMinor !== '');
}
...
r.get('/fees/schedules', requirePermission('settings'), (req, res) =>
  res.json({
    items: listFeeSchedules({
      scope: (req.query.scope as any) ?? null,
      scopeRef: req.query.scope_ref ? String(req.query.scope_ref) : null,
...
r.post('/fees/schedules', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({
      scope: z.enum(['platform', 'country', 'tier', 'merchant']),
...
// ---------------------------------------------------------------- settlements
r.get('/settlements/cycles', requirePermission('transactions'), (req, res) =>
  res.json({
    items: listCycles({
...
// ---------------------------------------------------------------- disputes
r.get('/disputes', requirePermission('transactions'), (req, res) =>
  res.json({
    items: listDisputes({
      merchantId: req.query.merchant ? String(req.query.merchant) : null,
      status: req.query.status ? String(req.query.status) : null,
      limit: Math.min(500, Number(req.query.limit) || 100),
    }),
    settings: getDisputeSettings(),
  }),
);
r.put('/disputes/settings', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({
      responseDays: z.record(z.string(), z.number().int().min(1).max(120)).optional(),
      onDeadline: z.enum(['LOST', 'UNDER_REVIEW']).optional(),
      reasonCodes: z.array(z.string().min(2).max(40)).min(1).optional(),
    }),
    req.body,
  );
...
r.post('/disputes', requirePermission('transactions'), (req, res) => {
  const b = validate(
    z.object({
      ...
      switchPaymentId: z.string().optional().nullable(),
      openedBy: z.enum(['customer', 'merchant', 'processor', 'institution', 'admin']).optional(),
      reasonCode: z.string().min(2),
      reason: z.string().max(2000).optional().nullable(),
      amountMinor: z.number().int().positive().optional().nullable(),
      responsibleInstitution: z.string().max(80).optional().nullable(),
      evidenceText: z.string().max(8000).optional().nullable(),
    }),
    req.body,
  );
...
// ---------------------------------------------------------------- holds
r.get('/holds', requirePermission('transactions'), (req, res) =>
  res.json({
    items: listHolds({
      userId: req.query.user ? String(req.query.user) : null,
      walletId: req.query.wallet ? String(req.query.wallet) : null,
      status: req.query.status ? String(req.query.status) : null,
      limit: Math.min(500, Number(req.query.limit) || 100),
    }),
  }),
...
r.post('/holds', requirePermission('treasury'), (req, res) => {
  const b = validate(
    z.object({
      ...
      kind: z.enum(['dispute', 'reserve', 'review', 'settlement', 'compliance']),
      reason: z.string().min(3).max(500),
      refType: z.string().max(40).optional().nullable(),
      refId: z.string().max(80).optional().nullable(),
...
// ---------------------------------------------------------------- commissions
r.get('/commissions/overview', requirePermission('agents'), (req, res) =>
  res.json({ ...commissionOverview(req.query.period ? String(req.query.period) : undefined), settings: getCommissionSettings() }),
);
r.put('/commissions/settings', requirePermission('settings'), (req, res) => {
  const b = validate(
    z.object({ platformShareBps: z.number().int().min(0).max(10_000).optional(), payoutThreshold: z.number().int().min(0).optional(), onboardingFeeMinor: z.number().int().min(0).optional() }),
    req.body,
  );
...
r.get('/commissions/:agentId', requirePermission('agents'), (req, res) =>
  res.json({
    statement: commissionStatement(String(req.params.agentId), req.query.period ? String(req.query.period) : undefined),
    entries: listCommissions(String(req.params.agentId), { period: req.query.period ? String(req.query.period) : null, limit: Number(req.query.limit) || 100 }),
  }),
);
...
r.post('/reconciliation/processors/:gatewayId/statements', requirePermission('reconciliation'), (req, res) => {
  const lineSchema = z.object({
    ...
    amountMinor: z.union([z.number().int(), z.string()]),
    currency: z.string().length(3),
    status: z.string().min(1),
    feeMinor: z.union([z.number().int(), z.string()]).optional().nullable(),
    settlementRef: z.string().optional().nullable(),
    occurredAt: z.string().optional().nullable(),
  });
  // Either structured lines or the processor's CSV export (header: reference, amountMinor, status, feeMinor[, currency, settlementRef, occurredAt]).
  const b = validate(
    z.object({
      source: z.enum(['PROCESSOR', 'BANK']).optional().default('PROCESSOR'),
      cycleRef: z.string().min(4).max(40),
      currency: z.string().length(3),
      lines: z.array(lineSchema).max(50_000).optional(),
      csv: z.string().max(5_000_000).optional(),
      controlTotalMinor: z.union([z.number().int(), z.string()]).optional().nullable(),
  ...
  const lines = b.lines?.length ? b.lines : statementLinesFromCsv(b.csv ?? '', b.currency);
  if (!lines.length) throw badRequest('Provide statement lines or a CSV with at least one line', 'empty_statement');
  const imp = importProcessorStatement(String(req.params.gatewayId), { ...b, lines }, req.user!.id);
  audit(req.user!.id, 'reconciliation.processor.import', 'reconciliation_import', imp.id, {
...
r.get('/users/:userId/finance', requirePermission('users'), (req, res) => {
  ...
  res.json({
    tier: (getDb().prepare('SELECT fee_tier FROM users WHERE id = ?').get(u.id) as any)?.fee_tier ?? null,
    fees: effectiveFees({ userId: u.id }),
    holds: listHolds({ userId: u.id, status: 'ACTIVE' }),
    profiles: listProfiles(u.id),
    cycles: listCycles({ userId: u.id, limit: 20 }),
    disputes: listDisputes({ merchantId: u.id, limit: 20 }),
  });
});

// ---------------------------------------------------------------- bulk payout batches (module 14): oversight and four-eyes approval
r.get('/payout-batches', requirePermission('transactions'), (req, res) =>
  res.json({ items: listAllBatches({ status: req.query.status ? String(req.query.status) : null, limit: Number(req.query.limit) || 100 }) }),
);
