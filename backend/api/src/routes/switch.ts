...
import { createPayment, getPayment, listPayments, cancelPayment, attachConsent, createLinkedRefund, listLinkedOperations, refundable, createBinding, listBindings, getBinding, recordConsent, paymentTimeline } from '../services/switch/payments';
...
switchRouter.get('/payments', ...merchantOnly, requireScope('payments:read'), (req, res) => res.json({ data: listPayments({ merchantUserId: req.user!.id, status: req.query.status ? String(req.query.status) : null, limit: Number(req.query.limit) || 50 }) }));
...
switchRouter.get('/payments/:id/timeline', ...merchantOnly, requireScope('payments:read'), (req, res) => {
  ...
  res.json({ payment: t.payment, events: t.events.map((e) => ({ seq: e.seq, type: e.type, source: e.source, from: e.from, to: e.to, occurredAt: e.occurredAt, receivedAt: e.receivedAt })), journal: t.journal.map((j) => ({ fact: j.fact, amountMinor: j.amountMinor, currency: j.currency, occurredAt: j.occurredAt })), linkedOperations: t.linkedOperations, webhooks: t.webhooks });
});
switchRouter.post('/payments/:id/cancel', ...merchantOnly, requireScope('payments:cancel'), writeLimit, (req, res) => res.json(cancelPayment(req.user!, String(req.params.id), actor(req), req.body?.reason ? String(req.body.reason) : null)));
...
switchRouter.post('/payments/:id/refunds', ...merchantOnly, requireScope('refunds:create'), writeLimit, (req, res) => {
  const body = validate(z.object({ amount_minor: z.union([z.string().regex(/^\d+$/), z.number().int().positive()]).optional().nullable(), reason: z.string().min(1).max(200) }), req.body);
  res.status(201).json(createLinkedRefund(req.user!, String(req.params.id), { amountMinor: body.amount_minor != null ? Number(body.amount_minor) : null, reason: body.reason, idemKey: (req.headers['idempotency-key'] as string | undefined) ?? null }, actor(req)));
});
...
switchRouter.get('/participants', ...merchantOnly, requireScope('participants:read'), (req, res) => {
  ...
  const data = listParticipants({ country, status: 'ACTIVE' }).map((p) => ({ participant_id: p.id, name: p.name, kind: p.kind, currencies: p.currencies, services: p.services, channels: p.channels, valid_from: p.validFrom, valid_to: p.validTo, source: p.source, open_pairs: pairs.filter((x) => x.debtorId === p.id).map((x) => ({ creditor_id: x.creditorId, currency: x.currency, product: x.product, channel: x.channel })) }));
  res.json({ data, connection: conn ? { id: conn.id, scheme_id: conn.schemeId, access_mode: conn.accessMode, environment: conn.environment, simulation: conn.simulation, link_state: conn.linkState } : null });
});
...
switchRouter.post('/qr-intents', ...merchantOnly, requireScope('qr:create', 'payment_intents:write'), writeLimit, (req, res) => {
  const b = validate(z.object({ amount: amountSchema, reference: z.string().max(64).optional().nullable(), description: z.string().max(200).optional().nullable(), purpose_code: z.enum(PURPOSE_CODES).optional().nullable(), ttl_seconds: z.number().int().min(30).max(300).optional(), location_id: z.string().optional().nullable(), terminal_id: z.string().optional().nullable() }), req.body);
  const { row, clientSecret } = createIntent(req.user!, { amountMinor: Number(b.amount.value_minor), currency: b.amount.currency, reference: b.reference ?? null, description: b.description ?? null, purposeCode: b.purpose_code ?? null, source: 'qr', locationId: b.location_id ?? null, terminalId: b.terminal_id ?? null, idemKey: (req.headers['idempotency-key'] as string | undefined) ?? null, expiresInMinutes: Math.max(1, Math.ceil((b.ttl_seconds ?? 300) / 60)) });
  const qr = createDynamicQr(req.user!, row, b.ttl_seconds ?? 300);
  const view = intentView(getIntentRow(row.id));
  res.status(201).json({ intent_id: view.id, status: view.status, amount: { currency: view.amount.currency, value_minor: String(view.amount.valueMinor) }, qr: { id: qr.id, payload: qr.payload, uri: qr.uri, expires_at: qr.expiresAt, signed: qr.signed, key_id: qr.keyId }, checkout_url: view.checkoutUrl, client_secret: clientSecret, expires_at: view.expiresAt });
});

/** §11 `POST /v1/webhook-endpoints` (hyphenated alias of the gateway endpoint object). */
switchRouter.post('/webhook-endpoints', ...merchantOnly, requireScope('webhooks:manage'), writeLimit, wrap(async (req, res) => {
...
switchRouter.get('/reconciliation/cases', ...merchantOnly, requireScope('reconciliation:read'), (req, res) => {
  const r = listCases({ merchantUserId: req.user!.id, status: req.query.status ? String(req.query.status) : null, class: req.query.class ? String(req.query.class) : null, limit: Number(req.query.limit) || 50, cursor: req.query.cursor ? String(req.query.cursor) : null });
  res.json({ data: r.data.map((c) => ({ case_id: c.id, class: c.class, payment_id: c.paymentId, status: c.status, priority: c.priority, exposure: { currency: c.exposure.currency, value_minor: String(c.exposure.valueMinor) }, age_hours: c.ageHours, next_action: c.nextAction, due_at: c.dueAt, opened_at: c.createdAt, updated_at: c.updatedAt })), next_cursor: r.nextCursor });
});

// Beneficiary bindings (CMP-02): requested by the merchant, verified and activated by operations under dual approval.
const bindingSchema = z.object({ participant_id: z.string().min(1), account_token: z.string().min(4).max(200), account_name: z.string().min(2).max(120), replaces_id: z.string().optional().nullable() });
...
switchRouter.post('/consents', ...merchantOnly, writeLimit, (req, res) => {
  const b = validate(z.object({ participant_id: z.string().min(1), beneficiary_binding_id: z.string().optional().nullable(), amount: amountSchema, account_token: z.string().max(200).optional().nullable(), ttl_seconds: z.number().int().min(30).max(3600).optional(), proof: z.string().min(1).max(4000) }), req.body);
  ...
  res.status(201).json(recordConsent({ participantId: b.participant_id, audience: 'bitripay', merchantUserId: req.user!.id, bindingId: b.beneficiary_binding_id ?? null, amountMinor: Number(b.amount.value_minor), currency: b.amount.currency, accountToken: b.account_token ?? null, ttlSeconds: b.ttl_seconds, proof: b.proof }));
});
