...
import { openBankingOverview, listInstitutions, createLink, completeLink, syncLink, revokeLink, listLinks, listTransactions, importStatement, verifyIncome, getIncomeReport, createMandate, listMandates, revokeMandate } from '../services/openBanking';
...
openBankingRouter.post('/sandbox/authorise/:linkId', (req, res) => {
  ...
  if ((req.headers.accept ?? '').includes('text/html')) return res.redirect(`${process.env.WEB_URL ?? ''}/app/banks?link=${view.id}&status=${view.status.toLowerCase()}`);
  res.json({ link: view });
});
...
openBankingRouter.get('/transactions', (req, res) => res.json({ items: listTransactions(req.user!.id, { linkId: req.query.link ? String(req.query.link) : null, accountId: req.query.account ? String(req.query.account) : null, limit: Number(req.query.limit) || 200 }) }));
openBankingRouter.post('/statements', (req, res) => {
  const b = validate(z.object({ institutionName: z.string().min(2).max(120), currency: z.string().length(3), accountName: z.string().max(80).optional().nullable(), csv: z.string().min(10).max(4_000_000) }), req.body);
...
openBankingRouter.post('/mandates', (req, res) => {
  const b = validate(z.object({ linkId: z.string(), accountId: z.string(), purpose: z.enum(['top_up', 'billing']), maxPerPayment: z.string(), maxPerMonth: z.string(), pin: z.string().optional() }), req.body);
  ...
  const cur = getCurrency(String((listLinks(req.user!.id).find((l) => l.id === b.linkId)?.accounts.find((a) => a.id === b.accountId)?.currency) ?? 'USD'));
  res.status(201).json({ mandate: createMandate(req.user!, { linkId: b.linkId, accountId: b.accountId, purpose: b.purpose, maxPerPaymentMinor: toMinor(b.maxPerPayment, cur.decimals), maxPerMonthMinor: toMinor(b.maxPerMonth, cur.decimals) }, { type: 'user', id: req.user!.id }) });
});
