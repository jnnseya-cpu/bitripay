...
const destinationSchema = z.discriminatedUnion('method', [
  ...
  z.object({ method: z.literal('bank'), bank_account_id: z.string().optional().nullable(), bank_name: z.string().optional().nullable(), account_name: z.string().optional().nullable(), account_number: z.string().optional().nullable(), country: z.string().length(2).optional().nullable(), swift: z.string().optional().nullable(), currency: z.string().length(3).optional().nullable() }),
...
function toDest(d: z.output<typeof destinationSchema>): RouteDestination {
  ...
  return { method: 'bank', bankName: d.bank_name ?? '', accountName: d.account_name ?? '', accountNumber: d.account_number ?? '', country: d.country ?? null, currency: d.currency ?? null } as RouteDestination;
}
/** Funding leg of a cross-border route: the account's wallet, or a card / mobile money / bank payment through a processor. */
const cardSchema = z.object({ number: z.string().min(12).max(23), exp_month: z.coerce.number().int().min(1).max(12), exp_year: z.coerce.number().int().min(0).max(2100), cvc: z.string().min(3).max(4), holder_name: z.string().min(2).max(120) });
const sourceSchema = z.object({ method: z.enum(['wallet', 'card', 'bank', 'mobile_money']), gateway: z.string().optional().nullable(), operator_id: z.string().optional().nullable(), phone: z.string().optional().nullable(), card: cardSchema.optional(), saved_card_id: z.string().optional().nullable(), return_url: z.string().url().optional().nullable() });
function toSource(s: z.output<typeof sourceSchema>): RouteSource {
  return { method: s.method, gateway: s.gateway ?? null, operatorId: s.operator_id ?? null, phone: s.phone ?? null, card: s.card ? { number: s.card.number, expMonth: s.card.exp_month, expYear: s.card.exp_year, cvc: s.card.cvc, holderName: s.card.holder_name } : undefined, savedCardId: s.saved_card_id ?? null, returnUrl: s.return_url ?? null };
}
...
v1ExtRouter.post('/routes/quote', requireAuth, requireScope('routes:write', 'routes:read'), (req, res) => {
  const b = validate(z.object({ amount_minor: z.number().int().positive(), currency: z.string().length(3), target_currency: z.string().length(3).optional().nullable(), source: sourceSchema.optional(), destination: destinationSchema }), req.body);
  const cur = getCurrency(b.currency);
  ...
  const quote = quoteRoute(b.amount_minor, cur.code, target.code, b.source?.method ?? 'wallet', dest, { userId: req.user!.id, country: req.user!.country ?? null, operatorId: b.source?.operator_id ?? null, gateway: b.source?.gateway ?? null, persistQuote: true } as any);
...
v1ExtRouter.post('/routes/payout-currencies', requireAuth, requireScope('routes:write', 'routes:read'), (req, res) => {
  const b = validate(z.object({ amount_minor: z.number().int().positive(), currency: z.string().length(3), destination: destinationSchema, requested: z.string().length(3).optional().nullable() }), req.body);
...
v1ExtRouter.post('/routes', requireAuth, requireScope('routes:write'), writeLimit, wrap(async (req, res) => {
  const b = validate(z.object({ amount_minor: z.number().int().positive(), currency: z.string().length(3), target_currency: z.string().length(3).optional().nullable(), source: sourceSchema.default({ method: 'wallet' }), destination: destinationSchema, note: z.string().max(200).optional().nullable(), quote_id: z.string().optional().nullable(), source_of_funds: z.string().max(120).optional().nullable(), pin: z.string().optional() }), req.body);
  ...
  const route = await createRoute(req.user!, { source: toSource(b.source), destination: toDest(b.destination), amount: b.amount_minor, currency: cur.code, targetCurrency: b.target_currency?.toUpperCase() ?? null, note: b.note ?? null, quoteId: b.quote_id ?? null, sourceOfFunds: b.source_of_funds ?? null }, routeAuth(req));
...
v1ExtRouter.get('/routes/:id', requireAuth, requireScope('routes:read', 'routes:write'), wrap(async (req, res) => res.json({ route: await refreshRoute(req.user!.id, String(req.params.id)) })));
...
v1ExtRouter.post('/routes/:id/cancel', requireAuth, requireScope('routes:write'), writeLimit, wrap(async (req, res) => {
...
v1ExtRouter.post('/remittances', requireAuth, requireScope('remittances:write'), writeLimit, wrap(async (req, res) => {
  const b = validate(z.object({ amount_minor: z.number().int().positive(), currency: z.string().length(3), target_currency: z.string().length(3), payout_method: z.enum(['wallet', 'bank', 'cash_pickup', 'mobile_money']), recipient: recipientSchema.extend({ operator_id: z.string().optional().nullable(), bank_account_id: z.string().optional().nullable() }), source: sourceSchema.optional(), note: z.string().max(200).optional().nullable(), pin: z.string().optional() }), req.body);
  if (req.authVia !== 'api_key') assertPin(req.user!, b.pin, req);
  const key = idem(req);
  if ((b.source && b.source.method !== 'wallet') || b.payout_method === 'mobile_money') {
    const r = b.recipient;
    const dest: RouteDestination = b.payout_method === 'mobile_money' ? { method: 'mobile_money', operatorId: r.operator_id ?? '', phone: r.phone ?? '', name: r.name } : b.payout_method === 'bank' ? (r.bank_account_id ? { method: 'bank', bankAccountId: r.bank_account_id } : { method: 'bank', bankName: r.bank_name ?? '', accountName: r.name, accountNumber: r.account_number ?? '', country: r.country ?? null, currency: b.target_currency.toUpperCase() }) : b.payout_method === 'wallet' ? { method: 'wallet', to: r.tag ?? r.phone ?? r.email ?? '' } : { method: 'agent', agent: r.tag ?? '' };
    ...
    const route = await createRoute(req.user!, { source: toSource(b.source ?? { method: 'wallet' }), destination: dest, amount: b.amount_minor, currency: getCurrency(b.currency).code, targetCurrency: b.target_currency.toUpperCase(), note: b.note ?? null }, routeAuth(req));
