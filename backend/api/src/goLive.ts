 ...
 * Boots the platform against the configured database, provisions every rail whose credentials are in the environment
 * (connectivity-checked, enabled only when the check passes), prints the go-live checklist and the gate-to-scale
 * metrics, and exits 1 while a blocking item is open. Nothing here invents readiness: an unconfigured rail stays off.
 */
...
async function main() {
  ...
  console.log('Rails from the environment');
  for (const r of rails)
    console.log(
      `  ${r.outcome === 'enabled' || r.outcome === 'configured' ? '✓' : r.outcome === 'failed' ? '✗' : '·'} ${r.gatewayId.padEnd(12)} ${r.outcome.padEnd(11)} ${r.mode.padEnd(8)} ${r.message}`,
    );
