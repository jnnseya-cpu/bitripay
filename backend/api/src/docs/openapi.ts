...
const OPS: Op[] = [
  ...
  { method: 'post', path: '/routes/quote', tag: 'Cross-border routes', summary: 'Quote any → any: fund from the wallet or any card / bank / mobile money, deliver to a wallet, QR, bank account, mobile money number or agent anywhere, in the currency the recipient will receive; rate, margin and every fee disclosed', scope: 'routes:read', body: { amount_minor: { type: 'integer' }, currency: { type: 'string' }, target_currency: { type: 'string' }, source: { type: 'object', properties: { method: { enum: ['wallet', 'card', 'bank', 'mobile_money'] }, gateway: { type: 'string' }, operator_id: { type: 'string' } } }, destination: { type: 'object', properties: { method: { enum: ['wallet', 'qr', 'mobile_money', 'bank', 'agent', 'keep'] } } } } },
  { method: 'post', path: '/routes/payout-currencies', tag: 'Cross-border routes', summary: 'Which currencies the recipient can receive for a destination right now (corridor rules, licence, liquidity)', scope: 'routes:read', body: { amount_minor: { type: 'integer' }, currency: { type: 'string' }, destination: { type: 'object' }, requested: { type: 'string' } } },
  { method: 'post', path: '/routes', tag: 'Cross-border routes', summary: 'Create a route: the funding leg runs through the processor (card / bank / mobile money) or the wallet, the payout leg follows automatically once funds are confirmed', scope: 'routes:write', idempotent: true, body: { amount_minor: { type: 'integer' }, currency: { type: 'string' }, target_currency: { type: 'string' }, source: { type: 'object' }, destination: { type: 'object' }, note: { type: 'string' }, quote_id: { type: 'string' }, source_of_funds: { type: 'string' } } },
  ...
  { method: 'post', path: '/routes/{id}/cancel', tag: 'Cross-border routes', summary: 'Cancel before the payout is executed (funds return; card funding refunded where the processor allows)', scope: 'routes:write', body: { reason: { type: 'string' } } },
  ...
  { method: 'post', path: '/remittances', tag: 'Remittances', summary: 'Send an international transfer to a wallet, bank account, mobile money number or cash pickup in the recipient currency; add a `source` (any card, bank or mobile money) to fund it through a processor instead of the wallet', scope: 'remittances:write', idempotent: true, body: { amount_minor: { type: 'integer' }, currency: { type: 'string' }, target_currency: { type: 'string' }, payout_method: { enum: ['wallet', 'bank', 'mobile_money', 'cash_pickup'] }, recipient: { type: 'object', properties: { name: { type: 'string' }, country: { type: 'string' }, phone: { type: 'string' }, tag: { type: 'string' }, operator_id: { type: 'string' }, bank_name: { type: 'string' }, account_number: { type: 'string' }, bank_account_id: { type: 'string' } } }, source: { type: 'object', properties: { method: { enum: ['wallet', 'card', 'bank', 'mobile_money'] }, card: { type: 'object' }, saved_card_id: { type: 'string' }, gateway: { type: 'string' }, operator_id: { type: 'string' }, phone: { type: 'string' } } }, note: { type: 'string' } } },
