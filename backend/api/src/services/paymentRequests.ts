...
import { aggregatorPerimeterApplied } from './modules';
...
export function payWithWallet(payer: UserRow, code: string, amount?: number | null, note?: string | null): { tx: TransactionRow; request: PaymentRequestRow } {
  ...
  return db.transaction(() => {
    ...
    const fee =
      type === 'money_request' ? calculateFee('money_request', finalAmount, currency.code, null, { userId: payer.id }) : calculateFee(type, finalAmount, currency.code, null, { userId: requester.id });
...
export function checkoutInfo(code: string) {
  ...
  let methods: string[] = isMerchantRole(requester.role) ? gateway.methods.filter((m) => allowed.length === 0 || allowed.includes(m)) : ['wallet'];
  ...
  const viaSwitch = isMerchantRole(requester.role) && (allowed.length === 0 || allowed.includes('national_switch')) && switchAvailableFor(requester.id, row.currency);
  // Under the perimeter no method that would put funds in BitriPay's hands is offered (card, mobile money or bank to a platform account, wallet): the switch is the only rail.
  if (aggregatorPerimeterApplied()) methods = viaSwitch ? ['national_switch'] : [];
  else if (viaSwitch) methods.unshift('national_switch');
  return {
