...
const toView = (r: any): Binding => ({
  ...
  eligibleForPromotionAt: new Date(Date.parse(r.shadow_since) + SHADOW_DAYS * 86_400_000).toISOString(),
});
...
export function setBindingEnabled(id: string, enabled: boolean, adminId: string): Binding {
  getBinding(id);
  getDb()
    .prepare('UPDATE agent_bindings SET enabled = ? WHERE id = ?')
    .run(enabled ? 1 : 0, id);
...
export function setBindingKillSwitch(id: string, on: boolean, adminId: string, reason?: string | null): Binding {
  getBinding(id);
  getDb()
    .prepare('UPDATE agent_bindings SET kill_switch = ? WHERE id = ?')
    .run(on ? 1 : 0, id);
...
export function promoteBinding(id: string, adminId: string, override?: string | null): Binding {
  ...
  if (!eligible && !override)
    throw badRequest(`Shadow mode runs for ${SHADOW_DAYS} days (eligible ${b.eligibleForPromotionAt.slice(0, 10)}); give an override reason to promote earlier`, 'shadow_period_active', {
      eligibleAt: b.eligibleForPromotionAt,
      runs: b.runs,
    });
...
function describeEvent(ev: DomainEvent): string {
  ...
  switch (ev.type) {
    case 'attempt.unknown':
      return `Payment attempt ${p.attemptId ?? ''} on intent ${p.intentId ?? ev.aggregateId} via ${p.connector ?? 'unknown connector'} ended UNKNOWN. Own the ambiguity window: inspect, recover with inquiries only, never re-push.`;
    case 'connector.degraded':
      return `Connector ${p.connector ?? ev.aggregateId} opened its circuit after ${p.failures ?? '?'} consecutive failures. Check health, propose a pause or reroute, and describe the incident.`;
    case 'dispute.opened':
      return `Dispute ${ev.aggregateId} opened (${p.reasonCode ?? ''}, ${p.amountMinor ?? ''} ${p.currency ?? ''}). Summarise both sides and propose a ruling for a human to confirm.`;
    case 'agent.float_low':
      return `Agent ${p.agentId ?? ev.aggregateId} float is ${p.status ?? 'low'} in ${p.currency ?? ''} (runway ${p.runwayDays ?? '?'} days). Recommend a refill within the envelope.`;
    case 'recon.exception_aged':
      return `Reconciliation case ${ev.aggregateId} (${p.class ?? ''}) is older than 24 hours. Build the evidence pack and propose a resolution for approval.`;
    case 'statement.imported':
      return `Statement ${ev.aggregateId} imported for ${p.connectionId ?? p.gatewayId ?? ''} (${p.lines ?? '?'} lines). Review the run and the exceptions it opened.`;
    case 'verification.requested':
      return `Verification ${ev.aggregateId} requested (${p.reference ?? p.msisdn ?? ''}). Confirm across the three doors and explain the result.`;
    case 'diaspora.quote_created':
      return `Diaspora-Direct quote ${ev.aggregateId} on ${p.pair ?? ''}. Check the rate card against the live mid-market rate and the signed policy.`;
    case 'sanctions.hit':
      return `Sanctions hit on account ${p.userId ?? ev.aggregateId} (${(p.hits as string[] | undefined)?.join(', ') ?? ''}). Freeze outranks everything; prepare the case for the MLRO.`;
    case 'merchant.created':
      return `New merchant ${ev.aggregateId}. Check verification level and draft the onboarding next steps.`;
    case 'transaction.fraud_scored':
      return `Movement ${ev.aggregateId} scored ${p.score ?? '?'} (${p.band ?? ''}). Explain the factors and confirm the policy decision.`;
    default:
      return `${ev.type} on ${ev.aggregateId ?? 'platform'}: ${JSON.stringify(p).slice(0, 300)}`;
  }
}
...
export async function dispatchEvent(ev: DomainEvent): Promise<{ started: string[] }> {
  ...
  for (const b of rows) {
    ...
    try {
      const run = await startRun(operator, b.agentKey, describeEvent(ev), {
        trigger: 'event',
        triggerRef: ev.eventId,
        context: { event: ev.type, aggregateId: ev.aggregateId, payload: ev.payload, autonomy: b.autonomy, registryId: b.registryId },
        readOnly: b.autonomy === 'shadow',
        wait: false,
      });
