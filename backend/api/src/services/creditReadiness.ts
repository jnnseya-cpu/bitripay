...
export function computeReadiness(userId: string): Readiness {
  ...
  const rows = db
    .prepare(
      "SELECT type, amount, fee, currency, sender_user_id, receiver_user_id, created_at FROM transactions WHERE (sender_user_id = ? OR receiver_user_id = ?) AND status = 'completed' AND created_at >= ?",
    )
  ...
  for (const t of rows) {
    ...
    if (t.receiver_user_id === userId && t.sender_user_id !== userId && INCOME_TYPES.has(t.type)) {
      income += base;
      monthly[m].income += base;
    }
    if (t.sender_user_id === userId && t.receiver_user_id !== userId && SPEND_TYPES.has(t.type)) {
      spend += base + toBase(t.fee ?? 0, t.currency);
      monthly[m].spend += base;
      if (t.type === 'bill_payment' || t.type === 'subscription' || t.type === 'mobile_topup') bills += 1;
    }
  }
  ...
  const saved = db
    .prepare(
      "SELECT COALESCE(SUM(CASE WHEN kind IN ('anchor', 'round_up', 'manual') THEN amount_minor ELSE 0 END), 0) s, COALESCE(SUM(CASE WHEN kind = 'withdrawal' THEN -amount_minor ELSE 0 END), 0) w FROM savings_movements WHERE user_id = ? AND created_at >= ?",
    )
    .get(userId, since) as any;
  const balances = db
    .prepare('SELECT e.balance_after, w.currency FROM ledger_entries e JOIN wallets w ON w.id = e.wallet_id WHERE w.user_id = ? AND e.created_at >= ? ORDER BY e.created_at')
  ...
  const tips: string[] = [];
  const add = (key: string, label: string, value: number | string | null, points: number, max: number, note: string, tip?: string) => {
    signals.push({ key, label, value, points: Math.max(0, Math.min(max, Math.round(points))), max, note });
    if (tip && points < max * 0.6) tips.push(tip);
  };
  ...
  add(
    ...
    regularity * 200,
    200,
    `${monthsWithIncome} of the last ${elapsedMonths} month(s) had money coming in`,
    'Receive your income into BitriPay every month: regular inflows are the strongest signal a lender looks for.',
  );
  ...
  add(
    ...
    income > 0 ? Math.round(ratio * 100) : null,
    disciplinePts,
    180,
    income > 0 ? `You spent ${Math.round(ratio * 100)}% of what you received` : 'No income in the window yet',
    'Keep spending under 80% of income; the wellbeing monitor shows where to trim.',
  );
  ...
  add(
    'savings',
    'Savings behaviour',
    income > 0 ? Math.round(savingsRate * 100) : null,
    savingsRate >= 0.15 ? 150 : savingsRate >= 0.1 ? 120 : savingsRate > 0 ? 70 : 0,
    150,
    netSaved > 0 ? `${Math.round(savingsRate * 100)}% of income set aside in goals` : 'Nothing set aside yet',
    'Turn on the 10% anchor and round-ups; steady saving weighs more than large one-off deposits.',
  );
  ...
  add(
    'balance_stability',
    'Balance stability',
    avgBal > 0 ? Math.round(avgBal) : 0,
    stabilityPts,
    150,
    balBase.length ? `Lowest balance ${minBal <= 0 ? 'hit zero' : 'stayed above zero'}; average ${Math.round(avgBal)} ${base.code} minor units` : 'No ledger history yet',
    'Keep a buffer: an account that never empties reads as dependable.',
  );
  ...
  add(
    ...
    agePts + kycPts,
    120,
    `${ageDays} day(s) old, KYC tier ${tier}${user.kyc_status === 'verified' ? ' (verified)' : ''}`,
    'Complete a higher KYC tier: verified identity and address lift the ceiling of what a lender can offer.',
  );
  ...
  add(
    ...
    bills,
    Math.min(100, bills * 12),
    100,
    `${bills} bill, subscription or top-up payment(s) in the window`,
    'Pay bills and subscriptions from BitriPay so your repayment habit is visible.',
  );
  ...
  add(
    'verified_income',
    'Verified income (linked bank)',
    ext?.monthly_income_base ?? null,
    extPts,
    100,
    ext && ext.confidence !== 'none' ? `Regular income verified from ${ext.months_covered} month(s) of bank statements (${ext.confidence} confidence)` : 'No linked bank account or statement yet',
    'Link your bank account or import a statement so a lender can see your income is regular.',
  );
  const score = Math.min(
    1000,
    signals.reduce((a, s) => a + s.points, 0),
  );
  ...
  db.prepare(
    'INSERT INTO credit_readiness (user_id, score, band, signals, tips, computed_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET score = excluded.score, band = excluded.band, signals = excluded.signals, tips = excluded.tips, computed_at = excluded.computed_at',
  ).run(userId, score, readiness.band, JSON.stringify(signals), JSON.stringify(tips), readiness.computedAt);
...
export function runReadinessBatch(): { computed: number } {
  ...
  const ids = getDb()
    .prepare(
      "SELECT DISTINCT u.id FROM users u JOIN transactions t ON (t.sender_user_id = u.id OR t.receiver_user_id = u.id) WHERE u.is_system = 0 AND u.role IN ('user', 'merchant', 'agent') AND t.created_at >= ?",
    )
  ...
  for (const { id } of ids) {
    try {
      ...
      console.warn(`[credit] ${id}: ${(err as Error).message}`);
    }
  }
  return { computed };
}
...
const consentView = (r: any): Consent => ({
  id: r.id,
...
export function grantConsent(user: UserRow, input: { lenderName: string; purpose?: string | null; days?: number | null }, actor: Actor): Consent {
  ...
  getDb()
    .prepare('INSERT INTO credit_consents (id, user_id, lender_name, purpose, access_code, granted_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(id, user.id, input.lenderName.trim(), input.purpose ?? null, code, now(), new Date(Date.now() + days * 86_400_000).toISOString());
...
export function listConsents(userId: string): Consent[] {
  return (getDb().prepare('SELECT * FROM credit_consents WHERE user_id = ? ORDER BY granted_at DESC').all(userId) as any[]).map(consentView);
}
...
export function readinessForLender(accessCode: string, lender: { id: string; label: string }) {
  ...
  return {
    consent: { id: c.id, lender: c.lender_name, purpose: c.purpose, expiresAt: c.expires_at },
    subject: { reference: u.tag, country: u.country, kycTier: Number((u as any).kyc_tier ?? 0), accountAgeDays: Math.floor((Date.now() - Date.parse(u.created_at)) / 86_400_000) },
    score: r.score,
    band: r.band,
    signals: r.signals.map((s) => ({ key: s.key, label: s.label, points: s.points, max: s.max, note: s.note })),
    computedAt: r.computedAt,
    disclaimer: "A readiness signal computed from the account's own BitriPay history. It is not a credit decision and BitriPay does not lend.",
  };
}
