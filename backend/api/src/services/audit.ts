...
export function audit(adminId: string, action: string, targetType?: string, targetId?: string, details: Record<string, unknown> = {}) {
  getDb()
    .prepare('INSERT INTO audit_logs (id, admin_id, action, target_type, target_id, details, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(uuid(), adminId, action, targetType ?? null, targetId ?? null, JSON.stringify(details), now());
}
