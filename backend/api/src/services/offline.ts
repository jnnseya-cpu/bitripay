...
export function registerOfflineDevice(user: UserRow, input: { deviceId: string; publicKey: string; label?: string | null }): OfflineDevice {
  ...
  if (existing) db.prepare('UPDATE offline_devices SET key_id = ?, label = COALESCE(?, label) WHERE device_id = ?').run(key.keyId, input.label ?? null, input.deviceId);
    ...
    db.prepare('INSERT INTO offline_devices (device_id, user_id, key_id, label, last_counter, registered_at) VALUES (?, ?, ?, ?, 0, ?)').run(
      input.deviceId,
      user.id,
      key.keyId,
      input.label ?? null,
...
export function offlineQr(
  ...
  const payload = bitriqr.encodeSigned(
    {
      mode: 'dynamic',
      merchantId: merchantCode(merchant),
      merchantName: merchant.business_name || merchant.full_name,
      country: merchant.country ?? 'CD',
      currency: cur.code,
      amount: (input.amountMinor / 10 ** cur.decimals).toFixed(cur.decimals),
      rails: ['wallet'],
      intentRef: null,
      keyId: key.keyId,
      expiresAt,
...
export async function settlePromise(submitter: UserRow, p: OfflinePromiseInput): Promise<SyncOutcome> {
  const db = getDb();
  const s = getOfflineSettings();
  const canonical = promiseCanonical({
  ...
  const reject = (reason: string, extra: Record<string, unknown> = {}): SyncOutcome => {
    db.prepare(
      'INSERT INTO offline_promises (intent_hash, merchant_user_id, payer_user_id, payer_device_id, merchant_key_id, payer_key_id, amount_minor, currency, nonce, reference, merchant_sig, payer_sig, payer_device_counter, promised_at, expires_at, submitted_by, sync_state, reject_reason, synced_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      hash,
      p.merchantId,
      p.payerId,
  ...
  if ((merchantKey.scope === 'MERCHANT' && merchantKey.partyId !== merchant.id) || (merchantKey.scope === 'DEVICE_OFFLINE' && !merchantKey.partyId.startsWith(`${merchant.id}:`)))
    return reject('merchant_signature_invalid');
    ...
    try {
      ...
      const v = await Promise.resolve(
        bitriqr.verify(
          d,
          (payload, sig) =>
            merchantKey.scope === 'DEVICE_OFFLINE' ? verifyDeviceSigAt(p.merchantKeyId, payload, Buffer.from(sig).toString('base64'), p.promisedAt) : verifyWithKey(p.merchantKeyId, payload, sig),
          Math.floor(Date.parse(p.promisedAt) / 1000),
        ),
      );
      const qrMinor = d.amount ? Math.round(parseFloat(d.amount) * 10 ** cur.decimals) : 0;
      merchantOk =
        v.trust === 'verified' &&
        d.keyId === p.merchantKeyId &&
        d.offlineNonce === p.nonce &&
        qrMinor === p.amountMinor &&
        d.currency.toUpperCase() === cur.code &&
        d.merchantId === merchantCode(merchant) &&
        (d.expiresAt ?? 0) * 1000 >= Date.parse(p.expiresAt) - 1000;
    } catch {
      merchantOk = false;
    }
  } else {
  ...
  const outstanding = (
    db
      .prepare("SELECT COALESCE(SUM(amount_minor), 0) s FROM offline_promises WHERE payer_device_id = ? AND sync_state = 'SETTLED' AND synced_at >= ?")
      .get(p.payerDeviceId, new Date(Date.now() - 86_400_000).toISOString()) as any
  ).s as number;
  ...
  const risk = assessRisk({
    ...
    currency: cur.code,
    subjectType: 'offline_promise',
    subjectId: hash,
    counterparty: { name: merchant.full_name, phone: merchant.phone, email: merchant.email, country: merchant.country },
    method: 'offline',
  ...
  try {
    ...
    tx = db.transaction(() => {
      const t = postTransaction({
        ...
        note: p.reference ?? `Offline payment to ${merchant.business_name || merchant.full_name}`,
  ...
  db.prepare(
    'INSERT INTO offline_promises (intent_hash, merchant_user_id, payer_user_id, payer_device_id, merchant_key_id, payer_key_id, amount_minor, currency, nonce, reference, merchant_sig, payer_sig, payer_device_counter, promised_at, expires_at, submitted_by, sync_state, transaction_id, receipt_sig, synced_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ).run(
    hash,
    merchant.id,
    payer.id,
  ...
  recordEvent(
    'payment',
    hash,
    'offline.promise_settled',
    { type: 'system' },
    { transactionId: tx.id, merchantId: merchant.id, payerId: payer.id, amount: p.amountMinor, currency: cur.code, counter: p.counter },
  );
  emitEvent(
    merchant.id,
    'payment.completed',
    { transaction: { id: tx.id, amount: p.amountMinor, currency: cur.code, method: 'offline', promiseHash: hash } },
    { resource: { type: 'transaction', id: tx.id } },
  );
  notify(merchant.id, 'Offline payment confirmed', `${p.amountMinor / 10 ** cur.decimals} ${cur.code} from ${payer.full_name} is now in your wallet.`, {
...
export async function syncPromises(submitter: UserRow, items: OfflinePromiseInput[]): Promise<{ results: SyncOutcome[]; settled: number; rejected: number; duplicates: number }> {
  ...
  return {
    results,
    settled: results.filter((r) => r.state === 'SETTLED').length,
    rejected: results.filter((r) => r.state === 'REJECTED').length,
    duplicates: results.filter((r) => r.state === 'DUPLICATE').length,
  };
}
...
export function listPromises(userId: string, filter: { state?: string | null; limit?: number } = {}) {
  return (
    getDb()
      .prepare(`SELECT * FROM offline_promises WHERE (merchant_user_id = ? OR payer_user_id = ?) ${filter.state ? 'AND sync_state = ?' : ''} ORDER BY created_at DESC LIMIT ?`)
      .all(...(filter.state ? [userId, userId, filter.state] : [userId, userId]), Math.min(500, filter.limit ?? 100)) as any[]
  ).map((r) => ({
    hash: r.intent_hash,
    merchantId: r.merchant_user_id,
    payerId: r.payer_user_id,
    deviceId: r.payer_device_id,
    amountMinor: r.amount_minor,
    currency: r.currency,
    reference: r.reference,
    counter: r.payer_device_counter,
...
export function purgeOfflineNonces(): number {
  return getDb()
    .prepare('DELETE FROM offline_nonces WHERE expires_at < ? AND used_at IS NULL')
    .run(new Date(Date.now() - 24 * 3600_000).toISOString()).changes;
}
