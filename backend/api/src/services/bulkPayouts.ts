...
export function rowFromCsv(rec: Record<string, string>, currency: string): BatchRowInput {
  ...
  if (method === 'bank') {
    ...
    return {
      amountMinor,
      destination: { method: 'bank', bankName: rec.bank_name, accountName: rec.account_name || name || '', accountNumber: rec.account_number, country: rec.country || null, swift: rec.swift || null },
      reference,
      name,
    };
  }
...
function validateRow(
  ...
  row: BatchRowInput,
  currency: string,
): { ok: true; destination: BatchDestination; fee: number; method: string; name: string | null } | { ok: false; error: string; method: string } {
  ...
  try {
    if (d.method === 'wallet') {
      ...
      return {
        ...
        fee: type === 'transfer' ? calculateFee('transfer', row.amountMinor, currency, null, { userId: user.id }) : 0,
        method,
        name: row.name ?? r.full_name,
      };
    }
    if (d.method === 'mobile_money') {
      ...
      if (d.operatorId) op = getOperator(d.operatorId);
        ...
        if (cands.length !== 1)
          return {
            ok: false,
            error: cands.length === 0 ? 'operator_id is required (no operator matches this number)' : `operator_id is required (${cands.map((c) => c.id).join(', ')} all match)`,
            method,
          };
        op = cands[0];
      }
      ...
      return {
        ok: true,
        destination: { method: 'mobile_money', operatorId: op.id, phone, name: row.name ?? d.name ?? null },
        fee: calculateFee('withdrawal', row.amountMinor, currency, null, { userId: user.id }),
        method,
    ...
    if (d.method === 'bank') {
      if ('bankAccountId' in d && d.bankAccountId) {
        ...
        return {
          ok: true,
          destination: { method: 'bank', bankAccountId: d.bankAccountId },
          fee: calculateFee('withdrawal', row.amountMinor, currency, null, { userId: user.id }),
          method,
          name: row.name ?? bank.account_name,
        };
      }
      ...
      return {
        ok: true,
        destination: { method: 'bank', bankName: d.bankName, accountName: d.accountName, accountNumber: d.accountNumber, country: d.country ?? null, swift: d.swift ?? null },
        fee: calculateFee('withdrawal', row.amountMinor, currency, null, { userId: user.id }),
        method,
        name: row.name ?? d.accountName,
      };
    }
...
function batchView(b: any, withRows = false): BatchView {
  const v: BatchView = {
    ...
    paidRows: b.paid_rows,
    failedRows: b.failed_rows,
    totalMinor: b.total_minor,
    feeMinor: b.fee_minor,
    paidMinor: b.paid_minor,
    skipInvalid: !!b.skip_invalid,
    createdBy: b.created_by,
    approvedBy: b.approved_by,
    approvedAt: b.approved_at,
    approvalMethod: b.approval_method,
...
export function createBatch(
  ...
  if (input.csv) {
    ...
    recs.forEach((rec, i) => {
      try {
        rows.push({ input: rowFromCsv(rec, cur.code), error: null, lineNo: i + 1 });
      } catch (err) {
        rows.push({
          input: { amountMinor: 0, destination: { method: (rec.method || 'unknown') as any, to: '' }, reference: rec.reference || null, name: rec.name || null },
  ...
  db.transaction(() => {
    db.prepare(
      'INSERT INTO payout_batches (id, user_id, reference, note, currency, status, skip_invalid, created_by, created_via, idempotency_key, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      id,
      user.id,
    ...
    const ins = db.prepare(
      'INSERT INTO payout_batch_rows (id, batch_id, line_no, method, destination, amount_minor, fee_minor, reference, name, status, error, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    );
    for (const r of rows) {
      ...
      if (v.ok) {
        ...
        invalid += 1;
        ins.run(
          `pr_${shortCode(12).toLowerCase()}`,
          id,
          r.lineNo,
          v.method,
          JSON.stringify(r.input?.destination ?? {}),
          r.input?.amountMinor ?? 0,
          0,
          r.input?.reference ?? null,
          r.input?.name ?? null,
          'INVALID',
          v.error,
          now(),
    ...
    db.prepare('UPDATE payout_batches SET row_count = ?, valid_rows = ?, invalid_rows = ?, total_minor = ?, fee_minor = ?, updated_at = ? WHERE id = ?').run(
      rows.length,
      valid,
      invalid,
      total,
...
export function batchReadiness(user: UserRow, id: string) {
  ...
  return {
    ...
    available,
    needed,
    shortfallMinor: Math.max(0, needed - available),
    invalidRows: b.invalid_rows,
    blockedByInvalidRows: b.invalid_rows > 0 && !b.skip_invalid,
    fourEyes: 'a different account holder approves without step-up; the creator approves with PIN or passkey',
  };
}
...
export function approveBatch(user: UserRow, id: string, opts: { stepUpVerified: boolean; approverId?: string | null; deviceHash?: string | null; ipCountry?: string | null }, actor: Actor): BatchView {
  ...
  if (b.invalid_rows > 0 && !b.skip_invalid)
    throw unprocessable(`${b.invalid_rows} row(s) are invalid; fix the file or create the batch with skip_invalid`, 'batch_has_invalid_rows', { invalidRows: b.invalid_rows });
  ...
  if (readiness.shortfallMinor > 0)
    throw unprocessable(`Insufficient available balance: ${formatMoney(readiness.shortfallMinor, getCurrency(b.currency))} short`, 'insufficient_funds', {
      available: readiness.available,
      needed: readiness.needed,
    });
  db.prepare("UPDATE payout_batches SET status = 'EXECUTING', approved_by = ?, approved_at = ?, approval_method = ?, updated_at = ? WHERE id = ?").run(
    approverId,
    now(),
    fourEyes ? 'four_eyes' : 'step_up',
    now(),
    id,
  );
    ...
    try {
      ...
      if (dest.method === 'wallet') {
        const tx = sendMoney(user, {
          ...
          currency: b.currency,
          note: r.reference ?? b.reference ?? `Batch ${id}`,
          idempotencyKey: `batch:${r.id}`,
          stepUpVerified: true,
        ...
        if (existing) txId = existing.id;
          ...
          const tx = requestWithdrawal(user, {
            amount: r.amount_minor,
            currency: b.currency,
            destination: dest as WithdrawalDestination,
            note: r.reference ?? b.reference ?? `Batch ${id}`,
            stepUpVerified: true,
            deviceHash: opts.deviceHash ?? null,
            ipCountry: opts.ipCountry ?? null,
  ...
  db.prepare('UPDATE payout_batches SET status = ?, paid_rows = ?, failed_rows = ?, paid_minor = ?, executed_at = ?, updated_at = ? WHERE id = ?').run(
    status,
    already.c,
    failed,
    already.s,
  ...
  notify(
    user.id,
    status === 'EXECUTED' ? 'Batch paid' : status === 'PARTIAL' ? 'Batch partly paid' : 'Batch failed',
    `${already.c} of ${b.row_count} rows paid (${formatMoney(already.s, getCurrency(b.currency))})${failed ? `; ${failed} failed — open the batch to see why` : ''}.`,
    { kind: 'withdrawal', batchId: id },
  );
  return view;
}
...
export function listBatches(userId: string, filter: { status?: string | null; limit?: number } = {}): BatchView[] {
  return (
    getDb()
      .prepare(`SELECT * FROM payout_batches WHERE user_id = ? ${filter.status ? 'AND status = ?' : ''} ORDER BY created_at DESC LIMIT ?`)
      .all(...(filter.status ? [userId, filter.status] : [userId]), Math.min(200, filter.limit ?? 50)) as any[]
  ).map((b) => batchView(b, false));
}
/** Admin view across merchants. */
export function listAllBatches(filter: { status?: string | null; limit?: number } = {}) {
  return (
    getDb()
      .prepare(`SELECT b.*, u.full_name, u.business_name FROM payout_batches b JOIN users u ON u.id = b.user_id ${filter.status ? 'WHERE b.status = ?' : ''} ORDER BY b.created_at DESC LIMIT ?`)
      .all(...(filter.status ? [filter.status] : []), Math.min(500, filter.limit ?? 100)) as any[]
  ).map((b) => ({ ...batchView(b, false), owner: { id: b.user_id, name: b.business_name || b.full_name, userId: findUserById(b.user_id)?.id ?? b.user_id } }));
}
