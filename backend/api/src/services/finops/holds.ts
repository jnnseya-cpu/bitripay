...
export function createHold(
  input: { walletId: string; amountMinor: number; kind: HoldKind; refType?: string | null; refId?: string | null; reason?: string | null; expiresAt?: string | null },
  actor: Actor,
): HoldView {
  ...
  getDb()
    .prepare(
      'INSERT INTO holds (id, wallet_id, user_id, amount_minor, currency, kind, ref_type, ref_id, reason, status, created_by, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
    .run(
      id,
      wallet.id,
...
export function releaseHold(id: string, actor: Actor, reason?: string | null): HoldView {
  ...
  getDb()
    .prepare("UPDATE holds SET status = 'RELEASED', released_by = ?, released_at = ? WHERE id = ?")
...
export function heldAmount(walletId: string, kind?: HoldKind | null): number {
  return (
    getDb()
      .prepare(`SELECT COALESCE(SUM(amount_minor), 0) s FROM holds WHERE wallet_id = ? AND status = 'ACTIVE' ${kind ? 'AND kind = ?' : ''}`)
      .get(...(kind ? [walletId, kind] : [walletId])) as any
  ).s as number;
}
...
export function listHolds(filter: { userId?: string | null; walletId?: string | null; status?: string | null; limit?: number } = {}): HoldView[] {
  ...
  return (
    getDb()
      .prepare(`SELECT * FROM holds ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY created_at DESC LIMIT ?`)
