...
export function upsertProfile(
  ...
  if (existing) {
    ...
    db.prepare('UPDATE settlement_profiles SET schedule = ?, cutoff_hour_utc = ?, destination = ?, min_amount = ?, auto = ?, active = ?, updated_at = ? WHERE id = ?').run(
      input.schedule,
      cutoff,
  ...
  db.prepare(
    'INSERT INTO settlement_profiles (id, user_id, rail, currency, schedule, cutoff_hour_utc, destination, min_amount, auto, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ).run(id, user.id, rail, cur.code, input.schedule, cutoff, JSON.stringify(input.destination ?? {}), input.minAmount ?? 0, (input.auto ?? true) ? 1 : 0, (input.active ?? true) ? 1 : 0, now(), now());
...
export function closeCycle(
  ...
  const collections = db
    .prepare(
      "SELECT * FROM transactions WHERE receiver_user_id = ? AND currency = ? AND type IN ('merchant_payment', 'qr_payment') AND status IN ('completed', 'reversed') AND completed_at > ? AND completed_at <= ? AND id NOT IN (SELECT transaction_id FROM settlement_items)",
    )
  ...
  db.transaction(() => {
    db.prepare(
      'INSERT INTO settlement_cycles (id, user_id, profile_id, currency, rail, period_from, period_to, business_date, status, gross_minor, fees_minor, refunds_minor, splits_minor, holds_minor, net_minor, item_count, due_at, closed_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      id,
      userId,
      profile?.id ?? null,
      ...
      holds,
      net,
      inRail.length + refunds.length + splits.length,
  ...
  if (rows.length)
    doc.table(
      [
        ...
        { title: 'Amount', width: 85, align: 'right' as const },
        { title: 'Fee', width: 82, align: 'right' as const },
      ],
      rows,
      { zebra: true },
    );
