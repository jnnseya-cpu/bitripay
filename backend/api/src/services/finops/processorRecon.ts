...
export function importProcessorStatement(
  gatewayId: string,
  input: { source: 'PROCESSOR' | 'BANK'; cycleRef: string; currency: string; lines: ProcessorLine[]; controlTotalMinor?: number | string | null; periodFrom?: string | null; periodTo?: string | null },
  ...
  db.transaction(() => {
    db.prepare(
      'INSERT INTO reconciliation_imports (id, connection_id, source, period_from, period_to, cycle_ref, currency, checksum, line_count, control_total, imported_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      id,
      connectionId,
      input.source,
      input.periodFrom ?? `${input.cycleRef}T00:00:00.000Z`,
      input.periodTo ?? `${input.cycleRef}T23:59:59.999Z`,
      input.cycleRef,
      currency,
      checksum,
      input.lines.length,
    ...
    const ins = db.prepare(
      'INSERT INTO reconciliation_lines (id, import_id, external_reference, amount_minor, currency, status, fee_minor, settlement_ref, occurred_at, raw) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    );
  ...
  recordEvent(
    'reconciliation',
    id,
    'processor_statement.imported',
    { type: importerId ? 'admin' : 'system', id: importerId },
    { gatewayId, source: input.source, cycleRef: input.cycleRef, lines: input.lines.length, total, proofRef },
  );
    ...
    for (const l of db.prepare('SELECT * FROM reconciliation_lines WHERE import_id = ?').all(imp.id) as any[]) {
      ...
      if (!gp) {
        openCase({
          ...
          currency: l.currency,
          references: { reference: l.external_reference, source: imp.source, gatewayId },
          sources: [imp.source],
        });
        cases += 1;
        continue;
      ...
      if (l.currency !== gp.currency) {
        openCase({
          ...
          runId,
          cycleRef,
          exposureMinor: gp.amount,
          currency: gp.currency,
          references: { reference: l.external_reference, local: gp.currency, external: l.currency },
          sources: ['BITRIPAY', imp.source],
          merchantUserId: gp.user_id,
        });
        cases += 1;
        clean = false;
        ...
        openCase({
          ...
          runId,
          cycleRef,
          exposureMinor: Math.abs(l.amount_minor - gp.amount),
          currency: gp.currency,
          references: { reference: l.external_reference, local: gp.amount, external: l.amount_minor },
          sources: ['BITRIPAY', imp.source],
        });
        cases += 1;
        clean = false;
      ...
      if ((extOk && !['SETTLED', 'DISPUTED'].includes(gp.stage)) || (extBad && gp.stage === 'SETTLED')) {
        openCase({
          ...
          exposureMinor: gp.amount,
          currency: gp.currency,
          references: { reference: l.external_reference, local: gp.stage, external: l.status },
          sources: ['BITRIPAY', imp.source],
        });
        cases += 1;
        clean = false;
      ...
      if (gp.stage === 'SETTLED') {
        ...
        if (!tx || tx.status !== 'completed' || tx.amount !== gp.amount) {
          openCase({
            ...
            exposureMinor: gp.amount,
            currency: gp.currency,
            references: { reference: l.external_reference, ledger: tx ? { amount: tx.amount, status: tx.status } : null },
            sources: ['BITRIPAY'],
          });
          cases += 1;
          clean = false;
      ...
      if (l.fee_minor != null) {
        const expectedFee = calculateFee(
          gp.purpose === 'checkout' ? 'merchant_payment' : gp.method === 'card' ? 'card_deposit' : gp.method === 'mobile_money' ? 'mobile_money_deposit' : 'bank_deposit',
          gp.amount,
          gp.currency,
        );
        ...
        if (Math.abs(l.fee_minor - expectedFee) > tolerance && Math.abs(l.fee_minor - gp.fee) > tolerance) {
          openCase({
            ...
            runId,
            cycleRef,
            exposureMinor: Math.abs(l.fee_minor - gp.fee),
            currency: gp.currency,
            references: { reference: l.external_reference, expected: gp.fee, observed: l.fee_minor },
            sources: ['BITRIPAY', imp.source],
          });
          cases += 1;
        }
  ...
  if (present.length) {
    ...
    for (const gp of locals) {
      ...
      openCase({
        ...
        exposureMinor: gp.amount,
        currency: gp.currency,
        references: { providerRef: gp.provider_ref },
        sources: ['BITRIPAY'],
        merchantUserId: gp.user_id,
      });
      cases += 1;
...
export function workbenchSummary() {
  ...
  const byConn = db
    .prepare(
      "SELECT connection_id, class, currency, COUNT(*) n, COALESCE(SUM(exposure_minor), 0) exposure, MIN(created_at) oldest FROM reconciliation_cases WHERE status != 'CLOSED' GROUP BY connection_id, class, currency ORDER BY exposure DESC",
    )
    .all() as any[];
  const runs = db.prepare('SELECT * FROM reconciliation_runs ORDER BY created_at DESC LIMIT 20').all() as any[];
  return {
    exceptions: byConn.map((r) => ({
      connectionId: r.connection_id,
      class: r.class,
      currency: r.currency,
      count: r.n,
      exposureMinor: r.exposure,
      oldestAgeHours: Math.floor((Date.now() - Date.parse(r.oldest)) / 3600_000),
    })),
