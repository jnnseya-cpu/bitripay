...
const toView = (r: any): CommissionEntry => ({ id: r.id, agentUserId: r.agent_user_id, transactionId: r.transaction_id, kind: r.kind, amountMinor: r.amount_minor, platformShareMinor: r.platform_share_minor, currency: r.currency, status: r.status, period: r.period, metadata: parseJson(r.metadata, {}), createdAt: r.created_at });

export function recordCommission(input: { agentUserId: string; transactionId?: string | null; kind: CommissionKind; amountMinor: number; currency: string; status?: CommissionEntry['status']; metadata?: Record<string, unknown> }): CommissionEntry {
  ...
  getDb().prepare('INSERT INTO commission_entries (id, agent_user_id, transaction_id, kind, amount_minor, platform_share_minor, currency, status, period, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, input.agentUserId, input.transactionId ?? null, input.kind, input.amountMinor, share, input.currency, input.status ?? 'CREDITED', now().slice(0, 7), JSON.stringify(input.metadata ?? {}), now());
...
export function listCommissions(agentUserId: string, filter: { period?: string | null; limit?: number } = {}): CommissionEntry[] {
  return (getDb().prepare(`SELECT * FROM commission_entries WHERE agent_user_id = ? ${filter.period ? 'AND period = ?' : ''} ORDER BY created_at DESC LIMIT ?`).all(...(filter.period ? [agentUserId, filter.period] : [agentUserId]), Math.min(500, filter.limit ?? 100)) as any[]).map(toView);
}
...
export function commissionOverview(period = now().slice(0, 7)) {
  const rows = getDb().prepare('SELECT agent_user_id, currency, kind, COUNT(*) n, SUM(amount_minor) s, SUM(platform_share_minor) p FROM commission_entries WHERE period = ? GROUP BY agent_user_id, currency, kind').all(period) as any[];
