...
function toView(r: any): RouteView {
  ...
  return {
    ...
    payout: r.payout_id
      ? (() => {
          try {
            return getPayout(r.payout_id, false);
          } catch {
            return null;
          }
        })()
    ...
    payment: r.payment_id
      ? (() => {
          try {
            return toPaymentView(getPayment(r.payment_id));
          } catch {
            return null;
          }
        })()
...
function setRoute(id: string, fields: Record<string, unknown>) {
  ...
  getDb()
    .prepare(`UPDATE money_routes SET ${keys.map((k) => `${k} = ?`).join(', ')}, updated_at = ? WHERE id = ?`)
...
export function previewDestination(dest: RouteDestination) {
  switch (dest.method) {
    ...
    case 'qr': {
      ...
      return {
        label: `${u.business_name || u.full_name} (@${u.tag})`,
        user: toPublicUser(u),
 ...
 */
export function executeDestination(
  user: UserRow,
  ...
  quoteId?: string | null,
  routeId?: string | null,
): { transactionId: string | null; status: string; extra?: Record<string, unknown> } {
  ...
  const targetUser =
    dest.method === 'wallet'
      ? findUserByIdentifier(dest.to)
      : dest.method === 'qr'
        ? (() => {
            const p = decodeQr(dest.data);
            return p?.type === 'pr' ? getUserById(getPaymentRequestByCode(p.id).requester_user_id) : p ? findUserByTag(p.id) : null;
          })()
        : null;
  ...
  switch (dest.method) {
    ...
    case 'bank': {
      const tx = requestWithdrawal(user, {
        amount: payAmount,
        currency: payCurrency,
        destination: dest.bankAccountId
          ? { method: 'bank', bankAccountId: dest.bankAccountId }
          : { method: 'bank', bankName: dest.bankName ?? '', accountName: dest.accountName ?? '', accountNumber: dest.accountNumber ?? '', country: dest.country ?? null },
        note,
        routeId,
    ...
    case 'mobile_money': {
      const tx = requestWithdrawal(user, {
        amount: payAmount,
        currency: payCurrency,
        destination: { method: 'mobile_money', operatorId: dest.operatorId, phone: dest.phone, name: dest.name ?? null },
        note,
        routeId,
        sourceCurrency: currency,
      });
...
export function payoutCurrencyOptions(dest: RouteDestination | undefined, sourceCurrency: string, amount: number, ctx: { requested?: string | null } = {}): PayoutCurrencyOptions | null {
  ...
  if (dest.method === 'wallet') {
    ...
    return {
      defaultCurrency: held.includes(sourceCurrency) || !local ? sourceCurrency : local,
      localCurrency: local,
      options: candidates.map((c) => ({ currency: c, available: true, isLocal: c === local, reasons: [], warnings: [], consentRequired: false, liquidityAvailable: null })),
    };
  }
  ...
  const options: PayoutCurrencyOption[] = candidates.map((cur) => {
    ...
    if (!usable.some((a) => a.balance >= needed))
      (isLocal ? warnings : reasons).push(
        liquidity > 0
          ? `Prefunded ${cur} liquidity is insufficient for this amount right now${isLocal ? '; the transfer will wait for liquidity' : ''}`
          : `No prefunded ${cur} payout account for ${country}${operatorId ? ` (${operatorId})` : ''}${isLocal ? '; the transfer will wait for liquidity' : ''}`,
      );
    return {
      currency: cur,
      ...
      reasons,
      warnings,
      consentRequired: !isLocal && !!(corridor?.beneficiaryConsent || local?.beneficiaryConsent),
...
export function quoteRoute(
  ...
  currency: string,
  targetCurrency: string,
  sourceMethod: RouteSource['method'] = 'wallet',
  dest?: RouteDestination,
  ctx: { userId?: string | null; country?: string | null; operatorId?: string | null; gateway?: string | null; persistQuote?: boolean } = {},
): RouteQuote {
  ...
  const declaration = describeRoute(sourceMethod, destKind as any, {
    ...
    destinationOperatorId: dest?.method === 'mobile_money' ? dest.operatorId : null,
    gateway: ctx.gateway,
    payoutConfirmation: corridor?.payoutConfirmation ?? null,
  });
  const compliance = getComplianceSettings();
  ...
  return {
    ...
    currency: c.code,
    senderAmount: amount,
    recipientAmount: delivered,
    fundingFee,
    cardFee: sourceMethod === 'card' ? fundingFee : 0,
    exchangeFee,
    payoutFee: converted - delivered,
    platformFee: exchangeFee + (converted - delivered) + (sourceMethod === 'card' ? 0 : fundingFee),
    rate,
    targetAmount: delivered,
    targetCurrency: t.code,
    fx,
    declaration,
    ...
    corridor: corridor
      ? { id: corridor.id, status: corridor.status, destCountry: corridor.destCountry, operatorId: corridor.operatorId, estimatedPayoutMinutes: corridor.estimatedPayoutMinutes }
      : null,
    ...
    const api = op
      ? candidates.find(
          (g) =>
            ...
            g.provider !== 'sandbox' &&
            (g.countries.length === 0 || g.countries.includes(op.country)) &&
            (g.currencies.length === 0 || g.currencies.includes(op.currency)),
        )
      : candidates.find((g) => g.provider !== 'manual_momo');
...
export async function createRoute(
  ...
  const quote = quoteRoute(input.amount, cur.code, target.code, input.source.method, input.destination, {
    userId: user.id,
  ...
  if (quote.receivingCurrencies) {
    ...
    if (!opt || !opt.available)
      throw unprocessable(`${target.code} cannot be paid out for this destination right now: ${(opt?.reasons ?? ['not offered in this corridor']).join('; ')}`, 'payout_currency_unavailable', {
        options: quote.receivingCurrencies,
      });
  }
  ...
  getDb()
    .prepare(
      'INSERT INTO money_routes (id, user_id, source_method, source_details, destination_method, destination_details, amount, currency, target_currency, status, stage, quote, corridor_id, expires_at, source_of_funds, note, created_at, updated_at, consent_token, currency_options) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
    .run(
      id,
      user.id,
      input.source.method,
      JSON.stringify({ operatorId: input.source.operatorId ?? null, phone: input.source.phone ?? null, gateway: input.source.gateway ?? null, quoteId: input.quoteId ?? null }),
      input.destination.method,
      ...
      'pending',
      'CREATED',
      JSON.stringify({ ...quote, fx: quote.fx ? { ...quote.fx, quoteId: input.quoteId ?? quote.fx.quoteId } : null }),
      corridor?.id ?? null,
      ...
      now(),
      now(),
      quote.recipientConsentRequired ? randomBytes(24).toString('base64url') : null,
      quote.receivingCurrencies ? JSON.stringify(quote.receivingCurrencies) : null,
    );
  recordEvent('route', id, 'route.created', actor, {
    source: input.source.method,
    destination: input.destination.method,
    amount: input.amount,
    currency: cur.code,
...
function payoutHold(row: any, user: UserRow, amount: number): { hold: boolean; reasons: string[] } {
  ...
  const risk = assessRisk({
    ...
    subjectType: 'route',
    subjectId: row.id,
    counterparty: { name: dest.name ?? dest.accountName ?? null, phone: dest.phone ?? null, country: dest.country ?? null },
  });
...
function dispatchPayout(row: any, user: UserRow, amount: number, actor: Actor, skipHold = false) {
  ...
  if (row.consent_token && !row.consent_confirmed_at) {
    ...
    if (recipient)
      notify(recipient.id, 'Confirm your payout currency', `${toPublicUser(user).fullName} is sending you money in ${row.target_currency}. Confirm the currency to receive it.`, {
        kind: 'route_consent',
        routeId: row.id,
    ...
    notify(
      ...
      `The recipient must confirm receiving ${row.target_currency} before the payout is executed. Share the confirmation link from the transfer details.`,
      { kind: 'route', routeId: row.id },
    );
    return;
  }
  try {
    const quote = parseJson<RouteQuote | null>(row.quote, null);
    if (row.currency !== row.target_currency)
      transitionRoute(row.id, 'FX_RESERVED', actor, {
        rate: quote?.rate ?? null,
        quoteId: src.quoteId ?? quote?.fx?.quoteId ?? null,
        provider: quote?.fx?.provider ?? null,
        guaranteed: quote?.fx?.guaranteed ?? false,
    ...
    notify(
      user.id,
      stage === 'SETTLED' ? 'Money delivered' : 'Money on its way',
      `${formatMoney(amount, getCurrency(row.currency, false))} ${stage === 'SETTLED' ? 'was delivered' : stage === 'INSUFFICIENT_LIQUIDITY' ? 'is held safely while local liquidity is arranged' : 'is queued for local payout'} to ${previewDestination(dest).label}.`,
      { kind: 'route', routeId: row.id },
    );
  } catch (err) {
...
export function retryRoute(user: UserRow, id: string, destination?: RouteDestination): RouteView {
  ...
  const held = getDb()
    ...
    .get(row.payout_transaction_id ?? '') as any;
  ...
  if (fresh.stage === 'PAYOUT_ROUTED') {
    /* dispatch will transition again via instruction */
  }
...
export async function cancelRoute(user: UserRow, id: string, reason = 'Cancelled by sender'): Promise<RouteView> {
  const row = getDb().prepare('SELECT * FROM money_routes WHERE id = ? AND user_id = ?').get(id, user.id) as any;
  if (!row) throw notFound('Route not found');
  if (!ROUTE_REFUNDABLE.includes(row.stage) && row.stage !== 'EXPIRED')
    throw conflict(`Transfer is ${row.stage.toLowerCase().replace(/_/g, ' ')} and can no longer be cancelled`, 'invalid_stage_transition');
...
export async function refundRoute(routeId: string, actor: Actor, reason: string, verificationId: string | null): Promise<RouteView> {
  const row = getRouteRow(routeId);
  if (!ROUTE_REFUNDABLE.includes(row.stage) && row.stage !== 'EXPIRED' && row.stage !== 'DISPUTED')
    throw conflict(`Transfer is ${row.stage.toLowerCase().replace(/_/g, ' ')} and cannot be refunded`, 'invalid_stage_transition');
...
export function consentView(token: string) {
  ...
  return {
    routeId: row.id,
    stage: row.stage,
    sender: { name: toPublicUser(sender).fullName },
    amount: quote?.recipientAmount ?? null,
    currency: row.target_currency,
    localCurrency: options?.localCurrency ?? null,
    options: options?.options.filter((o) => o.available).map((o) => ({ currency: o.currency, isLocal: o.isLocal })) ?? [],
...
export async function refreshRoute(userId: string, id: string): Promise<RouteView> {
  ...
  if (['FUNDING_PENDING', 'BIOMETRIC_APPROVAL_REQUIRED'].includes(row.stage) && row.payment_id) {
    ...
    } else if (row.stage === 'BIOMETRIC_APPROVAL_REQUIRED' && p.stage !== 'AUTHENTICATION_REQUIRED' && p.status !== 'succeeded')
      tryTransitionRoute(id, 'FUNDING_PENDING', { type: 'system' }, { paymentId: p.id });
  }
  const fresh = getRouteRow(id);
...
export function routeReceipt(userId: string, id: string) {
  ...
  const evidence = r.payout
    ? (getDb().prepare('SELECT id, outcome, raw_hash, external_ref, created_at FROM payment_evidence WHERE payout_id = ? ORDER BY created_at').all(r.payout.id) as any[]).map((e) => ({
        id: e.id,
        outcome: e.outcome,
        hash: e.raw_hash,
        operatorReference: e.external_ref,
        at: e.created_at,
      }))
    : [];
