...
const summary = (r: any): PostSummary => ({
  id: r.id,
  slug: r.slug,
  title: r.title,
  excerpt: r.excerpt,
...
export function listPosts(opts: { status?: PostStatus | 'all'; tag?: string | null; category?: string | null; q?: string | null; language?: string | null; page?: number; pageSize?: number } = {}): {
  items: PostSummary[];
  total: number;
} {
  const where: string[] = [];
  ...
  if (opts.q) {
    where.push('(title LIKE ? OR excerpt LIKE ? OR body_md LIKE ?)');
  ...
  const total = (
    getDb()
      .prepare(`SELECT COUNT(*) c FROM blog_posts ${sql}`)
      .get(...params) as any
  ).c as number;
  const items = (
    getDb()
      .prepare(`SELECT * FROM blog_posts ${sql} ORDER BY COALESCE(published_at, updated_at) DESC LIMIT ? OFFSET ?`)
...
export function listTags(): { tag: string; count: number }[] {
  ...
  for (const r of getDb().prepare("SELECT tags FROM blog_posts WHERE status = 'published'").all() as { tags: string }[])
    for (const t of parseJson<string[]>(r.tags, [])) counts.set(t, (counts.get(t) ?? 0) + 1);
  return [...counts.entries()].map(([tag, count]) => ({ tag, count })).sort((a, b) => b.count - a.count);
}
export function getPost(idOrSlug: string, publishedOnly = true): Post {
  const r = getDb()
    .prepare(`SELECT * FROM blog_posts WHERE (id = ? OR slug = ?)${publishedOnly ? " AND status = 'published' AND (published_at IS NULL OR published_at <= ?)" : ''}`)
    .get(...(publishedOnly ? [idOrSlug, idOrSlug, now()] : [idOrSlug, idOrSlug]));
...
export function createPost(input: PostInput, actor: Actor): Post {
  ...
  getDb()
    .prepare(
      `INSERT INTO blog_posts (id, slug, title, excerpt, body_md, cover_url, cover_alt, category, tags, keywords, language, author_name, author_user_id, status, meta_title, meta_description, canonical_url, faq, sources, social, reading_minutes, source, agent_run_id, views, published_at, scheduled_for, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`,
    )
    .run(
      id,
      slug,
      input.title.trim(),
      (input.excerpt ?? '').trim() || autoExcerpt(input.bodyMd),
      input.bodyMd,
      input.coverUrl ?? null,
      input.coverAlt ?? null,
      input.category ?? 'guides',
      JSON.stringify(input.tags ?? []),
      JSON.stringify(input.keywords ?? []),
      input.language ?? 'en',
      input.authorName ?? 'BitriPay Editorial',
...
export function updatePost(id: string, input: Partial<PostInput>, actor: Actor): Post {
  ...
  if (input.bodyMd !== undefined) {
    fields.body_md = input.bodyMd;
    fields.reading_minutes = readingMinutes(input.bodyMd);
  }
  ...
  if (keys.length)
    getDb()
      .prepare(`UPDATE blog_posts SET ${keys.map((k) => `${k} = ?`).join(', ')}, updated_at = ? WHERE id = ?`)
...
function postLinkRules(excludeId: string): LinkRule[] {
  ...
  for (const r of rows)
    for (const k of parseJson<string[]>(r.keywords, []).slice(0, 3))
      rules.push({ id: `post:${r.id}:${k}`, keyword: k, url: `/blog/${r.slug}`, title: r.title, kind: 'internal', maxPerPage: 1, priority: 30, enabled: true, createdAt: '' });
  return rules;
}
