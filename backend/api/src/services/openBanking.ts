...
export interface Institution {
  ...
  country: string;
  currencies: string[];
  features: ('accounts' | 'payments' | 'vrp')[];
}
export interface OpenBankingProvider {
  ...
  completeLink(
    link: { id: string; providerRef: string | null; institutionId: string; userId: string },
    callback: Record<string, unknown>,
  ): { approved: boolean; consentToken: string | null; consentExpiresAt: string | null; accounts: LinkedAccount[]; transactions: BankTransaction[] };
  ...
  pay(
    link: { id: string; consentToken: string | null },
    account: LinkedAccount,
    amountMinor: number,
    currency: string,
    reference: string,
  ): { status: 'succeeded' | 'pending' | 'failed'; providerRef: string; failureReason?: string };
...
function sandboxStatement(linkId: string, account: LinkedAccount, from: Date, to: Date): BankTransaction[] {
  ...
  while (d <= to) {
    ...
    const push = (t: BankTransaction) => {
      if (inRange(t.bookedAt)) out.push(t);
    };
    push({
      externalId: `${y}${String(m + 1).padStart(2, '0')}-sal`,
      bookedAt: day(25),
      amountMinor: salary + Math.round((rand() - 0.5) * 0.06 * salary),
    ...
    push({
      externalId: `${y}${String(m + 1).padStart(2, '0')}-rent`,
      bookedAt: day(2),
      amountMinor: -rent,
      currency: account.currency,
      description: 'RENT STANDING ORDER',
      counterparty: 'Landlord',
      category: 'housing',
    });
    push({
      externalId: `${y}${String(m + 1).padStart(2, '0')}-util`,
      bookedAt: day(9),
      amountMinor: -Math.round((40 + rand() * 40) * scale),
      currency: account.currency,
      description: 'ELECTRICITY BILL',
      counterparty: 'Power Co',
      category: 'utilities',
    });
    for (let i = 0; i < 6; i += 1)
      push({
        externalId: `${y}${String(m + 1).padStart(2, '0')}-g${i}`,
        bookedAt: day(3 + i * 4),
        amountMinor: -Math.round((8 + rand() * 40) * scale),
        currency: account.currency,
        description: i % 2 ? 'SUPERMARKET' : 'TRANSPORT',
        counterparty: null,
        category: i % 2 ? 'groceries' : 'transport',
      });
    d.setUTCMonth(m + 1);
  }
...
const sandboxBank: OpenBankingProvider = {
  ...
  institutions(country) {
    return SANDBOX_INSTITUTIONS.filter((i) => !country || i.country === country.toUpperCase());
  },
  createLink(link) {
    return { authUrl: `${config.apiUrl}/api/open-banking/sandbox/authorise/${link.id}`, providerRef: `sbxob_${shortCode(10)}` };
  },
  completeLink(link, callback) {
    ...
    const accounts: LinkedAccount[] = inst.currencies.slice(0, 2).map((currency, i) => ({
      id: `acc_${shortCode(8).toLowerCase()}`,
      name: i === 0 ? 'Current account' : 'Savings account',
      type: i === 0 ? 'current' : 'savings',
      masked: `••••${String(Math.floor(rand() * 9000) + 1000)}`,
      currency,
      balanceMinor: Math.round((300 + rand() * 2500) * 10 ** getCurrency(currency, false).decimals),
    }));
    const to = new Date();
  ...
  createMandate() {
    return { providerRef: `sbxvrp_${shortCode(10)}` };
  },
};
const PROVIDERS: Record<string, OpenBankingProvider> = { sandbox: sandboxBank };
export function listProviders() {
  return Object.values(PROVIDERS)
    .map((p) => ({ id: p.id, name: p.name, sandbox: p.sandbox }))
    .concat([{ id: 'statement_import', name: 'Statement import (CSV)', sandbox: false }]);
}
...
export interface LinkView {
  ...
  country: string | null;
  status: 'PENDING' | 'LINKED' | 'DECLINED' | 'EXPIRED' | 'REVOKED';
  authUrl: string | null;
...
function linkView(r: any): LinkView {
  ...
  return {
    ...
    status: r.status,
    authUrl:
      r.status === 'PENDING' && r.provider !== 'statement_import'
        ? (PROVIDERS[r.provider]?.createLink({ id: r.id, userId: r.user_id, institutionId: r.institution_id, redirectUrl: '' }).authUrl ?? null)
        : null,
    consentExpiresAt: r.consent_expires_at,
    accounts: parseJson(r.accounts, []),
...
export function createLink(user: UserRow, input: { institutionId: string; redirectUrl?: string | null }): LinkView {
  ...
  getDb()
    .prepare('INSERT INTO open_banking_links (id, user_id, provider, institution_id, institution_name, country, status, provider_ref, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(id, user.id, provider.id, inst.id, inst.name, inst.country, 'PENDING', started.providerRef, now());
...
function storeTransactions(linkId: string, userId: string, rows: BankTransaction[], accountOf: (t: BankTransaction) => string): number {
  ...
  const ins = db.prepare(
    'INSERT OR IGNORE INTO open_banking_transactions (id, link_id, user_id, account_id, external_id, booked_at, amount_minor, currency, description, counterparty, category, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  );
  let n = 0;
  db.transaction(() => {
    for (const t of rows) n += ins.run(uuid(), linkId, userId, accountOf(t), t.externalId, t.bookedAt, t.amountMinor, t.currency, t.description, t.counterparty, t.category, now()).changes;
  })();
  return n;
}
...
export function completeLink(userId: string, id: string, callback: Record<string, unknown>): LinkView {
  ...
  db.prepare("UPDATE open_banking_links SET status = 'LINKED', consent_encrypted = ?, consent_expires_at = ?, accounts = ?, linked_at = ?, last_synced_at = ? WHERE id = ?").run(
    result.consentToken ? encrypt(result.consentToken) : null,
  ...
  notify(userId, `${r.institution_name} linked`, `${result.accounts.length} account(s) connected; ${imported} transactions imported. Income verification runs from your real statement.`, {
    kind: 'wallet',
    linkId: id,
  });
  verifyIncome(userId);
...
export function importStatement(user: UserRow, input: { institutionName: string; currency: string; accountName?: string | null; csv: string }): LinkView & { imported: number; rejected: number } {
  ...
  for (const rec of recs) {
    ...
    if (!description || Number.isNaN(parsed) || !/^-?\d+(\.\d+)?$/.test(amountStr)) {
      rejected += 1;
      continue;
    }
    ...
    rows.push({
      externalId: rec.id || rec.reference || `${date}:${description}:${amountStr}`,
      bookedAt: new Date(parsed).toISOString(),
      amountMinor: Math.round(amount * 10 ** cur.decimals),
      currency: (rec.currency || cur.code).toUpperCase(),
  ...
  getDb()
    .prepare(
      'INSERT INTO open_banking_links (id, user_id, provider, institution_id, institution_name, country, status, accounts, created_at, linked_at, last_synced_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
    .run(
      id,
      user.id,
      ...
      user.country ?? null,
      'LINKED',
      JSON.stringify([{ id: accountId, name: input.accountName ?? 'Imported statement', type: 'current', masked: '••••', currency: cur.code, balanceMinor: balance }]),
      now(),
      now(),
      now(),
    );
...
export function listLinks(userId: string): LinkView[] {
  return (getDb().prepare("SELECT * FROM open_banking_links WHERE user_id = ? AND status != 'DECLINED' ORDER BY created_at DESC").all(userId) as any[]).map(linkView);
}
export function listTransactions(userId: string, filter: { linkId?: string | null; accountId?: string | null; limit?: number } = {}) {
  ...
  return (
    getDb()
      .prepare(`SELECT * FROM open_banking_transactions WHERE ${where.join(' AND ')} ORDER BY booked_at DESC LIMIT ?`)
      .all(...params, Math.min(1000, filter.limit ?? 200)) as any[]
  ).map((t) => ({
    id: t.id,
    linkId: t.link_id,
...
export function verifyIncome(userId: string): IncomeReport {
  ...
  const credits = db
    .prepare(
      "SELECT booked_at, amount_minor, currency, description, counterparty FROM open_banking_transactions t JOIN open_banking_links l ON l.id = t.link_id WHERE t.user_id = ? AND l.status = 'LINKED' AND t.amount_minor > 0 AND t.booked_at >= ? ORDER BY booked_at",
    )
  ...
  for (const [key, rows] of groups) {
    ...
    streams.push({
      label: key.split(':')[1] || 'CREDIT',
      currency,
      months: byMonth.size,
      medianMinor: median,
      monthlyBaseMinor: toBase(median, currency),
      lastAt: rows[rows.length - 1].booked_at,
      regular,
    });
  }
  ...
  const report: IncomeReport = {
    userId,
    monthlyIncomeBase: monthly,
    baseCurrency: getCurrency(getDb().prepare('SELECT code FROM currencies WHERE is_base = 1').pluck().get() as string, false).code,
  ...
  db.prepare(
    'INSERT INTO open_banking_income (user_id, monthly_income_base, months_covered, streams, confidence, computed_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET monthly_income_base = excluded.monthly_income_base, months_covered = excluded.months_covered, streams = excluded.streams, confidence = excluded.confidence, computed_at = excluded.computed_at',
  ).run(userId, monthly, months.size, JSON.stringify(streams), confidence, report.computedAt);
...
export function getIncomeReport(userId: string): IncomeReport | null {
  ...
  return {
    userId,
    monthlyIncomeBase: r.monthly_income_base,
    baseCurrency: getCurrency(getDb().prepare('SELECT code FROM currencies WHERE is_base = 1').pluck().get() as string, false).code,
    monthsCovered: r.months_covered,
    streams: parseJson(r.streams, []),
...
export interface MandateView {
  ...
  accountId: string;
  currency: string;
  purpose: 'top_up' | 'billing';
  maxPerPaymentMinor: number;
  maxPerMonthMinor: number;
  usedThisMonthMinor: number;
...
function mandateView(r: any): MandateView {
  ...
  return {
    ...
    currency: r.currency,
    purpose: r.purpose,
    maxPerPaymentMinor: r.max_per_payment_minor,
    maxPerMonthMinor: r.max_per_month_minor,
    usedThisMonthMinor: r.used_month === month ? r.used_month_minor : 0,
    status: r.status,
    createdAt: r.created_at,
  };
}
export function createMandate(
  user: UserRow,
  input: { linkId: string; accountId: string; purpose: 'top_up' | 'billing'; maxPerPaymentMinor: number; maxPerMonthMinor: number },
  actor: Actor,
): MandateView {
  ...
  if (!Number.isInteger(input.maxPerPaymentMinor) || input.maxPerPaymentMinor <= 0 || !Number.isInteger(input.maxPerMonthMinor) || input.maxPerMonthMinor < input.maxPerPaymentMinor)
    throw badRequest('Set a per-payment limit and a monthly limit at least as large', 'invalid_limits');
  const provider = providerFor(r.provider);
  const ref = provider.createMandate({ id: r.id, consentToken: r.consent_encrypted ? decrypt(r.consent_encrypted) : null }, account, {
    maxPerPaymentMinor: input.maxPerPaymentMinor,
    maxPerMonthMinor: input.maxPerMonthMinor,
    currency: account.currency,
  });
  const id = `vrp_${shortCode(10).toLowerCase()}`;
  getDb()
    .prepare(
      'INSERT INTO open_banking_mandates (id, user_id, link_id, account_id, currency, purpose, max_per_payment_minor, max_per_month_minor, status, provider_ref, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    )
    .run(id, user.id, r.id, account.id, account.currency, input.purpose, input.maxPerPaymentMinor, input.maxPerMonthMinor, 'ACTIVE', ref.providerRef, now());
  recordEvent('auth', user.id, 'open_banking.mandate_created', actor, {
    mandateId: id,
    purpose: input.purpose,
    maxPerPayment: input.maxPerPaymentMinor,
    maxPerMonth: input.maxPerMonthMinor,
    currency: account.currency,
  });
  return mandateView(getDb().prepare('SELECT * FROM open_banking_mandates WHERE id = ?').get(id));
}
export function listMandates(userId: string): MandateView[] {
  return (getDb().prepare('SELECT * FROM open_banking_mandates WHERE user_id = ? ORDER BY created_at DESC').all(userId) as any[]).map(mandateView);
}
...
export function executeBankPayment(input: {
  userId: string;
  linkId: string;
  accountId: string;
  amountMinor: number;
  ...
  db.prepare(
    'INSERT INTO open_banking_payments (id, user_id, link_id, account_id, mandate_id, gateway_payment_id, amount_minor, currency, status, provider_ref, reason, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ).run(
    `obp_${shortCode(10).toLowerCase()}`,
    input.userId,
    r.id,
    account.id,
    ...
    result.status,
    result.providerRef,
    input.reason ?? result.failureReason ?? null,
    now(),
  );
  if (result.status === 'succeeded') {
    if (provider.sandbox)
      db.prepare('UPDATE open_banking_links SET accounts = ? WHERE id = ?').run(
        JSON.stringify(accounts.map((a) => (a.id === account.id ? { ...a, balanceMinor: a.balanceMinor - input.amountMinor } : a))),
        r.id,
      );
    if (input.mandateId) {
      const month = new Date().toISOString().slice(0, 7);
      db.prepare('UPDATE open_banking_mandates SET used_month_minor = CASE WHEN used_month = ? THEN used_month_minor + ? ELSE ? END, used_month = ? WHERE id = ?').run(
        month,
        input.amountMinor,
        input.amountMinor,
        month,
 ...
 */
export async function topUpFromMandate(
  userId: string,
  currency: string,
  amountMinor: number,
  purpose: 'top_up' | 'billing',
  reason: string,
): Promise<{ ok: boolean; paymentId: string | null; failureReason?: string }> {
  const m = getDb()
    .prepare("SELECT * FROM open_banking_mandates WHERE user_id = ? AND currency = ? AND purpose = ? AND status = 'ACTIVE' ORDER BY created_at DESC")
    .get(userId, currency, purpose) as any;
  ...
  try {
    ...
    notify(
      ...
      `${formatMoney(amountMinor, getCurrency(currency))} came in from ${(getDb().prepare('SELECT institution_name FROM open_banking_links WHERE id = ?').get(m.link_id) as any)?.institution_name} under your mandate (${reason}).`,
      { kind: 'deposit', paymentId: payment.id },
    );
...
export function openBankingOverview(user: UserRow) {
  return {
    ...
    institutions: listInstitutions(user.country),
    links: listLinks(user.id),
    mandates: listMandates(user.id),
    income: getIncomeReport(user.id),
    countries: [...new Set(listInstitutions().map((i) => i.country))].map((c) => ({ code: c, name: COUNTRIES.find((x) => x.code === c)?.name ?? c })),
  };
}
...
export function listAllLinksAdmin(filter: { limit?: number } = {}) {
  ...
  const links = (
    db
      .prepare(
        'SELECT l.id, l.user_id, l.provider, l.institution_name, l.country, l.status, l.consent_expires_at, l.last_synced_at, l.created_at, u.full_name, u.tag FROM open_banking_links l JOIN users u ON u.id = l.user_id ORDER BY l.created_at DESC LIMIT ?',
      )
      .all(Math.min(1000, filter.limit ?? 200)) as any[]
  ).map((l) => ({
    id: l.id,
    userId: l.user_id,
    user: { name: l.full_name, tag: l.tag },
    provider: l.provider,
  ...
  const mandates = (db.prepare('SELECT m.*, u.tag FROM open_banking_mandates m JOIN users u ON u.id = m.user_id ORDER BY m.created_at DESC LIMIT 200').all() as any[]).map((m) => ({
    ...mandateView(m),
    userTag: m.tag,
  }));
  const payments = (db.prepare('SELECT * FROM open_banking_payments ORDER BY created_at DESC LIMIT 200').all() as any[]).map((p) => ({
    id: p.id,
    userId: p.user_id,
    linkId: p.link_id,
  ...
  const stats = {
    links: Object.fromEntries((db.prepare('SELECT status, COUNT(*) c FROM open_banking_links GROUP BY status').all() as any[]).map((r) => [r.status, r.c])),
    verifiedIncome: (db.prepare("SELECT COUNT(*) c FROM open_banking_income WHERE confidence != 'none'").get() as any).c,
  };
