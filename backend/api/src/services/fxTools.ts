...
// ---------------------------------------------------------------- alerts
export interface FxAlert { id: string; baseCurrency: string; quoteCurrency: string; direction: 'above' | 'below'; targetRate: number; status: 'ACTIVE' | 'TRIGGERED' | 'CANCELLED'; note: string | null; triggeredAt: string | null; triggeredRate: number | null; currentRate: number; createdAt: string }
const alertView = (r: any): FxAlert => ({ id: r.id, baseCurrency: r.base_currency, quoteCurrency: r.quote_currency, direction: r.direction, targetRate: r.target_rate, status: r.status, note: r.note, triggeredAt: r.triggered_at, triggeredRate: r.triggered_rate, currentRate: safeMid(r.base_currency, r.quote_currency), createdAt: r.created_at });
function safeMid(from: string, to: string): number { try { return currentRate(from, to).midRate; } catch { return 0; } }
export function createAlert(user: UserRow, input: { baseCurrency: string; quoteCurrency: string; direction: 'above' | 'below'; targetRate: number; note?: string | null }): FxAlert {
  ...
  getDb().prepare('INSERT INTO fx_alerts (id, user_id, base_currency, quote_currency, direction, target_rate, status, note, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, user.id, base.code, quote.code, input.direction, input.targetRate, 'ACTIVE', input.note ?? null, now());
  return alertView(getDb().prepare('SELECT * FROM fx_alerts WHERE id = ?').get(id));
}
export function listAlerts(userId: string): FxAlert[] { return (getDb().prepare('SELECT * FROM fx_alerts WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').all(userId) as any[]).map(alertView); }
...
export function checkFxAlerts(): { checked: number; triggered: number } {
  ...
  for (const a of rows) {
    let mid: number;
    try { mid = currentRate(a.base_currency, a.quote_currency).midRate; } catch { continue; }
    ...
    triggered += 1;
    notify(a.user_id, `${a.base_currency}/${a.quote_currency} ${a.direction === 'above' ? 'reached' : 'fell to'} ${mid.toFixed(4)}`, `Your alert at ${a.target_rate} fired. Open Exchange to convert at today's disclosed rate.`, { kind: 'wallet', alertId: a.id, url: '/app/exchange' });
...
// ---------------------------------------------------------------- auto-convert rules
export interface AutoRule { id: string; fromCurrency: string; toCurrency: string; kind: 'on_receipt' | 'sweep'; shareBps: number; keepMinor: number; minRate: number | null; status: 'ACTIVE' | 'PAUSED'; runs: number; convertedMinor: number; lastRunAt: string | null; lastError: string | null; createdAt: string }
const ruleView = (r: any): AutoRule => ({ id: r.id, fromCurrency: r.from_currency, toCurrency: r.to_currency, kind: r.kind, shareBps: r.share_bps, keepMinor: r.keep_minor, minRate: r.min_rate, status: r.status, runs: r.runs, convertedMinor: r.converted_minor, lastRunAt: r.last_run_at, lastError: r.last_error, createdAt: r.created_at });
export function createAutoRule(user: UserRow, input: { fromCurrency: string; toCurrency: string; kind: 'on_receipt' | 'sweep'; shareBps?: number | null; keepMinor?: number | null; minRate?: number | null }): AutoRule {
  ...
  const dup = getDb().prepare("SELECT id FROM fx_auto_rules WHERE user_id = ? AND from_currency = ? AND to_currency = ? AND kind = ? AND status = 'ACTIVE'").get(user.id, from.code, to.code, input.kind);
  ...
  getDb().prepare('INSERT INTO fx_auto_rules (id, user_id, from_currency, to_currency, kind, share_bps, keep_minor, min_rate, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, user.id, from.code, to.code, input.kind, share, input.keepMinor ?? 0, input.minRate ?? null, 'ACTIVE', now(), now());
...
export function listAutoRules(userId: string): AutoRule[] { return (getDb().prepare('SELECT * FROM fx_auto_rules WHERE user_id = ? ORDER BY created_at DESC').all(userId) as any[]).map(ruleView); }
...
function runRule(rule: any, amountMinor: number): boolean {
  ...
  try {
    ...
    db.prepare("UPDATE fx_auto_rules SET runs = runs + 1, converted_minor = converted_minor + ?, last_run_at = ?, last_error = NULL, updated_at = ? WHERE id = ?").run(amountMinor, now(), now(), rule.id);
    notify(user.id, 'Auto-convert done', `${formatMoney(amountMinor, getCurrency(rule.from_currency))} became ${formatMoney(r.received, getCurrency(rule.to_currency))} at ${r.rate.toFixed(4)} (your ${rule.kind === 'on_receipt' ? 'on-receipt' : 'sweep'} rule).`, { kind: 'wallet', transactionId: r.tx.id });
    return true;
  } catch (err) {
...
// ---------------------------------------------------------------- forwards
export interface ForwardView { id: string; fromCurrency: string; toCurrency: string; amountMinor: number; receiveMinor: number; rate: number; midRate: number; marginBps: number; forwardBps: number; settleOn: string; expiresOn: string; status: 'LOCKED' | 'SETTLED' | 'EXPIRED' | 'CANCELLED'; transactionId: string | null; createdAt: string; settledAt: string | null }
const forwardView = (r: any): ForwardView => ({ id: r.id, fromCurrency: r.from_currency, toCurrency: r.to_currency, amountMinor: r.amount_minor, receiveMinor: r.receive_minor, rate: r.rate, midRate: r.mid_rate, marginBps: r.margin_bps, forwardBps: r.forward_bps, settleOn: r.settle_on, expiresOn: r.expires_on, status: r.status, transactionId: r.transaction_id, createdAt: r.created_at, settledAt: r.settled_at });
export function quoteForward(from: string, to: string, amountMinor: number, settleOn: string) {
  ...
  return { fromCurrency: f.code, toCurrency: t.code, amountMinor, receiveMinor, rate, midRate: spot.midRate, marginBps: spot.marginBps, forwardBps: s.forwardBps, fee, settleOn: day, expiresOn, days, disclosure: `Locked rate ${rate.toFixed(6)} = reference ${spot.midRate.toFixed(6)} less the ${spot.marginBps / 100}% exchange margin and the ${s.forwardBps / 100}% forward margin. ${formatMoney(amountMinor + fee, f)} is ring-fenced until ${day}.` };
}
export function lockForward(user: UserRow, input: { fromCurrency: string; toCurrency: string; amountMinor: number; settleOn: string }, actor: Actor): ForwardView {
  ...
  const s = getForwardSettings();
  const db = getDb();
  const openAccount = (db.prepare("SELECT COALESCE(SUM(amount_minor), 0) s, from_currency FROM fx_forwards WHERE user_id = ? AND status = 'LOCKED' GROUP BY from_currency").all(user.id) as any[]).reduce((a, r) => a + toBase(r.s, r.from_currency), 0);
  if (openAccount + toBase(q.amountMinor, q.fromCurrency) > s.maxOpenPerAccountBase) throw unprocessable('Open forwards on this account would exceed the ceiling', 'forward_account_ceiling', { maxOpenPerAccountBase: s.maxOpenPerAccountBase });
  const openTotal = (db.prepare("SELECT COALESCE(SUM(amount_minor), 0) s, from_currency FROM fx_forwards WHERE status = 'LOCKED' GROUP BY from_currency").all() as any[]).reduce((a, r) => a + toBase(r.s, r.from_currency), 0);
  ...
  const held = Object.values(heldByKind(wallet.id)).reduce((a, b) => a + b, 0);
  if (wallet.balance - held < q.amountMinor + q.fee) throw unprocessable('Not enough available balance to ring-fence for this forward', 'insufficient_funds', { available: wallet.balance - held, needed: q.amountMinor + q.fee });
  const id = `fwd_${shortCode(10).toLowerCase()}`;
  db.transaction(() => {
    const hold = createHold({ walletId: wallet.id, amountMinor: q.amountMinor + q.fee, kind: 'reserve', refType: 'fx_forward', refId: id, reason: `forward ${q.fromCurrency}→${q.toCurrency} settling ${q.settleOn}` }, actor);
    db.prepare('INSERT INTO fx_forwards (id, user_id, from_currency, to_currency, amount_minor, receive_minor, rate, mid_rate, margin_bps, forward_bps, settle_on, expires_on, status, hold_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, user.id, q.fromCurrency, q.toCurrency, q.amountMinor, q.receiveMinor, q.rate, q.midRate, q.marginBps, q.forwardBps, q.settleOn, q.expiresOn, 'LOCKED', hold.id, now());
  })();
  ...
  notify(user.id, 'Rate locked', `${formatMoney(q.amountMinor, getCurrency(q.fromCurrency))} will become ${formatMoney(q.receiveMinor, getCurrency(q.toCurrency))} on ${q.settleOn} at ${q.rate.toFixed(4)}, whatever the market does.`, { kind: 'wallet', forwardId: id });
  return forwardView(db.prepare('SELECT * FROM fx_forwards WHERE id = ?').get(id));
}
export function listForwards(userId: string): ForwardView[] { return (getDb().prepare('SELECT * FROM fx_forwards WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').all(userId) as any[]).map(forwardView); }
...
export function settleForward(id: string, actor: Actor, opts: { force?: boolean } = {}): ForwardView {
  ...
  const tx = db.transaction(() => {
    ...
    const posted = postTransaction({ type: 'exchange', amount: f.amount_minor, fee, currency: f.from_currency, receiveAmount: f.receive_minor, receiveCurrency: f.to_currency, fromWalletId: fromWallet.id, toWalletId: toWallet.id, senderUserId: user.id, receiverUserId: user.id, note: `Forward ${f.from_currency} → ${f.to_currency} at ${Number(f.rate).toFixed(4)} (locked ${f.created_at.slice(0, 10)})`, metadata: { forwardId: id, rate: f.rate, midRate: f.mid_rate, marginBps: f.margin_bps, forwardBps: f.forward_bps, guaranteed: true, settleOn: f.settle_on } });
  ...
  notify(user.id, 'Forward settled', `${formatMoney(f.amount_minor, getCurrency(f.from_currency))} became ${formatMoney(f.receive_minor, getCurrency(f.to_currency))} at your locked rate ${Number(f.rate).toFixed(4)}.`, { kind: 'wallet', transactionId: tx.id });
  return forwardView(db.prepare('SELECT * FROM fx_forwards WHERE id = ?').get(id));
}
...
export function runForwards(): { settled: number; expired: number } {
  ...
  for (const f of db.prepare("SELECT id, expires_on FROM fx_forwards WHERE status = 'LOCKED' AND settle_on <= ?").all(today) as any[]) {
    try { settleForward(f.id, { type: 'system' }); settled += 1; } catch (err) {
      if (f.expires_on < today) {
        const row = db.prepare('SELECT * FROM fx_forwards WHERE id = ?').get(f.id) as any;
        if (row.hold_id) try { releaseHold(row.hold_id, { type: 'system' }, 'forward expired'); } catch { /* already released */ }
...
export function fxOverview(user: UserRow) {
  ...
  const rates = pairs.flatMap((a) => pairs.filter((b) => b !== a).map((b) => { try { const r = currentRate(a, b); return { from: a, to: b, ...r }; } catch { return null; } })).filter(Boolean);
  return { alerts: listAlerts(user.id), rules: listAutoRules(user.id), forwards: listForwards(user.id), rates, forwardSettings: (({ enabled, forwardBps, maxTenorDays }) => ({ enabled, forwardBps, maxTenorDays }))(getForwardSettings()), currencies: listCurrencies(true).map((c) => c.code) };
}
