...
| `npm run go-live` | reports the digital rail (money moves without any bank, mobile-money or BTCPay API), provisions the optional external rails whose credentials are in the environment (connectivity-checked), prints the go-live checklist and gate-to-scale metrics, exits non-zero while a blocking item is open |
