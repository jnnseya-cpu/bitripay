...
| Offline lifecycle §28: OFFLINE_CREATED → OFFLINE_ACCEPTED_LOCALLY → SYNC_PENDING → ONLINE_VALIDATING → CONFIRMED / REJECTED on every outcome, "Pending confirmation" receipts until the platform confirms, counter-gap evidence per device, encrypted local queue on the phone (XSalsa20-Poly1305, key in the secure store) | `services/offline.ts` (`OFFLINE_STATES`, `offlineLifecycle`, `listCounterGaps`), `routes/intelligence.ts` (`/offline/settings`, `/offline/devices/:id/gaps`), `frontend/mobile/src/lib/offline.ts`, `frontend/mobile/src/screens/Offline.tsx` | `contract_offline.test.ts`; mobile type check |
