...
| Aggregation fee (the aggregator's revenue) | Every switch payment whose beneficiary credit is confirmed accrues an aggregation fee for the merchant (`switch_payment` on the tariff grid, default 0.80 %, per-merchant contracts apply) recorded as a journal fact `AGGREGATION_FEE` and a `switch_fee_entries` row — never deducted from the interbank flow, never a ledger posting at that moment. An administrator closes a past period (`POST /api/admin/switch/fees/close`, step-up) which issues one invoice per merchant and currency (`AGG-000001`, `switch_fee_invoices`); the merchant sees accrued fees and invoices under Merchant centre → Fees (`GET /api/v1/fees/aggregation`) and pays an invoice from the wallet with the PIN (`POST /api/v1/fees/invoices/:id/pay`, a `subscription` posting to the fees revenue wallet); the administrator records a bank payment or voids an invoice (`/fees/invoices/:id/settle`, entries return to accrued). The payment view exposes `fees.aggregation` (quoted before, accrued after). |
...
Money movement (transfers, QR, cross-payments, remittance) runs on the BitriPay digital rail — the ledger, operator
collection numbers and bank details entered in the console, payout accounts, payout devices and agents — and needs no
bank, mobile-money operator or BTCPay API; the go-live checklist item `digital_rail` blocks until that rail is on and
no external API can ever block it. Optional external rails are provisioned from the environment at start-up:
credentials present → connectivity check → enabled when the check passes (`RAILS_AUTO_ENABLE`); the card processor
from `STRIPE_*` / `PAYSTACK_*` / `FLUTTERWAVE_*`, the national switch adapter from `SWITCH_ADAPTER_MODULE`, and the
operator-API / BTCPay adapters only for a deployment that contracts them (`backend/api/.env.optional-integrations.example`).
`npm run go-live` prints the checklist and exits non-zero while a blocking item is open.
