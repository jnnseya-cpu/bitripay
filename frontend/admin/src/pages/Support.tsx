        ...
        {sel && (
          <>
            <div className="row wrap mb">
              <UserCell user={sel.user} />
              <StatusBadge status={sel.status} />
              <span className="chip">{sel.category}</span>
              <span className="chip">{sel.priority}</span>
            </div>
            ...
            <div className="row mt-sm">
              <Button
                disabled={!reply.trim()}
                onClick={() =>
                  api.post<any>(`/api/admin/support/tickets/${sel.id}/reply`, { body: reply }).then((r) => {
                    setSel(r.ticket);
                    setReply('');
                    list.reload();
              ...
              {sel.status !== 'closed' ? (
                <Button
                  variant="ghost"
                  onClick={() =>
                    api.post<any>(`/api/admin/support/tickets/${sel.id}/status`, { status: 'closed' }).then((r) => {
                      setSel(r.ticket);
                      list.reload();
                    })
                  }
                ...
                <Button
                  variant="ghost"
                  onClick={() =>
                    api.post<any>(`/api/admin/support/tickets/${sel.id}/status`, { status: 'open' }).then((r) => {
                      setSel(r.ticket);
                      list.reload();
                    })
                  }
...
export function Chat() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      load(msgs[msgs.length - 1]?.createdAt);
      convos.reload();
    }, 4000);
    return () => clearInterval(t);
  }, [msgs, userId]); // eslint-disable-line react-hooks/exhaustive-deps
                ...
                {msgs.map((m) => (
                  <div
                    ...
                    style={{
                      ...
                      background: m.isAdmin ? 'var(--primary)' : 'var(--bg-soft)',
                      color: m.isAdmin ? '#fff' : 'inherit',
                      padding: '8px 12px',
                      borderRadius: 12,
                      maxWidth: '75%',
                    }}
        ...
        {sel && (
          <>
            <div className="small muted mb">
              {sel.name} · {sel.email} · {fmtDate(sel.createdAt)}
            </div>
