...
export function Gateways() {
  ...
  const save = async () => {
    try {
      ...
      toast(tr('Gateway saved'), 'success');
      setEdit(null);
      data.reload();
    } catch (err) {
      toast((err as Error).message, 'error');
  ...
  return (
    <div>
      <PageHeader
        title={tr('Deposit & payment gateways (aggregator)')}
        subtitle={tr('Route cards, mobile money and bank transfers through multiple providers. Credentials are encrypted at rest; environment variables act as defaults.')}
        actions={<Button onClick={startNew}>{tr('+ Add gateway')}</Button>}
      />
      <Alert kind="warning">
        <b>Onboarding a real processor:</b> create the account with Stripe, Paystack or Flutterwave, paste the <b>test</b> keys first, register the webhook URL shown by <i>{tr('Test connection')}</i>{' '}
        at the processor and paste its signing secret, complete a full sandbox run, then replace with <b>live</b> keys. Live keys are recognised automatically and are never offered to payers while the
      ...
      <Alert kind="info">
        ...
        works for every operator in the world without an API through the <b>direct rail</b> – configure collection numbers under <b>{tr('Mobile money operators')}</b>.
      </Alert>
      <Alert kind="info">
        <b>{tr('Bitcoin / Lightning')}</b> is a payment rail behind the same intent and QR, never a second checkout: it is offered only where the country capability matrix allows it (Capabilities →{' '}
      ...
      <div className="card">
        <Table
          ...
          rows={(data.data?.items ?? []).map((g) => [
            ...
            <span className="tiny">
              {g.config?.settlementT != null || g.config?.settlementDays != null ? `T+${g.config.settlementT ?? g.config.settlementDays}` : <span className="muted">{tr('T+default')}</span>}
      ...
      <Modal open={!!edit} onClose={() => setEdit(null)} title={edit?.id ? `Configure ${edit.name}` : tr('New gateway')} wide>
        {edit && (
          <div className="grid cols-2">
            <div>
              ...
              {edit.provider === 'stripe' && (
                <Field label={tr('3-D Secure')} hint="automatic = Stripe/issuer decide (SCA); any = always challenge">
              ...
              <Field label={tr('Currencies (comma separated, blank = all)')} hint={`Enabled: ${(config?.currencies ?? []).map((c: any) => c.code).join(', ')}`}>
                <Input value={edit.currencies} onChange={(e) => setEdit({ ...edit, currencies: e.target.value })} placeholder={tr('USD,EUR,NGN')} />
              </Field>
              <Field label={tr('Countries (ISO codes, blank = all)')}>
                <Input value={edit.countries} onChange={(e) => setEdit({ ...edit, countries: e.target.value })} placeholder={tr('NG,GH,KE')} />
              </Field>
              <Field label={tr('Sort order (lower = preferred)')}>
              ...
              {edit.provider === 'sandbox' && (
                <div className="mt">
                  <Switch
                    ...
                    label={tr('Allow sandbox in production (not recommended)')}
                  />
                </div>
              )}
              {edit.provider === 'bitcoin' && (
                <div className="mt">
                  <Switch
                    on={!!edit.config?.allowInProduction}
                    onChange={(v) => setEdit({ ...edit, config: { ...edit.config, allowInProduction: v } })}
                    label={tr('Allow sandbox-mode invoices in production (not recommended)')}
                  />
                  <div className="muted small mt">
                    {tr('Merchants settle in the intent currency at the disclosed rate by default, or keep a BTC balance when their policy says')} <code>bitcoinSettlement: 'btc'</code> (the BTC
                    currency must be enabled). The disclosed rate is the platform BTC rate: sandbox until a live provider or an imported rate sheet supplies one.
                  </div>
                </div>
              )}
              <h4 className="mt">{tr('Provider operations')}</h4>
              <div className="muted small">{tr('Stored on the gateway; the rail registry, Smart Route and the go-live checklist read them. Blank = provider default.')}</div>
              <div className="grid cols-2">
                <Field label={tr('Settlement T+n (days)')}>
                  <Input
                    type="number"
                    min={0}
                ...
                <Field label={tr('Maintenance window')} hint="e.g. Sun 02:00–04:00 UTC">
                  <Input value={cfgText('maintenanceWindow')} onChange={(e) => setCfg('maintenanceWindow', e.target.value)} placeholder="none" />
                </Field>
                <Field label={tr('Support contact')} hint={tr('Provider support desk (email / phone)')}>
                  <Input value={cfgText('supportContact')} onChange={(e) => setCfg('supportContact', e.target.value)} placeholder="support@provider.example" />
                </Field>
                <Field label={tr('Incident contact')} hint={tr('24/7 escalation for outages')}>
                  <Input value={cfgText('incidentContact')} onChange={(e) => setCfg('incidentContact', e.target.value)} placeholder="noc@provider.example" />
                </Field>
                <Field label={tr('Minimum amount (minor units)')}>
                  <Input type="number" min={0} value={cfgText('minMinor')} onChange={(e) => setCfg('minMinor', e.target.value === '' ? '' : Number(e.target.value))} placeholder={tr('0 = none')} />
                </Field>
                <Field label={tr('Maximum amount (minor units)')}>
                  <Input type="number" min={0} value={cfgText('maxMinor')} onChange={(e) => setCfg('maxMinor', e.target.value === '' ? '' : Number(e.target.value))} placeholder={tr('0 = none')} />
                </Field>
                <Field label={tr('Refunds supported')}>
                ...
                <Field label={tr('Webhooks supported')}>
            ...
            <div>
              <h4>{tr('Credentials')}</h4>
              {(provider?.credentialFields ?? []).length === 0 && <div className="muted small">{tr('This provider needs no credentials.')}</div>}
              {(provider?.credentialFields ?? []).map((f: any) => (
                <Field key={f.key} label={f.label} hint={edit.configuredKeys?.includes(f.key) ? tr('Configured – leave blank to keep, type to replace') : tr('Not configured')}>
              ...
              {edit.provider === 'bitcoin' && (
                <Alert kind="info">
                  ...
                  register the webhook URL below on the store and paste its secret (<code>{tr('BTCPay-Sig')}</code>{' '}
                  {tr('HMAC-SHA256). Confirmations default to 1 on-chain; Lightning settles on payment.')}
                </Alert>
              )}
