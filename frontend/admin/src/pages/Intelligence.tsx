...
export function Intelligence() {
  ...
  return (
    <div>
      ...
      <Tabs
        tabs={[
          { id: 'gateway', label: 'AI gateway & ACU' },
          { id: 'mesh', label: 'Agent mesh' },
          { id: 'events', label: 'Domain events' },
          { id: 'diaspora', label: 'Diaspora-Direct' },
          { id: 'offline', label: 'Offline protocol' },
        ]}
        value={tab}
  ...
  return (
    <div>
      <Alert kind={d.totals.belowFloor ? 'error' : 'success'}>
        Realised gross margin over {d.days} days: <b>{d.totals.acuRevenueMicros ? `${(d.totals.margin * 100).toFixed(1)}%` : 'n/a'}</b> · floor {(d.policy.minGrossMargin * 100).toFixed(0)}% ·{' '}
      ...
      <div className="grid cols-2">
        <div className="card">
          ...
          <Table
            head={['Model', 'Provider', 'Circuit', 'List price in/out', 'Projected margin', '']}
          ...
          <p className="small muted">
            Failover after errors or {d.routing.timeoutMs} ms; a model's circuit opens after {d.routing.circuitFailures} failures in {d.routing.circuitWindowMs / 60000} min.
          </p>
        </div>
        <div className="card">
          <h4>ACU policy</h4>
          <div className="grid cols-2">
            <Field label="ACU price (micro-dollars)">
              <Input type="number" value={p.acuPriceMicros} onChange={(e) => setPolicy({ ...p, acuPriceMicros: Number(e.target.value) })} />
            </Field>
            <Field label="ACU per 1k tokens">
              <Input type="number" step="0.1" value={p.acuPerKiloToken} onChange={(e) => setPolicy({ ...p, acuPerKiloToken: Number(e.target.value) })} />
            </Field>
            <Field label="Margin floor (≥ 0.66)">
            ...
            <Field label="Expected tokens (standard / deep)">
              <div className="row">
      ...
      <div className="card mt">
        ...
        <Table
          head={['When', 'Agent', 'Task', 'Provider / model', 'Tokens', 'Cost', 'ACU', 'Margin', 'Outcome']}
...
function Mesh({ ok, err }: { ok: (m: string) => void; err: (e: any) => void }) {
  ...
  return (
    <div>
      <p className="small muted">
        Every binding starts in shadow mode: the agent reads and reports, side-effecting tools are refused. After {data.data?.shadowDays} days it can be promoted to propose (approvals by a second
        administrator); money never moves without a person.
      </p>
      <div className="card">
        <Table
          head={['Registry', 'Agent', 'Event', 'Autonomy', 'Runs', 'Last run', 'Eligible', '']}
...
function Events(_props: { ok: (m: string) => void; err: (e: any) => void }) {
  ...
  return (
    <div className="card">
      ...
      <Table
        head={['When', 'Type', 'Tenant', 'Aggregate', 'Handled by', 'Payload']}
  ...
  return (
    <div className="grid cols-2">
      <div className="card">
        ...
        <p className="small muted">
          Your signature binds the markup, fees and ceilings; rate cards are then re-issued from the live mid-market rate under this policy at most every {form.maxValidityHours} hours.
        </p>
        <div className="grid cols-2">
        ...
        <Table
          head={['Pair', 'Markup', 'Fees', 'Validity', 'Version', 'Status']}
      ...
      <div className="card">
        <div className="row">
          <h4 style={{ margin: 0 }}>Rate cards</h4>
          <Button
            size="sm"
            variant="secondary"
        ...
        <Table
          head={['Pair', 'Mid', 'Customer', 'Provider', 'Valid until']}
        ...
        <Table
          head={['Institution', 'Kind', 'Purposes', 'Status', '']}
  ...
  return (
    <div className="grid cols-2">
      ...
      <div className="card">
        ...
        <Table
          head={['Hash', 'Amount', 'Payer', 'Merchant', 'State', 'Reason', 'Synced']}
