...
export function Seo() {
  ...
  return (
    <div>
      <PageHeader
        title="Blog & SEO"
        subtitle="Articles written by editors or drafted by the content agent, dynamic internal links, backlinks, feeds and structured data. Everything the search engines, social networks and AI answer engines read is rendered on the server."
        actions={
          <>
      ...
      {agent && (
        <Alert kind={agent.keyConfigured ? 'success' : 'warning'}>
          {agent.keyConfigured ? (
            <>
              Content agent <b>live</b> on {agent.model}. Drafts go to review{agent.autoPublish ? ' and are published automatically' : ' until an editor publishes them'}; {agent.postsPerWeek}{' '}
      ...
      <Tabs
        tabs={[
          ...
          { id: 'agent', label: 'Content agent' },
          { id: 'links', label: 'Dynamic links' },
      ...
      {tab === 'posts' && (
        <>
          <div className="grid cols-4 mb">
            <div className="card">
              <KV k="Published" v={posts.filter((p) => p.status === 'published').length} />
            </div>
            <div className="card">
              <KV k="In review" v={posts.filter((p) => p.status === 'review').length} />
            </div>
            <div className="card">
              <KV k="Views (30 days)" v={d?.views?.total ?? 0} />
            </div>
            <div className="card">
              <KV
                k="Feeds"
          ...
          <div className="card">
            <Table
              head={['Title', 'Status', 'Source', 'Tags', 'Views', 'Published', '']}
              rows={posts.map((p) => [
                <div>
                  <b>{p.title}</b>
      ...
      {tab === 'agent' && (
        <div className="grid cols-2">
          <div className="card">
            ...
            <Field label="Topic or working title">
              <Input value={draft.topic} onChange={(e) => setDraft({ ...draft, topic: e.target.value })} placeholder="How to pay school fees with mobile money" />
            </Field>
            <Field label="Target keywords (comma-separated, optional)">
            ...
            <h4>Keyword ideas</h4>
            <div className="row">
              <Input value={seed} onChange={(e) => setSeed(e.target.value)} placeholder="Seed topic, e.g. school fees" />
              <Button
                variant="secondary"
                loading={busy === 'kw'}
            ...
            {ideas.length > 0 && (
              <Table
                head={['Query', 'Intent', 'Difficulty', 'Angle', '']}
                rows={ideas.map((i) => [
                  <b>{i.keyword}</b>,
                  i.intent,
          ...
          <div className="card">
            {audit ? (
              <>
                ...
                />
                <KV k="Suggested title" v={audit.audit.suggestedTitle} />
                <KV k="Suggested description" v={<span className="tiny">{audit.audit.suggestedDescription}</span>} />
              ...
              <>
                ...
                <Table
                  head={['When', 'Task', 'Model', 'Tokens', 'Status', 'Article']}
      ...
      {tab === 'links' && (
        <div className="card">
          <div className="row between mb">
            <div>
              <h4>Dynamic hyperlinks</h4>
              <div className="tiny muted">
                Keywords that become links automatically wherever they appear in articles and policy pages (first N occurrences, never inside headings, code or existing links). Published articles also
                link to each other through their keywords.
              </div>
            </div>
            <Button onClick={() => setRule({ keyword: '', url: '/', title: '', kind: 'internal', maxPerPage: 1, priority: 50, enabled: true })}>+ Rule</Button>
          </div>
          <Table
            head={['Keyword', 'Links to', 'Kind', 'Max / page', 'Priority', 'On', '']}
        ...
        <>
          <div className="card mb">
            <div className="row between mb">
              <div>
                <h4>Backlinks</h4>
                <div className="tiny muted">Inbound links are discovered automatically from referrers; partner and outbound links are tracked and re-verified weekly.</div>
              </div>
              <div className="row">
                ...
                <Button onClick={() => setBacklink({ direction: 'partner', sourceUrl: '', targetUrl: '/', anchor: '', status: 'pending', notes: '' })}>+ Backlink</Button>
              </div>
            </div>
            <Table
              head={['Direction', 'Source', 'Target', 'Status', 'Hits', 'Last seen', '']}
              ...
              empty="No backlinks recorded yet"
            />
          </div>
          <div className="card mb">
            ...
            <Table
              head={['Domain', 'Cited in', 'Links back?']}
              ...
              empty="Publish articles with sources to build this list"
            />
          </div>
          <div className="card">
          ...
          {settings && (
            <>
              <div className="grid cols-2">
                ...
                <Field label="IndexNow key" hint="Any 8-64 character key; published at /<key>.txt and used to notify Bing, Yandex, Seznam and Naver on every publish">
                  <Input value={settings.indexNowKey} onChange={(e) => setSettings({ ...settings, indexNowKey: e.target.value })} />
                </Field>
                <Field label="Languages (hreflang, comma-separated)">
                  <Input
                ...
                <Field label="Legal name (structured data)">
              ...
              <Field label="Social profiles (sameAs, one URL per line)">
                <Textarea
                  rows={3}
              ...
              <h4>Content agent</h4>
              <div className="grid cols-2">
                <Field label="Anthropic API key" hint="Stored encrypted. Leave the dots to keep the current key.">
                ...
                <Field label="Articles per week from the backlog">
                  <Input
                    type="number"
                    min={0}
                    max={14}
                ...
                <Field label="Markets (ISO countries)">
                  <Input
              ...
              <div className="row wrap mb">
                <Switch on={settings.agent.enabled} onChange={(v) => setSettings({ ...settings, agent: { ...settings.agent, enabled: v } })} label="Agent enabled" />
                <Switch
                  on={settings.agent.autoPublish}
                  onChange={(v) => setSettings({ ...settings, agent: { ...settings.agent, autoPublish: v } })}
                  label="Auto-publish agent drafts (otherwise they wait for an editor)"
                />
              </div>
              ...
              <Field label="Topic backlog (one per line; the scheduler works from the top)">
                <Textarea
                  rows={6}
        ...
        {edit && (
          <>
            <div className="grid cols-2">
              ...
              <Field label="Target keywords (comma-separated; other articles link to this one through them)">
            ...
            <Field label="Body (Markdown; use ## headings, lists, tables, links)">
            ...
            {edit.social && Object.keys(edit.social).length > 0 && (
              <div className="card soft mt">
                <b className="small">Social pack</b>
        ...
        {rule && (
          <>
            <div className="grid cols-2">
              ...
              <Field label="Links to (path or URL)">
              ...
              <Field label="Max links per page">
            ...
            <div className="mb">
              <Switch on={rule.enabled !== false} onChange={(v) => setRule({ ...rule, enabled: v })} label="Enabled" />
            </div>
            <Button
              onClick={() =>
                api
