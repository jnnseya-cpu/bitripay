    ...
    <div>
          ...
          <StepUpButton
            ...
            onConfirm={(pin) =>
              api
                ...
                .then(() => ok(rt.killSwitch ? 'Agents resumed' : 'All agents paused'))
                .catch(err)
            }
          >
      ...
      <Alert kind={rt.killSwitch ? 'error' : rt.mode === 'live' ? 'success' : 'warning'}>
        {rt.killSwitch ? (
          ...
          <>
            Agents run in <b>offline mode</b>: no model key configured, so answers come from the built-in planner over the same tools. Add a key under Settings to switch to {rt.model}.
          </>
        )}
      </Alert>
      <Tabs
          ...
          rows={d.agents.map((a: any) => [
            <div>
              ...
              <div className="tiny muted">
                {a.tagline}
                {a.schedule ? ' · scheduled daily' : ''}
              </div>
            </div>,
            <span className="tiny">{a.roles.join(', ')}</span>,
            <span className="tiny" title={a.tools.join('\n')}>
              {a.tools.length} tools · max {a.budget.maxSteps} steps
            </span>,
            a.runs30d,
            a.failed30d ? <Chip kind="danger">{a.failed30d}</Chip> : 0,
            a.awaiting ? <Chip kind="warning">{a.awaiting}</Chip> : 0,
            a.acu30d,
            <span className="tiny">{fmtDate(a.lastRunAt)}</span>,
            <div className="row">
              {a.paused ? (
                <Button
                  size="sm"
                  ...
                  onClick={() =>
                    api
                      .post(`/api/admin/agents/${a.key}/resume`)
                      .then(() => ok(`${a.name} resumed`))
                      .catch(err)
                  }
                >
                  Resume
                ...
                <Button
                  ...
                  onClick={() =>
                    api
                      .post(`/api/admin/agents/${a.key}/pause`)
                      .then(() => ok(`${a.name} paused`))
                      .catch(err)
                  }
                >
                  Pause
              ...
              {a.roles.includes('admin') && (
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    setAsk({ key: a.key, input: a.schedule ? `Run the ${a.name.toLowerCase()} check` : '' });
                    setResult(null);
                  }}
                >
                  Run
      ...
      <Modal open={!!ask} onClose={() => setAsk(null)} title={`Run ${ask?.key.replace(/_/g, ' ')} as yourself`} wide>
        {ask && (
          <>
            ...
            <Button
              loading={busy}
              disabled={ask.input.trim().length < 2}
              onClick={() => {
                setBusy(true);
                api
                  .post<any>('/api/admin/agents/run?wait=1', { agent: ask.key, input: ask.input })
                  .then((r) => {
                    setResult(r.run);
                    ok('Run finished');
                  })
    ...
    <div className="card">
      ...
      <Table
        ...
        rows={items.map((a) => [
          ...
          <span className="tiny">{fmtDate(a.expiresAt)}</span>,
          canApprove ? (
            <div className="row">
              <StepUpButton
                size="sm"
                ...
                onConfirm={(pin, reason) =>
                  api
                    .post(`/api/admin/agents/approvals/${a.id}/approve`, { pin, reason })
                    .then(() => ok('Approved and executed'))
                    .catch(err)
                }
              >
                Approve
              ...
              <ConfirmButton
                ...
                onConfirm={(reason) =>
                  api
                    .post(`/api/admin/agents/approvals/${a.id}/decline`, { reason })
                    .then(() => ok('Declined'))
                    .catch(err)
                }
              >
...
function RunDetail({ run }: { run: any }) {
  return (
    <div className="mt">
      <div className="row wrap">
        <Chip kind={run.status === 'completed' ? 'success' : run.status === 'failed' ? 'danger' : 'warning'}>{run.status}</Chip>
      ...
      <Table
        ...
        rows={(run.actions ?? []).map((a: any) => [
          ...
          <span className="tiny mono">{JSON.stringify(a.input)}</span>,
          <Chip kind={a.outcome === 'executed' ? 'success' : a.outcome === 'denied' ? 'danger' : 'warning'}>
            {a.outcome}
            {a.reason ? ` · ${a.reason}` : ''}
          </Chip>,
          <details>
            <summary className="tiny">view</summary>
    ...
    <div className="card">
      ...
      <Table
        ...
        rows={(runs.data?.items ?? []).map((r: any) => [
          ...
          r.agent,
          r.trigger,
          <span className="small">{r.input.slice(0, 80)}</span>,
          <Chip kind={r.status === 'completed' ? 'success' : r.status === 'failed' ? 'danger' : 'warning'}>{r.status}</Chip>,
          r.steps,
          r.acu,
...
function Policies({ d, ok, err }: { d: any; ok: (m: string) => void; err: (e: any) => void }) {
  ...
  const toggle = (list: string, name: string) =>
    setForm({ ...form, rules: { ...form.rules, [list]: form.rules[list].includes(name) ? form.rules[list].filter((x: string) => x !== name) : [...form.rules[list], name] } });
  return (
    <>
      <div className="card">
        ...
        <Table
          ...
          rows={d.policies.map((p: any) => [
            ...
            p.scopeId,
            p.version,
            <span className="tiny">{(p.rules.deny ?? []).join(', ') || '—'}</span>,
            <span className="tiny">{(p.rules.requireApproval ?? []).join(', ') || '—'}</span>,
            <span className="tiny">{(p.rules.allow ?? []).join(', ') || '—'}</span>,
            <span className="tiny">
              {p.rules.maxStepsPerRun ? `${p.rules.maxStepsPerRun} steps/run ` : ''}
              {p.rules.maxRunsPerDay ? `${p.rules.maxRunsPerDay} runs/day` : ''}
            </span>,
              ...
              <Field key={list} label={list === 'deny' ? 'Deny these tools' : list === 'requireApproval' ? 'Require a checker for these tools' : 'Allow only these tools (empty = no restriction)'}>
                <div className="row wrap">
                  {tools.map((t) => (
                    <Chip
                      key={t.name}
                      kind={form.rules[list].includes(t.name) ? (list === 'deny' ? 'danger' : list === 'requireApproval' ? 'warning' : 'primary') : undefined}
                      onClick={() => toggle(list, t.name)}
                    >
                      {t.name}
                    </Chip>
                  ))}
              ...
              onConfirm={(pin) =>
                api
                  .put('/api/admin/agents/policies', {
                    ...
                    pin,
                    rules: {
                      deny: form.rules.deny,
                      requireApproval: form.rules.requireApproval,
                      ...(form.rules.allow.length ? { allow: form.rules.allow } : {}),
                      ...(form.maxStepsPerRun ? { maxStepsPerRun: Number(form.maxStepsPerRun) } : {}),
                      ...(form.maxRunsPerDay ? { maxRunsPerDay: Number(form.maxRunsPerDay) } : {}),
                    },
                  })
                  .then(() => {
                    setForm(null);
      ...
      <div className="grid cols-3 mb">
        ...
        {a.revenue.map((r: any) => (
          <div key={r.currency} className="card">
            <KV k={`Revenue ${r.currency}`} v={`${(r.total / 100).toFixed(2)} (${r.c} payments)`} />
          </div>
        ))}
      </div>
      <div className="card">
...
function Settings({ d, ok, err }: { d: any; ok: (m: string) => void; err: (e: any) => void }) {
  ...
  const save = () => {
    let p = s.pricing;
    try {
      p = JSON.parse(pricing);
    } catch {
      return err(new Error('Pricing must be valid JSON'));
    }
    api
      .put('/api/admin/agents/settings', { ...s, pricing: p })
      .then(() => ok('Settings saved'))
      .catch(err);
  };
  return (
    <div className="card">
        ...
        <div>
          ...
          <div className="grid cols-2">
            {['user', 'merchant', 'agent', 'admin'].map((r) => (
              <Field key={r} label={r}>
                <Input type="number" value={s.allowances[r] ?? 0} onChange={(e) => setS({ ...s, allowances: { ...s.allowances, [r]: Number(e.target.value) } })} />
              </Field>
            ))}
          </div>
