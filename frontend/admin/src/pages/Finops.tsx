      ...
      <div className="card">
        ...
        <Table
          ...
          rows={(data.data?.items ?? []).map((s: any) => [
            ...
            <span className="tiny">
              {s.authorId?.slice(0, 8)} / {s.approvedBy?.slice(0, 8) ?? '—'}
            </span>,
            <div className="row">
              {s.status === 'DRAFT' && (
                <ConfirmButton
                  ...
                  onConfirm={() =>
                    api
                      .post(`/api/admin/finops/fees/schedules/${s.id}/approve`, {})
                      .then(() => {
                        ok('Approved');
                        data.reload();
                      })
              ...
              {s.status === 'APPROVED' && (
                <ConfirmButton
                  ...
                  onConfirm={() =>
                    api
                      .post(`/api/admin/finops/fees/schedules/${s.id}/activate`, {})
                      .then(() => {
                        ok('Activated');
                        data.reload();
                      })
              ...
              {s.status === 'ACTIVE' && (
                <ConfirmButton
                  ...
                  onConfirm={() =>
                    api
                      .post(`/api/admin/finops/fees/schedules/${s.id}/retire`, {})
                      .then(() => {
                        ok('Retired');
                        data.reload();
                      })
        ...
        <div className="row">
          ...
          <Select value={explain.type} onChange={(e) => setExplain({ ...explain, type: e.target.value })}>
            {(data.data?.feeTypes ?? ['merchant_payment']).map((t: string) => (
          ...
          <Button
            ...
            onClick={() =>
              api
                .get<any>(`/api/admin/finops/fees/effective${qs({ user: explain.user, type: explain.type })}`)
                .then(setResolved)
                .catch(err)
            }
          >
        ...
        <Button
          onClick={() => {
            ...
            try {
              ...
              return err(new Error('Rules must be valid JSON'));
            }
            api
              .post('/api/admin/finops/fees/schedules', { scope: draft.scope, scopeRef: draft.scopeRef || null, rules, notes: draft.notes || null })
              .then(() => {
                ok('Draft created');
                data.reload();
              })
      ...
      <div className="grid cols-3 mb">
        ...
        <div className="card">
          ...
          <label className="checkbox">
            <input type="checkbox" checked={close.pay} onChange={(e) => setClose({ ...close, pay: e.target.checked })} /> pay immediately
          </label>
          <div className="row mt">
            <Button
              ...
              onClick={() =>
                api
                  .post('/api/admin/finops/settlements/cycles', close)
                  .then((c: any) => {
                    ok(`Cycle ${c.status}`);
                    cycles.reload();
                    obligations.reload();
            ...
            <Button
              ...
              onClick={() =>
                api
                  .post('/api/admin/finops/settlements/run', {})
                  .then((r: any) => {
                    ok(`Run: closed ${r.closed}, paid ${r.paid}`);
                    cycles.reload();
                  })
                  .catch(err)
              }
      ...
      <div className="card">
        ...
        <Table
          ...
          rows={(cycles.data?.items ?? []).map((c: any) => [
            ...
            <span className="tiny">
              {c.periodFrom.slice(0, 10)} → {c.periodTo.slice(0, 10)}
            </span>,
            money(c.grossMinor, c.currency),
            <b>{money(c.netMinor, c.currency)}</b>,
            <StatusBadge status={c.status} />,
            <span className="tiny">{c.dueAt ? fmtDate(c.dueAt) : '—'}</span>,
            <div className="row">
              <a className="btn ghost sm" href={`/api/admin/finops/settlements/cycles/${c.id}/statement?format=pdf`} target="_blank" rel="noreferrer">
                PDF
              </a>
              {c.status === 'CLOSED' && (
                <ConfirmButton
                  ...
                  onConfirm={() =>
                    api
                      .post(`/api/admin/finops/settlements/cycles/${c.id}/pay`, {})
                      .then(() => {
                        ok('Payout requested');
                        cycles.reload();
                        obligations.reload();
              ...
              <>
                ...
                <div className="row">
                  <ConfirmButton
                    ...
                    onConfirm={() =>
                      api
                        .post(`/api/admin/finops/disputes/${sel}/decide`, { decision: 'WON', reason })
                        .then(() => {
                          ok('Merchant won');
                          data.reload();
                        })
                  ...
                  <ConfirmButton
                    ...
                    onConfirm={() =>
                      api
                        .post(`/api/admin/finops/disputes/${sel}/decide`, { decision: 'LOST', reason })
                        .then(() => {
                          ok('Refunded to the payer');
                          data.reload();
                        })
                        .catch(err)
                    }
                  ...
                  <ConfirmButton
                    ...
                    onConfirm={(note) =>
                      api
                        .post(`/api/admin/finops/disputes/${sel}/request-evidence`, { note: note || 'More evidence needed' })
                        .then(() => {
                          ok('Evidence requested');
                          data.reload();
                        })
                        .catch(err)
                    }
    ...
    <div className="grid cols-2">
      <div className="card">
        <Table
          ...
          rows={(data.data?.items ?? []).map((h: any) => [
            ...
            h.kind,
            <span className="tiny">{h.reason}</span>,
            <span className="tiny">{fmtDate(h.createdAt)}</span>,
            <ConfirmButton
              size="sm"
              variant="ghost"
              prompt="Release reason"
              onConfirm={(r) =>
                api
                  .post(`/api/admin/finops/holds/${h.id}/release`, { reason: r || 'released' })
                  .then(() => {
                    ok('Released');
                    data.reload();
                  })
      ...
      <div className="card">
        ...
        <Button
          onClick={() =>
            api
              .post('/api/admin/finops/holds', form)
              .then(() => {
                ok('Hold placed');
                data.reload();
              })
  ...
  return (
    <div className="card">
      <div className="row mb">
        ...
        <span className="small muted">
          platform share {data.data?.settings?.platformShareBps ?? 0} bps · onboarding {data.data?.settings?.onboardingFeeMinor}
        </span>
      </div>
      <Table
    ...
    <div className="grid cols-2">
      <div className="card">
        ...
        <Button
          onClick={() =>
            api
              .post('/api/admin/finops/reconciliation/processors/' + form.gatewayId + '/statements', { cycleRef: form.cycleRef, currency: form.currency, csv: form.csv, run: true })
              .then((r: any) => {
                ok(`Imported ${r.import.lineCount} lines · matched ${r.run?.matched} · cases ${r.run?.casesOpened}`);
                wb.reload();
              })
              .catch(err)
          }
...
function PayoutBatches({ ok, err, money }: { ok: (m: string) => void; err: (e: any) => void; money: (m: number, c: string) => string }) {
  ...
  const approve = (id: string) =>
    api
      .post<any>(`/api/admin/finops/payout-batches/${id}/approve`, {})
      .then((r) => {
        setSelected({ batch: r.batch });
        list.reload();
        ok(`Batch ${r.batch.status.toLowerCase()}: ${r.batch.paidRows} row(s) paid`);
      })
      .catch(err);
  const cancel = (id: string) =>
    api
      .post<any>(`/api/admin/finops/payout-batches/${id}/cancel`, {})
      .then((r) => {
        setSelected({ batch: r.batch });
        list.reload();
        ok('Batch cancelled');
          ...
          <>
            ...
            <Table
              ...
              rows={(b.rows ?? []).map((r: any) => [
                r.lineNo,
                r.method,
                r.method === 'wallet' ? r.destination.to : r.method === 'mobile_money' ? `${r.destination.operatorId} ${r.destination.phone}` : (r.destination.bankName ?? r.destination.bankAccountId),
                money(r.amountMinor, b.currency),
                <StatusBadge status={r.status} />,
                r.error ?? (r.transactionId ? r.transactionId.slice(0, 8) : ''),
              ])}
            />
          </>
        )}
