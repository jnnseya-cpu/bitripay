  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: th.bg }} edges={['top', 'left', 'right']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        {scroll ? <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 40 }}>{body}</ScrollView> : body}
...
export function T({ children, muted, bold, size = 15, color, center, style, mono }: { children: React.ReactNode; muted?: boolean; bold?: boolean; size?: number; color?: string; center?: boolean; style?: any; mono?: boolean }) {
  const th = useTheme();
  return <Text style={[{ color: color ?? (muted ? th.muted : th.text), fontSize: size, fontWeight: bold ? '700' : '400', textAlign: center ? 'center' : 'left', fontFamily: mono ? (Platform.OS === 'ios' ? 'Menlo' : 'monospace') : undefined }, style]}>{children}</Text>;
}

export function Button({ title, onPress, variant, loading, disabled, small, icon }: { title: string; onPress?: () => void; variant?: 'secondary' | 'ghost' | 'danger' | 'success'; loading?: boolean; disabled?: boolean; small?: boolean; icon?: string }) {
  const th = useTheme();
  ...
  return (
    <Pressable onPress={onPress} disabled={disabled || loading} style={({ pressed }) => ({ backgroundColor: bg, opacity: disabled ? 0.5 : pressed ? 0.85 : 1, paddingVertical: small ? 8 : 14, paddingHorizontal: small ? 12 : 18, borderRadius: 12, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8, borderWidth: variant === 'secondary' ? 1 : 0, borderColor: th.border })}>
...
export function Input({ label, hint, style, big, ...props }: TextInputProps & { label?: string; hint?: string; big?: boolean }) {
  ...
  return (
    <View style={{ gap: 6 }}>
      ...
      <TextInput placeholderTextColor={th.muted} {...props} style={[{ backgroundColor: th.card, color: th.text, borderWidth: 1, borderColor: th.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: big ? 14 : 12, fontSize: big ? 28 : 16, fontWeight: big ? '700' : '400' }, style]} />
...
export function Select({ label, value, options, onChange }: { label?: string; value: string; options: { value: string; label: string }[]; onChange: (v: string) => void }) {
  ...
  return (
    <View style={{ gap: 6 }}>
      {label && <Text style={{ color: th.text, fontWeight: '600', fontSize: 13 }}>{label}</Text>}
      <Pressable onPress={() => setOpen(true)} style={{ backgroundColor: th.card, borderWidth: 1, borderColor: th.border, borderRadius: 12, padding: 12, flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      <Sheet open={open} onClose={() => setOpen(false)} title={label}>
        <ScrollView style={{ maxHeight: 380 }}>
          {options.map((o) => (
            <Pressable key={o.value} onPress={() => { onChange(o.value); setOpen(false); }} style={{ padding: 14, borderBottomWidth: 1, borderColor: th.border, backgroundColor: o.value === value ? th.primarySoft : undefined, borderRadius: 8 }}>
...
const STATUS_KIND: Record<string, 'success' | 'warning' | 'danger' | 'primary' | undefined> = { completed: 'success', paid: 'success', succeeded: 'success', verified: 'success', active: 'success', open: 'primary', pending: 'warning', escrowed: 'warning', ready_for_pickup: 'warning', negotiating: 'primary', failed: 'danger', rejected: 'danger', declined: 'danger', disputed: 'danger', reversed: 'danger', frozen: 'warning' };
...
export function Avatar({ user, size = 40 }: { user?: PublicUser | null; size?: number }) {
  ...
  return <View style={{ width: size, height: size, borderRadius: size / 2, backgroundColor: user?.avatarColor || '#64748b', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#fff', fontWeight: '700', fontSize: size * 0.38 }}>{initials}</Text></View>;
}
    ...
    <RNModal visible={open} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={{ backgroundColor: th.card, borderTopLeftRadius: 22, borderTopRightRadius: 22, padding: 20, paddingBottom: 34, gap: 12, maxHeight: '88%' }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Pressable onPress={onClose}><Text style={{ color: th.muted, fontSize: 18 }}>✕</Text></Pressable>
        </View>
        {children}
      </View>
    </RNModal>
...
export function Alert({ kind = 'info', text }: { kind?: 'info' | 'error' | 'success' | 'warning'; text: string }) {
  ...
  return <View style={{ backgroundColor: bg, padding: 12, borderRadius: 12 }}><Text style={{ color: fg, fontSize: 14 }}>{text}</Text></View>;
}

export function KV({ k, v }: { k: string; v: React.ReactNode }) {
  const th = useTheme();
  return <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: th.border, gap: 12 }}><Text style={{ color: th.muted }}>{k}</Text>{typeof v === 'string' || typeof v === 'number' ? <Text style={{ color: th.text, fontWeight: '600', flexShrink: 1, textAlign: 'right' }}>{v}</Text> : v}</View>;
}

export function Qr({ value, size = 220 }: { value: string; size?: number }) {
  return <View style={{ backgroundColor: '#fff', padding: 14, borderRadius: 16, alignSelf: 'center' }}><QRCodeSvg value={value} size={size} color="#0f172a" /></View>;
}

export function Loading() {
  const th = useTheme();
  return <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}><ActivityIndicator color={th.primary} size="large" /></View>;
}

export function Empty({ icon = '🗂️', text }: { icon?: string; text?: string }) {
  const { t } = useStore();
  return <View style={{ alignItems: 'center', padding: 28, gap: 6 }}><Text style={{ fontSize: 34 }}>{icon}</Text><T muted>{text ?? t('common.none')}</T></View>;
}
...
export function TxRow({ tx, onPress }: { tx: Transaction; onPress?: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: th.border }}>
      <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: th.soft, alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 18 }}>{TX_ICONS[tx.type] ?? '•'}</Text></View>
      <View style={{ flex: 1 }}>
        <Text numberOfLines={1} style={{ color: th.text, fontWeight: '600' }}>{who}</Text>
        <Text numberOfLines={1} style={{ color: th.muted, fontSize: 12 }}>{TRANSACTION_TYPE_LABELS[tx.type]} · {new Date(tx.createdAt).toLocaleDateString()}</Text>
      </View>
      <View style={{ alignItems: 'flex-end', gap: 4 }}>
        <Text style={{ color: isIn ? th.success : th.text, fontWeight: '700' }}>{isIn ? '+' : tx.direction === 'out' ? '-' : ''}{money(shown, isIn ? tx.receiveCurrency ?? tx.currency : tx.currency)}</Text>
...
export function useAsync<T>(fn: () => Promise<T>, deps: unknown[] = []) {
  ...
  const run = () => {
    setLoading(true);
    return ref.current().then((d) => { setData(d); setError(null); }).catch((e) => setError(e.message)).finally(() => setLoading(false));
  };
...
export function Toasts() {
  ...
  return (
    <View pointerEvents="none" style={{ position: 'absolute', bottom: 90, left: 16, right: 16, gap: 8 }}>
      {toasts.map((x) => <View key={x.id} style={{ backgroundColor: x.kind === 'error' ? th.danger : x.kind === 'success' ? th.success : th.text, padding: 12, borderRadius: 12 }}><Text style={{ color: '#fff', fontWeight: '600' }}>{x.message}</Text></View>)}
    </View>
  );
}

export function AmountInput({ amount, currency, onAmount, onCurrency, currencies, label }: { amount: string; currency: string; onAmount: (v: string) => void; onCurrency: (c: string) => void; currencies?: string[]; label?: string }) {
  ...
  return (
    <View style={{ gap: 6 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        <TextInput value={amount} onChangeText={(v) => onAmount(v.replace(/[^\d.]/g, ''))} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={th.muted} style={{ flex: 1, backgroundColor: th.card, color: th.text, borderWidth: 1, borderColor: th.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 28, fontWeight: '700' }} />
        <View style={{ width: 110 }}><Select value={currency} onChange={onCurrency} options={codes.map((c: string) => ({ value: c, label: c }))} /></View>
      </View>
    </View>
  );
}
