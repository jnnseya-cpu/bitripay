...
export function PayHub() {
  ...
  return (
    <Screen>
      <Header title={tr('Pay')} />
      <T muted>{tr('Scan a code, pay a link, send to a person or pay a bill. Every payment is confirmed by the ledger, never by a screen.')}</T>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
        <Tile ico="📷" label={t('nav.scan')} hint={tr('Any BitriPay or EMVCo code')} onPress={() => nav.navigate('QrLink')} />
        <Tile ico="📤" label={t('nav.send')} hint="@tag, phone or email" onPress={() => nav.navigate('Send')} />
        {m.transfers !== false && <Tile ico="🔀" label={t('nav.move')} hint={tr('Any source to any destination')} onPress={() => nav.navigate('Move')} />}
        {m.moneyRequests !== false && <Tile ico="🔗" label={t('nav.requests')} hint={tr('Pay or create a request')} onPress={() => nav.navigate('Requests')} />}
        {m.remittance !== false && <Tile ico="🌍" label={t('nav.remittance')} hint={tr('Send abroad, they receive locally')} onPress={() => nav.navigate('Remittance')} />}
        ...
        {m.qrPayments !== false && <Tile ico="📴" label={tr('Offline payments')} hint={tr('Pending confirmation until synced')} onPress={() => nav.navigate('Offline')} />}
        {isMerchant && <Tile ico="🏪" label={t('nav.merchant')} hint={tr('Charge a customer')} onPress={() => nav.navigate('Merchant')} />}
      </View>
    </Screen>
  );
}
...
export function WalletHub() {
  ...
  return (
    <Screen>
      ...
      <Card>
        <Row between>
          <T bold>{tr('Balances')}</T>
          <Button title={tr('Refresh')} small variant="secondary" onPress={() => void refreshWallets()} />
        </Row>
        {wallets.length === 0 && <T muted>{tr('No balance yet. Add money to open your first wallet.')}</T>}
        {wallets.map((w) => (
          <Row key={w.id} between>
            <Row style={{ gap: 8 }}>
              <T bold>{w.currency}</T>
              {w.frozen ? <Chip label={tr('Frozen')} kind="danger" /> : null}
              {(w.promoBalance ?? 0) > 0 ? <Chip label={`promo ${money(w.promoBalance ?? 0, w.currency)}`} /> : null}
            </Row>
            <T bold>{money(w.balance, w.currency)}</T>
          </Row>
        ))}
        <KV k={tr('Wallets')} v={String(wallets.length)} />
      </Card>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
        {m.addMoney !== false && <Tile ico="➕" label={t('nav.addMoney')} hint={tr('Card, mobile money, bank, agent')} onPress={() => nav.navigate('AddMoney')} />}
        {m.withdrawals !== false && <Tile ico="🏦" label={t('nav.withdraw')} onPress={() => nav.navigate('Withdraw')} />}
        {m.exchange !== false && <Tile ico="💱" label={t('nav.exchange')} hint={tr('Disclosed rate and margin')} onPress={() => nav.navigate('Exchange')} />}
        {m.agents !== false && <Tile ico="🏪" label={t('nav.agents')} hint={tr('Cash in and out')} onPress={() => nav.navigate('Agents')} />}
        ...
        <Tile ico="📑" label={t('nav.statements')} hint={tr('Bank-grade statements')} onPress={() => nav.navigate('Statements')} />
        <Tile ico="📊" label={t('nav.insights')} hint={tr('Every chart family')} onPress={() => nav.navigate('Insights')} />
