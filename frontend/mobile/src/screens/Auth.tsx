...
export function Onboarding() {
  ...
  const screens: any[] = config?.site?.onboarding?.length ? config.site.onboarding : [{ title: 'Scan & pay in seconds', body: 'Point your camera at any BitriPay QR code to pay a friend, a shop or an agent.', color: '#2563eb' }, { title: 'One wallet, every currency', body: 'Hold multiple currencies and send money across borders.', color: '#7c3aed' }, { title: 'Cards, bills & top-ups', body: 'Add money by card or mobile money, pay bills and create virtual cards.', color: '#16a34a' }];
  const s = screens[idx];
    ...
    <Screen scroll={false} padded={false}>
      ...
      <View style={{ flex: 1, backgroundColor: s.color, justifyContent: 'flex-end' }}>
        ...
        <View style={{ backgroundColor: th.bg, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, gap: 14 }}>
          ...
          <T muted>{s.body}</T>
          <View style={{ flexDirection: 'row', gap: 6 }}>{screens.map((_, i) => <View key={i} style={{ width: i === idx ? 22 : 8, height: 8, borderRadius: 4, backgroundColor: i === idx ? th.primary : th.border }} />)}</View>
          ...
          {idx < screens.length - 1 && <Pressable onPress={() => setIdx(screens.length - 1)}><T muted center>Skip</T></Pressable>}
        </View>
      </View>
    </Screen>
  );
      ...
      <Card>
        ...
        {mfa ? (
          ...
          <>
            ...
            {mode === 'password' ? <Input label={t('auth.password')} value={password} onChangeText={setPassword} secureTextEntry /> : otpSent ? <><Alert text={otpSent} /><Input label={t('auth.code')} value={code} onChangeText={setCode} keyboardType="number-pad" /></> : null}
          </>
        )}
        <Button title={mfa ? t('common.confirm') : mode === 'otp' && !otpSent ? t('auth.sendCode') : t('auth.signIn')} onPress={submit} loading={loading} />
        <Pressable onPress={() => nav.navigate('Register')}><T center muted>{t('auth.noAccount')} <T color="#2563eb">{t('auth.signUp')}</T></T></Pressable>
      </Card>
    </Screen>
  );
}
...
export function Register({ route }: ScreenProps<'Register'>) {
  ...
  return (
    <Screen title={t('auth.signUp')}>
      <Card>
        {error && <Alert kind="error" text={error} />}
        <Select label={t('auth.role')} value={form.role} onChange={(v) => set('role', v)} options={[{ value: 'user', label: t('auth.role.user') }, { value: 'merchant', label: t('auth.role.merchant') }, { value: 'agent', label: t('auth.role.agent') }]} />
        ...
        <Select label={t('auth.country')} value={form.country} onChange={(v) => set('country', v)} options={[{ value: '', label: '—' }, ...((config?.countries ?? []) as any[]).map((c) => ({ value: c.code, label: c.name }))]} />
        ...
        <View style={{ flexDirection: 'row', gap: 8, alignItems: 'flex-end' }}>
          <View style={{ flex: 1 }}><Input label={t('auth.code')} value={form.otpCode} onChangeText={(v) => set('otpCode', v)} keyboardType="number-pad" hint={otpInfo ?? 'Optional: verify your phone or email now'} /></View>
        ...
        <Pressable onPress={() => nav.navigate('Login')}><T center muted>{t('auth.haveAccount')} <T color="#2563eb">{t('auth.signIn')}</T></T></Pressable>
      </Card>
      <ScrollView />
    </Screen>
  );
