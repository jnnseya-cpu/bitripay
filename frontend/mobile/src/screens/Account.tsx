...
export function Activity() {
  ...
  return (
    <Screen title={t('nav.transactions')}>
      <Tabs tabs={[{ id: '', label: t('common.all') }, { id: 'in', label: t('tx.in') }, { id: 'out', label: t('tx.out') }]} value={direction} onChange={(v) => { setDirection(v); setPage(1); }} />
      <Card>
        {data.data?.items.length === 0 && <Empty icon="📜" />}
        {data.data?.items.map((tx) => <TxRow key={tx.id} tx={tx} onPress={() => nav.navigate('TxDetail', { id: tx.id })} />)}
        <Row between><Button title="←" small variant="secondary" disabled={page <= 1} onPress={() => setPage(page - 1)} /><T muted size={12}>{page} / {Math.max(1, Math.ceil((data.data?.total ?? 0) / 25))}</T><Button title="→" small variant="secondary" disabled={page * 25 >= (data.data?.total ?? 0)} onPress={() => setPage(page + 1)} /></Row>
      </Card>
    </Screen>
  );
}
...
export function TxDetail({ route }: ScreenProps<'TxDetail'>) {
  ...
  const d = data.data;
  if (!d) return <Screen><Header title="Transaction" /></Screen>;
  ...
  return (
    <Screen>
      ...
      <Card style={{ alignItems: 'center' }}>
        <T bold size={32} color={isIn ? th.success : th.text}>{isIn ? '+' : tx.direction === 'out' ? '-' : ''}{money(isIn ? tx.receiveAmount ?? tx.amount : tx.amount, isIn ? tx.receiveCurrency ?? tx.currency : tx.currency)}</T>
        <Status status={tx.status} />
        <T muted size={12}>{new Date(tx.createdAt).toLocaleString()}</T>
      </Card>
      <Card>
      ...
      <Row><Button title="Share receipt" variant="secondary" onPress={() => Share.share({ message: `BitriPay ${TRANSACTION_TYPE_LABELS[tx.type]} ${tx.reference}: ${money(tx.amount, tx.currency)} · ${tx.status}` })} />{d.receiver && tx.direction === 'out' && <Button title="Send again" variant="ghost" onPress={() => nav.navigate('Send', { to: d.receiver.tag })} />}</Row>
    </Screen>
  );
}
...
export function Settings() {
  ...
  return (
    <Screen>
      ...
      <Card>
        <Row><Avatar user={user} size={52} /><View><T bold size={18}>{user?.fullName}</T><T muted>@{user?.tag} · {user?.role}</T></View></Row>
        ...
        <Select label={t('auth.country')} value={form.country} onChange={(v) => setForm({ ...form, country: v })} options={[{ value: '', label: '—' }, ...(config?.countries ?? []).map((c: any) => ({ value: c.code, label: c.name }))]} />
      ...
      <Card>
        ...
        <KV k={t('auth.phone')} v={<Row>{user?.phone ? <T>{user.phone}</T> : <T muted>—</T>}{user?.phone && (user.phoneVerified ? <Chip label="verified" kind="success" /> : <Button title="Verify" small variant="secondary" onPress={() => nav.navigate('Security')} />)}</Row>} />
  ...
  return (
    <Screen>
      ...
      <Card>
        ...
        <Input label={user?.hasPin ? 'New PIN' : 'Choose a 4–6 digit PIN'} value={pin.pin} onChangeText={(v) => setPin({ ...pin, pin: v.replace(/\D/g, '') })} secureTextEntry keyboardType="number-pad" maxLength={6} />
        <Button title={t('common.save')} disabled={pin.pin.length < 4} onPress={() => run(api.post('/api/account/pin', { pin: pin.pin, currentPin: pin.currentPin || undefined }), 'PIN saved').then(() => setPin({ pin: '', currentPin: '' }))} />
      </Card>
      <Card>
        ...
        <Button title={t('common.save')} disabled={pw.newPassword.length < 8} onPress={() => run(api.post('/api/account/password', pw), 'Password changed').then(() => setPw({ currentPassword: '', newPassword: '' }))} />
      </Card>
      <Card>
        <Row between><T bold>{t('settings.2fa')}</T>{user?.twoFactorEnabled ? <Chip label="Enabled" kind="success" /> : <Chip label="Disabled" />}</Row>
        ...
        {setup && <><Qr value={`otpauth://totp/BitriPay:${user?.email ?? user?.tag}?secret=${setup.secret}&issuer=BitriPay`} size={160} /><T mono size={12} center>{setup.secret}</T><Button title="Copy secret" small variant="secondary" onPress={() => Clipboard.setStringAsync(setup.secret)} /><Input label="Code from authenticator" value={code} onChangeText={setCode} keyboardType="number-pad" /><Button title="Enable" onPress={() => run(api.post('/api/account/2fa/enable', { code }), '2FA enabled').then(() => setSetup(null))} /></>}
        {user?.twoFactorEnabled && <><Input label="Code from authenticator" value={code} onChangeText={setCode} keyboardType="number-pad" /><Button title="Disable 2FA" variant="danger" onPress={() => run(api.post('/api/account/2fa/disable', { code }), '2FA disabled')} /></>}
      </Card>
      <Card>
        <T bold>Verification</T>
        {user?.email && !user.emailVerified && <Button title="Verify email" variant="secondary" onPress={() => api.post<{ devCode?: string }>('/api/account/verify/request', { channel: 'email' }).then((r) => setOtp({ channel: 'email', info: r.devCode ? `Code sent (sandbox: ${r.devCode})` : 'Code sent' }))} />}
        {user?.phone && !user.phoneVerified && <Button title="Verify phone" variant="secondary" onPress={() => api.post<{ devCode?: string }>('/api/account/verify/request', { channel: 'sms' }).then((r) => setOtp({ channel: 'sms', info: r.devCode ? `Code sent (sandbox: ${r.devCode})` : 'Code sent' }))} />}
        {user?.emailVerified && user?.phoneVerified && <T muted>All verified ✓</T>}
      </Card>
      <Sheet open={!!otp} onClose={() => setOtp(null)} title="Enter code">{otp && <><Alert text={otp.info} /><Input value={otpCode} onChangeText={setOtpCode} keyboardType="number-pad" /><Button title={t('common.confirm')} onPress={() => api.post<{ user: User }>('/api/account/verify/confirm', { channel: otp.channel, code: otpCode }).then((r) => { setUser(r.user); setOtp(null); toast('Verified', 'success'); }).catch((e) => toast(e.message, 'error'))} /></>}</Sheet>
    </Screen>
  );
}
...
export function Kyc() {
  ...
  const submit = () => api.post('/api/kyc', form).then(() => { toast('Submitted for review', 'success'); refresh(); kyc.reload(); }).catch((e) => toast(e.message, 'error'));
  return (
    <Screen>
      ...
      {(status === 'none' || status === 'rejected') && (
        <Card>
          <Select label="Document type" value={form.docType} onChange={(v) => setForm({ ...form, docType: v })} options={[{ value: 'passport', label: 'Passport' }, { value: 'national_id', label: 'National ID' }, { value: 'drivers_license', label: "Driver's license" }, { value: 'voter_card', label: 'Voter card' }]} />
          ...
          <T muted size={12}>Document photos and selfie can be uploaded from the web app; details submitted here are reviewed by our team.</T>
  ...
  return (
    <Screen>
      ...
      <Card style={{ alignItems: 'center' }}>
        ...
        <T bold size={22} mono>{user?.referralCode}</T>
        <Row><Button title="Copy link" small variant="secondary" onPress={() => Clipboard.setStringAsync(link)} /><Button title={t('common.share')} small onPress={() => Share.share({ message: `Join me on BitriPay: ${link}` })} /></Row>
      </Card>
      <Card>
        <KV k="Friends referred" v={String(s?.referredCount ?? 0)} />
        <KV k="Total earned" v={s ? money(s.totalEarned, s.currency) : '—'} />
        <Row style={{ flexWrap: 'wrap' }}>{(s?.settings?.rewards ?? []).map((r: number, i: number) => <Chip key={i} label={`Level ${i + 1}: ${money(r, s.currency)}`} kind="primary" />)}</Row>
      </Card>
      {(s?.referred ?? []).map((r: any) => <Card key={r.id}><T bold>{r.fullName}</T><T muted size={12}>@{r.tag} · joined {new Date(r.joinedAt).toLocaleDateString()}</T></Card>)}
    </Screen>
  );
}
...
export function Support() {
  ...
  useEffect(() => { if (tab === 'chat') load(); }, [tab]);
  useEffect(() => { if (tab !== 'chat') return; const id = setInterval(() => load(msgs[msgs.length - 1]?.createdAt), 4000); return () => clearInterval(id); }, [msgs, tab]);
  const send = async () => {
    if (!text.trim()) return;
      ...
      {tab === 'chat' ? (
        <View style={{ flex: 1 }}>
          <ScrollView ref={scroll} onContentSizeChange={() => scroll.current?.scrollToEnd({ animated: true })} contentContainerStyle={{ gap: 8, paddingVertical: 8 }}>
            ...
            {msgs.map((m) => <View key={m.id} style={{ alignSelf: m.isAdmin ? 'flex-start' : 'flex-end', backgroundColor: m.isAdmin ? th.soft : th.primary, padding: 10, borderRadius: 12, maxWidth: '80%' }}><T color={m.isAdmin ? th.text : '#fff'} size={14}>{m.body}</T><T color={m.isAdmin ? th.muted : 'rgba(255,255,255,0.7)'} size={10}>{m.isAdmin ? 'Support' : user?.fullName} · {new Date(m.createdAt).toLocaleTimeString()}</T></View>)}
          </ScrollView>
          <Row><View style={{ flex: 1 }}><Input value={text} onChangeText={setText} placeholder="Type a message…" /></View><Button title="Send" small onPress={send} /></Row>
        </View>
      ) : (
        <>
          <Button title="+ New ticket" small onPress={() => setCreate(true)} />
          {tickets.data?.items.length === 0 && <Empty icon="🎫" />}
          {tickets.data?.items.map((tk) => <Card key={tk.id}><Row between><T bold>{tk.subject}</T><Status status={tk.status} /></Row><Button title="Open" small variant="secondary" onPress={() => api.get<{ ticket: any }>(`/api/support/tickets/${tk.id}`).then((r) => setSel(r.ticket))} /></Card>)}
          <Sheet open={!!sel} onClose={() => setSel(null)} title={sel?.subject}>
            <ScrollView style={{ maxHeight: 300 }} contentContainerStyle={{ gap: 8 }}>{sel?.messages?.map((m: any) => <Card key={m.id} soft><T bold size={12}>{m.isAdmin ? 'Support' : 'You'}</T><T size={14}>{m.body}</T></Card>)}</ScrollView>
            {sel?.status !== 'closed' && <><Input value={reply} onChangeText={setReply} placeholder="Reply…" /><Button title="Reply" disabled={!reply.trim()} onPress={() => api.post<{ ticket: any }>(`/api/support/tickets/${sel.id}/reply`, { body: reply }).then((r) => { setSel(r.ticket); setReply(''); tickets.reload(); })} /></>}
          </Sheet>
