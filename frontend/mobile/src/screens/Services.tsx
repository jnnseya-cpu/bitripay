...
export function Remittance() {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(
      () =>
        api
          .get(`/api/remittances/quote?from=${from}&to=${to}&amount=${amount}`)
          .then(setQuote)
          .catch(() => setQuote(null)),
      300,
    );
  ...
  const submit = async (p: string) => {
    setLoading(true);
    try {
      const r = await api.post<any>('/api/remittances', {
        amount,
        sourceCurrency: from,
        targetCurrency: to,
        payoutMethod: method,
  ...
  return (
    <Screen>
      ...
      {tab === 'send' ? (
        <>
          <Card>
            <AmountInput label={tr('You send')} amount={amount} currency={from} onAmount={setAmount} onCurrency={setFrom} />
            ...
            {quote && (
              <Card soft>
                ...
                <KV k={tr('Recipient gets')} v={money(quote.targetAmount, to)} />
              </Card>
            )}
            <Tabs
              tabs={[
              ...
              value={method}
              onChange={(m) => setMethod(m as any)}
            />
          </Card>
          <Card>
            <T bold>{tr('Recipient')}</T>
              ...
              <Row style={{ flexWrap: 'wrap' }}>
                {recipients.data.items.map((r) => (
                  <Chip
                    ...
                    label={r.name}
                    onPress={() => {
                      setRec({ name: r.name, country: r.country || '', phone: r.phone || '', tag: r.tag || '', bankName: r.bankName || '', accountNumber: r.accountNumber || '', idNumber: '' });
            ...
            <Input label={tr('Full name')} value={rec.name} onChangeText={(v) => setRec({ ...rec, name: v })} />
            <Select
              label={tr('Country')}
              value={rec.country}
            ...
            />
            {method === 'wallet' && <Input label={tr('BitriPay @tag, email or phone')} value={rec.tag} onChangeText={(v) => setRec({ ...rec, tag: v })} autoCapitalize="none" />}
            {method === 'bank' && (
              <>
                <Input label={tr('Bank name')} value={rec.bankName} onChangeText={(v) => setRec({ ...rec, bankName: v })} />
                <Input label={tr('Account number / IBAN')} value={rec.accountNumber} onChangeText={(v) => setRec({ ...rec, accountNumber: v })} />
              </>
            )}
            {method === 'cash_pickup' && (
              <Input label={tr('Recipient ID number')} value={rec.idNumber} onChangeText={(v) => setRec({ ...rec, idNumber: v })} hint={tr('The agent checks this ID before paying out')} />
            )}
            <Input label={tr('Phone')} value={rec.phone} onChangeText={(v) => setRec({ ...rec, phone: v })} keyboardType="phone-pad" />
            ...
            <Card key={r.id}>
              <Row between>
                <View>
                  ...
                  <T muted size={12}>
                    {money(r.sourceAmount, r.sourceCurrency)} · {r.payoutMethod.replace('_', ' ')}
                    {r.pickupCode ? ` · ${r.pickupCode}` : ''}
                  </T>
                </View>
      ...
      <PinSheet
        ...
        summary={
          quote && (
            <>
              <KV k={tr('Debit')} v={money(quote.total, from)} />
...
export function Cards() {
  ...
  return (
    <Screen>
      <Header title={t('nav.cards')} right={<Button title={tr('+ New')} small onPress={() => setAction({ type: 'issue' })} />} />
      {cards.data?.items.length === 0 && <Empty icon="💳" text={tr('No virtual cards yet')} />}
      {cards.data?.items.map((c) => (
        <View key={c.id} style={{ gap: 8 }}>
          <View style={{ backgroundColor: '#1e3a8a', borderRadius: 18, padding: 20, gap: 18, opacity: c.status === 'frozen' ? 0.6 : 1 }}>
            <Row between>
              <Text style={{ color: '#fff', fontWeight: '700' }}>{tr('BitriPay Virtual')}</Text>
              <Text style={{ color: '#fff', fontSize: 12 }}>{c.status}</Text>
            </Row>
            <Text style={{ color: '#fff', fontSize: 20, letterSpacing: 3, fontFamily: 'monospace' }}>{c.maskedNumber}</Text>
            <Row between>
              <View>
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10 }}>{tr('HOLDER')}</Text>
                <Text style={{ color: '#fff', fontWeight: '600' }}>{c.holderName}</Text>
              </View>
              <View>
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10 }}>EXP</Text>
                <Text style={{ color: '#fff', fontWeight: '600' }}>
                  {String(c.expMonth).padStart(2, '0')}/{String(c.expYear).slice(-2)}
                </Text>
              </View>
              <View>
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10 }}>{tr('BALANCE')}</Text>
                <Text style={{ color: '#fff', fontWeight: '600' }}>{money(c.balance, c.currency)}</Text>
              </View>
            </Row>
          </View>
          <Row style={{ flexWrap: 'wrap' }}>
            <Button title={tr('Fund')} small onPress={() => setAction({ type: 'fund', card: c })} disabled={c.status !== 'active'} />
            <Button title={tr('Withdraw')} small variant="secondary" onPress={() => setAction({ type: 'withdraw', card: c })} />
            <Button title={tr('Details')} small variant="secondary" onPress={() => setAction({ type: 'reveal', card: c })} />
            <Button
              title={c.status === 'active' ? tr('Freeze') : tr('Unfreeze')}
              small
              variant="ghost"
              onPress={() => api.post(`/api/virtual-cards/${c.id}/${c.status === 'active' ? 'freeze' : 'unfreeze'}`).then(cards.reload)}
            />
          </Row>
        </View>
      ))}
      ...
      <PinSheet open={action?.type === 'reveal'} onClose={() => setAction(null)} onSubmit={run} loading={loading} title={tr('Reveal card details')} />
      <Sheet open={!!revealed} onClose={() => setRevealed(null)} title={tr('Card details')}>
        {revealed && (
          <>
            <KV
              k={tr('Number')}
...
function PinInline({ onSubmit, loading }: { onSubmit: (pin: string) => void; loading: boolean }) {
  ...
  return (
    <>
      <Input label={t('common.pin')} value={pin} onChangeText={(v) => setPin(v.replace(/\D/g, ''))} secureTextEntry keyboardType="number-pad" maxLength={6} />
...
function Tile({ label, sub, color, selected, onPress }: { label: string; sub: string; color: string; selected: boolean; onPress: () => void }) {
  return (
    <Pressable
      onPress={onPress}
      style={{ width: '47%', flexGrow: 1, backgroundColor: color, borderRadius: 14, padding: 14, minHeight: 80, justifyContent: 'flex-end', borderWidth: selected ? 3 : 0, borderColor: '#fff' }}
    >
...
export function Bills() {
  const { t, toast, refreshWallets, user } = useStore();
  ...
  return (
    <Screen>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
        {billers.data?.items.map((b) => (
          <Tile key={b.id} label={b.name} sub={`${b.category} · ${b.currency}`} color={b.color} selected={sel?.id === b.id} onPress={() => setSel(b)} />
        ))}
      </View>
      {billers.data?.items.length === 0 && <Empty icon="🧾" text={tr('No billers available')} />}
      {sel && (
        <Card>
          <T bold size={18}>
            {sel.name}
          </T>
          <Input label={sel.accountLabel} value={account} onChangeText={setAccount} />
          ...
          <Button title={tr('Pay bill')} onPress={() => setPin(true)} disabled={!account || !amount} />
        </Card>
      )}
    ...
    <Screen>
      ...
      {sel && (
        <Card>
          ...
          <Input label={tr('Phone number')} value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
          <Row style={{ flexWrap: 'wrap' }}>
            {sel.denominations.map((d: number) => (
              <Chip key={d} label={money(d, sel.currency)} selected={amount === fromMinor(d, 2)} onPress={() => setAmount(fromMinor(d, 2))} />
            ))}
          </Row>
          <Input label={`${t('common.amount')} (${sel.currency})`} value={amount} onChangeText={(v) => setAmount(v.replace(/[^\d.]/g, ''))} keyboardType="decimal-pad" big />
          <Button title={tr('Top up')} onPress={() => setPin(true)} disabled={!phone || !amount} />
        </Card>
      )}
    ...
    <Screen>
      ...
      {sel && (
        <Card>
          ...
          <Row style={{ flexWrap: 'wrap' }}>
            {sel.denominations.map((d: number) => (
              <Chip key={d} label={money(d, sel.currency)} selected={amount === d} onPress={() => setAmount(d)} />
            ))}
          </Row>
          <Button title={`Buy ${amount ? money(amount, sel.currency) : ''}`} onPress={() => setPin(true)} disabled={!amount} />
        </Card>
      )}
      {(mine.data?.items ?? []).length > 0 && (
        <Card>
          <T bold>{tr('My gift cards')}</T>
          {mine.data!.items.map((g) => (
            <KV
              key={g.id}
              k={`${g.brand} · ${money(g.amount, g.currency)}`}
              v={
                <T mono size={12}>
                  {g.code} · PIN {g.pin}
                </T>
