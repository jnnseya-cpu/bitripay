    ...
    <Screen>
      ...
      {tab === 'send' ? (
        <>
          <Card>
            ...
            {quote && (
              <Card soft>
                ...
                <KV k="Recipient gets" v={money(quote.targetAmount, to)} />
              </Card>
            )}
            <Tabs
              tabs={[
          ...
          <Card>
            ...
            {method === 'bank' && (
              <>
                <Input label="Bank name" value={rec.bankName} onChangeText={(v) => setRec({ ...rec, bankName: v })} />
                <Input label="Account number / IBAN" value={rec.accountNumber} onChangeText={(v) => setRec({ ...rec, accountNumber: v })} />
              </>
            )}
            {method === 'cash_pickup' && (
              <Input label="Recipient ID number" value={rec.idNumber} onChangeText={(v) => setRec({ ...rec, idNumber: v })} hint="The agent checks this ID before paying out" />
            )}
      ...
      <PinSheet
        ...
        summary={
          quote && (
            <>
              <KV k="Debit" v={money(quote.total, from)} />
      ...
      {cards.data?.items.map((c) => (
        <View key={c.id} style={{ gap: 8 }}>
          <View style={{ backgroundColor: '#1e3a8a', borderRadius: 18, padding: 20, gap: 18, opacity: c.status === 'frozen' ? 0.6 : 1 }}>
            <Row between>
              <Text style={{ color: '#fff', fontWeight: '700' }}>BitriPay Virtual</Text>
            ...
            <Row between>
              <View>
                <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10 }}>HOLDER</Text>
          ...
          <Row style={{ flexWrap: 'wrap' }}>
            ...
            <Button
              title={c.status === 'active' ? 'Freeze' : 'Unfreeze'}
              small
              variant="ghost"
    ...
    <Screen>
      ...
      {sel && (
        <Card>
          ...
          <Button title="Pay bill" onPress={() => setPin(true)} disabled={!account || !amount} />
        </Card>
      )}
