...
interface Run {
  ...
  input: string;
  output: string | null;
  actions: { id: string; tool: string; outcome: string }[];
  proposals: any[];
  createdAt: string;
  error: string | null;
}
...
const SCREEN_FOR: Record<string, string> = {
  ...
  '/app/topup': 'Topup',
  '/app/bills': 'Bills',
  '/app/settings?tab=kyc': 'Kyc',
  '/app/settings?tab=security': 'Security',
};
...
export function Assist() {
  ...
  useEffect(() => {
    api
      .get<{ agents: AgentCard[]; runtime: any; usage: any; addon: any; billing: any }>('/api/assist/agents')
      .then((r) => {
        setAgents(r.agents);
        setMode(r.runtime.mode);
        setUsage(r.usage);
        setAddon(r.addon);
        setBilling(r.billing);
        setAgent((a) => a ?? r.agents[0] ?? null);
      })
  ...
  useEffect(() => {
    if (!agent) return;
    api
      .get<{ items: Run[] }>(`/api/assist/runs?agent=${agent.key}&limit=8`)
      .then((r) => setRuns(r.items.reverse()))
      .catch(() => setRuns([]));
  }, [agent]);
  useEffect(() => {
    setTimeout(() => scroll.current?.scrollToEnd({ animated: true }), 50);
  }, [runs.length, busy?.actions.length]);
  ...
  const ask = async (text: string) => {
    ...
    try {
      ...
      setBusy(null);
      api
        .get<{ usage: any }>('/api/assist/usage')
        ...
        <Card>
          ...
          <Button
            ...
            onPress={async () => {
              setActivating(true);
              try {
                const r = await api.post<{ billing: any }>('/api/assist/consent', { version: d.version });
                setBilling(r.billing);
              } catch (e: any) {
                setError(e.message);
              } finally {
    ...
    <Screen title="Command centre" scroll={false}>
      <View style={{ flex: 1, gap: 10 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
          {agents.map((a) => (
            <Chip key={a.key} label={`${a.icon} ${a.name}${a.paused ? ' (paused)' : !a.enabled ? ' (off)' : ''}`} selected={agent?.key === a.key} onPress={() => setAgent(a)} />
          ))}
        </ScrollView>
        {agent && (
          <T muted size={12}>
            {agent.tagline}
            {billing?.mode === 'per_use' && !billing.subscriptionActive && billing.prices[0]
              ? ` · lookups free · questions ${billing.prices[0].standardFormatted}${billing.freeRunsLeft ? ` · ${billing.freeRunsLeft} free left` : ''}`
              : usage && !usage.unlimited
                ? ` · ${usage.acuUsed}/${usage.allowance} credits used this month`
                : ''}
          </T>
        )}
        <ScrollView ref={scroll} style={{ flex: 1 }} contentContainerStyle={{ gap: 10, paddingBottom: 8 }}>
          {runs.length === 0 && !busy && (
            <Empty icon={agent?.icon ?? '🧭'} text={`Ask ${agent?.name ?? 'an agent'} about your account. It reads your data, explains it and prepares actions you confirm yourself.`} />
          )}
          {[...runs, ...(busy ? [busy] : [])].map((r) => (
            <View key={r.id} style={{ gap: 6 }}>
              <View style={{ alignSelf: 'flex-end', backgroundColor: th.primary, borderRadius: 14, padding: 10, maxWidth: '85%' }}>
                <T color="#fff">{r.input}</T>
              </View>
              {r.actions.length > 0 && (
                <Row style={{ flexWrap: 'wrap', gap: 6 }}>
                  {r.actions.map((a) => (
                    <Chip
                      key={a.id}
                      label={`${a.outcome === 'executed' ? '✓' : a.outcome === 'denied' ? '⛔' : a.outcome === 'awaiting_approval' ? '⏳' : '!'} ${label(a.tool)}`}
                      kind={a.outcome === 'executed' ? 'success' : a.outcome === 'denied' ? 'danger' : 'warning'}
                    />
                  ))}
                </Row>
              )}
              {r.output ? (
                <View style={{ alignSelf: 'flex-start', backgroundColor: th.card, borderWidth: 1, borderColor: th.border, borderRadius: 14, padding: 10, maxWidth: '92%' }}>
                  <T size={14}>{r.output}</T>
                </View>
              ...
              {r.proposals.map((p, i) => (
                <Pressable
                  key={i}
                  onPress={() => open(p.link)}
                  style={{ borderWidth: 1, borderStyle: 'dashed', borderColor: th.primary, borderRadius: 12, padding: 10, gap: 4, alignSelf: 'flex-start', maxWidth: '92%' }}
                >
                  <T bold>{p.title}</T>
                  <T muted size={12}>
