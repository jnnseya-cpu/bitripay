      ...
      <Field label="Card number" error={value.number.replace(/\s/g, '').length >= 12 && !valid ? 'Invalid card number' : null}>
        <div className="input-group">
          <input
            ...
            value={value.number}
            onChange={(e) => onChange({ ...value, number: formatCardNumber(e.target.value).slice(0, 23) })}
          />
          <span className="addon input" style={{ display: 'grid', placeItems: 'center' }}>
            {brand === 'unknown' ? '💳' : brand.toUpperCase()}
          </span>
        </div>
      </Field>
      <div className="grid cols-3">
        ...
        <Field label="Year">
          <Input
            ...
            value={value.expYear}
            onChange={(e) => onChange({ ...value, expYear: e.target.value.replace(/\D/g, '') })}
          />
        </Field>
        <Field label="CVC">
