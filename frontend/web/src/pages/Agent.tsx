...
export function AgentDashboard() {
  ...
  if (!allowed) return <Alert kind="warning">{tr("This area is for agent accounts and the members of an agent's team.")}</Alert>;
  const s = stats.data;
  ...
  const run = async (p: string) => {
    ...
    try {
      if (pin === 'cashin') {
        ...
        toast(tr('Cash-out confirmed – hand over the cash'), 'success');
        setCode('');
      } else if (pin === 'pickup') {
        await api.post(`/api/agents/me/pickups/${pickup.pickupCode}/payout`, { recipientIdNumber: idNumber, pin: p });
        toast(tr('Pickup paid out – your float was credited'), 'success');
        setPickup(null);
      }
      setPin(null);
      refreshWallets();
  ...
  return (
    <div>
      ...
      <div className="grid cols-4">
        <div className="card">
          <div className="stat">
            <span className="label">{tr('Float')}</span>
        ...
        <div className="card">
          <div className="stat">
            <span className="label">{tr('Cash-in (30d)')}</span>
            <span className="value">{s?.cashInCount ?? 0}</span>
          </div>
        </div>
        <div className="card">
          <div className="stat">
            <span className="label">{tr('Cash-out (30d)')}</span>
            <span className="value">{s?.cashOutCount ?? 0}</span>
          </div>
        </div>
        <div className="card">
          <div className="stat">
            <span className="label">{tr('Commission (30d)')}</span>
      ...
      {!!insights.data?.extras.cash && (
        <div className="card mt">
          <div className="card-title">
            <h3>{tr('Cash-in and cash-out per day (30 days)')}</h3>
      ...
      <Tabs
        tabs={[
          { id: 'cashin', label: tr('Cash-in (deposit)') },
          { id: 'cashout', label: tr('Cash-out (withdraw)') },
          { id: 'pickup', label: tr('Cash pickup') },
          { id: 'requests', label: tr('Requests') },
          { id: 'payouts', label: tr('📤 Payouts to execute') },
      ...
      <div className="grid cols-2">
        {tab === 'cashin' && (
          <div className="card">
            <h3>{tr("Credit a customer's wallet")}</h3>
            <p className="small muted">
              {tr("Take cash from the customer, then send the same amount from your float. The platform fee is deducted from the customer's credit and your commission is paid instantly.")}
            </p>
            <Field label={tr('Customer (@tag, email or phone)')}>
        ...
        {tab === 'cashout' && (
          <div className="card">
            <h3>{tr('Pay out cash')}</h3>
            <p className="small muted">{tr('The customer creates a cash-out request in their app and shows you the code. Confirm it to receive the funds in your float, then hand over the cash.')}</p>
            <Field label={tr('Cash-out code')}>
        ...
        {tab === 'pickup' && (
          <div className="card">
            <h3>{tr('Remittance cash pickup')}</h3>
            <Field label={tr('Pickup code')}>
              <div className="row">
                <Input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder={tr('XXXX-XXXX')} />
                <Button
                  variant="secondary"
                  onClick={() =>
                    api
            ...
            {pickup?.status === 'ready_for_pickup' && (
              <>
                <Field label={tr('Recipient ID number (verify their ID)')}>
                  <Input value={idNumber} onChange={(e) => setIdNumber(e.target.value)} />
                </Field>
                <Button block size="lg" onClick={() => setPin('pickup')}>
                  {tr('Pay out {0}', { 0: money(pickup.targetAmount, pickup.targetCurrency) })}
                </Button>
              </>
            )}
          </div>
        ...
        {tab !== 'team' && (
          <div className="card center">
            <h3>{counter ? tr('Agent QR') : tr('Your agent QR')}</h3>
            <p className="small muted">
              {tr('Customers scan this to cash out')} {counter ? 'at this counter' : 'with you'} or send money to the agent.
            </p>
      ...
      {tab === 'payouts' && (
        <div className="card">
          <h3>{tr('Payouts assigned to your payout account')}</h3>
          <p className="small muted">
            {tr(
            ...
            {(payouts.data?.items ?? []).map((p: any) => (
              <div key={p.id} className="list-item" style={{ alignItems: 'flex-start' }}>
                <div className="flex1">
                  ...
                  <div className="sub-text">
                    {tr('Ref')} <span className="mono">{p.reference}</span> · <StatusBadge status={p.stage.toLowerCase().replace(/_/g, ' ')} />
                    {p.riskFlags?.length ? (
                    ...
                    <Button
                      ...
                      onClick={() =>
                        api
                          ...
                          .then(() => {
                            toast(tr('Claimed – execute the transfer now'), 'success');
                            payouts.reload();
                          })
                  ...
                  {p.stage === 'IN_PROGRESS' && (
                    <>
                      <Button size="sm" variant="secondary" onClick={() => setEvidence({ id: p.id, text: '', externalRef: '' })}>
                        {tr('Enter confirmation manually')}
                      </Button>
      ...
      {evidence && (
        <div className="card">
          <h4>{tr('Manual confirmation (goes to maker-checker)')}</h4>
          <Field label={tr('Operator confirmation SMS, exactly as received')}>
            <Input value={evidence.text} onChange={(e) => setEvidence({ ...evidence, text: e.target.value })} />
          </Field>
          <Field label={tr('Operator transaction ID')}>
      ...
      <PinModal open={!!pin} onClose={() => setPin(null)} onSubmit={run} loading={loading} title={tr('Confirm with your PIN')} />
    </div>
  );
}
