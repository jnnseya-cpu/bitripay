...
export function MoveMoney() {
  ...
  const destination = () => {
    switch (dest) {
      case 'wallet':
        return { method: 'wallet', to, note: note || null };
      case 'qr':
        return { method: 'qr', data: qr, note: note || null };
      case 'mobile_money':
        return { method: 'mobile_money', operatorId: dstOp.operatorId, phone: dstOp.phone, name: dstOp.name || null };
      case 'bank':
        return bank.bankAccountId
          ? { method: 'bank', bankAccountId: bank.bankAccountId }
          : { method: 'bank', bankName: bank.bankName, accountName: bank.accountName, accountNumber: bank.accountNumber, country: bank.country || null };
      case 'agent':
  ...
  const destReady =
    dest === 'wallet'
      ...
      : dest === 'qr'
        ? dQr.length > 3
        : dest === 'mobile_money'
          ? !!dstOp.operatorId && dstOp.phone.length > 5
          : dest === 'bank'
            ? !!bank.bankAccountId || (!!bank.bankName && !!bank.accountNumber)
            : agent.length >= 2;

  useEffect(() => {
    if (!dAmount || !destReady) return setPreview(null);
    api
      .post<any>('/api/money/preview', { destination: destination(), sourceMethod: source, amount: dAmount, currency: cur, targetCurrency: target || cur })
      .then((r) => {
        setPreview(r);
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      async () => {
        ...
        if (TERMINAL.includes(r.route.stage)) {
          refreshWallets();
          history.reload();
        }
      },
  ...
  const submit = async (pin?: string) => {
    ...
    try {
      ...
      if (source === 'mobile_money') {
        src.operatorId = srcOp.operatorId || null;
        src.phone = srcOp.phone;
      }
          ...
          {route ? (
            <div>
              ...
              <div className="center">
                <div style={{ fontSize: '3rem' }}>
                  {route.stage === 'SETTLED'
                    ? '✅'
                    : ['FAILED', 'EXPIRED', 'REVERSED', 'REFUNDED'].includes(route.stage)
                      ? '❌'
                      : ['MANUAL_REVIEW', 'INSUFFICIENT_LIQUIDITY', 'MISMATCHED', 'DUPLICATE', 'DISPUTED'].includes(route.stage)
                        ? '🔍'
                        : '⏳'}
                </div>
                <h2>
                  {money(route.amount, route.currency)} → {destLabels[route.destination as Dest]}
                </h2>
              ...
              {['FUNDING_PENDING', 'BIOMETRIC_APPROVAL_REQUIRED'].includes(route.stage) && route.payment && (
                <div className="mt">
              ...
              <div className="row mt wrap">
                ...
                {['FUNDED', 'FAILED', 'INSUFFICIENT_LIQUIDITY'].includes(route.stage) && (
                  <Button
                    onClick={() =>
                      api
                        .post(`/api/money/${route.id}/retry`, {})
                        .then((r: any) => setRoute(r.route))
                ...
                <Button
                  variant="ghost"
                  onClick={() =>
                    api.get<any>(`/api/money/${route.id}/receipt`).then((r) => {
                      const w = window.open('', '_blank');
                      if (w) {
                        w.document.write(`<pre>${JSON.stringify(r, null, 2).replace(/</g, '&lt;')}</pre>`);
                        w.document.close();
                      }
                    })
                  }
            ...
            <>
                ...
                <Field
                  ...
                  hint={
                    preview?.quote?.receivingCurrencies
                      ? `Default: the local currency (${preview.quote.receivingCurrencies.defaultCurrency}). Other currencies are offered only when the corridor, the paying institution and prefunded liquidity allow it right now.`
                      : undefined
                  }
                >
                  {preview?.quote?.receivingCurrencies ? (
                    <div className="row wrap">
                      {preview.quote.receivingCurrencies.options.map((o: any) => (
                        <button
                          key={o.currency}
                          type="button"
                          className={`chip ${(target || preview.quote.targetCurrency) === o.currency ? 'primary' : ''}`}
                          disabled={!o.available}
                          title={o.reasons.concat(o.warnings ?? []).join('\n')}
                          onClick={() => setTarget(o.currency)}
                        >
                          {o.currency}
                          {o.isLocal ? ' · local' : ''}
                          {o.consentRequired ? ' · recipient confirms' : ''}
                  ...
                  {preview?.quote?.receivingCurrencies?.options
                    ...
                    .map((o: any) => (
                      <div key={o.currency} className="tiny muted">
              ...
              {preview && (
                <div className="card soft compact mb">
                  <div className="row mb-sm">
                    {preview.destination.user && <Avatar user={preview.destination.user} size="sm" />}
                  ...
                  <KV
                    ...
                    v={
                      preview.quote.guaranteedRecipientAmount != null ? (
                        ...
                        <span className="muted">not guaranteed – indicative rate</span>
                      )
                    }
                  />
                  ...
                  {preview.quote.confirmation && (
                    <KV
                      ...
                      v={
                        <span className="tiny">
                          funding: {preview.quote.confirmation.funding.replace(/_/g, ' ').toLowerCase()} · payout: {preview.quote.confirmation.payout.replace(/_/g, ' ').toLowerCase()}
                        </span>
                      }
                    />
                  )}
                  ...
                  <div className="tiny muted mt-sm">
                    <b>Refunds:</b> {preview.quote.refundConditions}
                  </div>
                </div>
              )}
              ...
              <Button
                ...
                loading={loading}
                disabled={
                  !preview || !amount || (source === 'card' && card.number.length < 12) || (source === 'mobile_money' && !srcOp.phone) || (preview?.quote?.sourceOfFundsRequired && !sourceOfFunds)
                }
                onClick={() => {
                  setPinFor('send');
                  setPinOpen(true);
          ...
          <div className="list">
            {history.data?.items.slice(0, 12).map((r) => (
              <div key={r.id} className="list-item clickable" onClick={() => setRoute(r)}>
                <div className="flex1">
                  <div className="main-text">{money(r.amount, r.currency)}</div>
                  <div className="sub-text">
                    {sourceLabels[r.source as Source]} → {destLabels[r.destination as Dest]} · {new Date(r.createdAt).toLocaleDateString()}
                  </div>
                </div>
