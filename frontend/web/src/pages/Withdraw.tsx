...
import { countryLabel, currencyFlag } from '@bitripay/shared';
      ...
      <div className="grid cols-2">
        <div className="card">
          ...
          <Button
            block
            size="lg"
            disabled={!amount || (dest === 'bank' ? eligible.length === 0 : !operatorId || !phone)}
            onClick={() => {
              setPinFor('withdraw');
              setPinOpen(true);
            }}
              ...
              <div key={a.id} className="list-item">
                <div className="flex1">
                  <div className="main-text">
                    {a.bankName} <span className="chip">{a.currency}</span>
                  </div>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title={tr('Add bank account')}>
        ...
        <div className="grid cols-2">
          <Field label={t('common.currency')}>
            <Select value={bank.currency} onChange={(e) => setBank({ ...bank, currency: e.target.value })}>
        ...
        <Button
          ...
          disabled={!bank.bankName || !bank.accountName || !bank.accountNumber}
        >
          {t('common.save')}
        </Button>
      </Modal>
