...
export function StaticPage() {
  ...
  if (page.error)
    return (
      <Shell>
        <Alert kind="error">Page not found</Alert>
      </Shell>
    );
  if (!page.data) return <Loading />;
  return (
    <Shell>
      <div className="card md" dangerouslySetInnerHTML={{ __html: renderMarkdown(page.data.content) }} />
    </Shell>
  );
}
  ...
  return (
    <Shell>
      <div className="card">
        <h2>Contact us</h2>
        <p className="muted">
          {config?.site?.contactEmail} {config?.site?.contactPhone && `· ${config.site.contactPhone}`}
        </p>
        {sent ? (
          <Alert kind="success">Thanks! We'll get back to you shortly.</Alert>
        ) : (
          <form
            onSubmit={(e) => {
              e.preventDefault();
