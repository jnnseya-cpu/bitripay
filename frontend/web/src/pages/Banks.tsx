...
export function Banks() {
  ...
  const link = (institutionId: string) =>
    api
      .post<any>('/api/open-banking/links', { institutionId })
      .then((r) => {
        if (r.link.authUrl) window.location.href = r.link.authUrl;
        else view.reload();
      })
      .catch(err);
  const importCsv = () =>
    api
      .post<any>('/api/open-banking/statements', statement)
      .then((r) => {
        view.reload();
        setStatement({ ...statement, csv: '' });
        toast(`${r.link.imported} rows imported${r.link.rejected ? `, ${r.link.rejected} skipped` : ''}`, 'success');
      })
      .catch(err);
  const sync = (id: string) =>
    api
      .post<any>(`/api/open-banking/links/${id}/sync`, {})
      .then((r) => {
        view.reload();
        toast(`${r.link.imported} new transaction(s)`, 'success');
      })
      .catch(err);
  const revoke = (id: string) => {
    if (!confirm('Revoke this consent? Mandates on it stop too.')) return;
    api
      .del(`/api/open-banking/links/${id}`)
      .then(() => view.reload())
      .catch(err);
  };
  const createMandate = (p: string) => {
    setBusy(true);
    api
      .post('/api/open-banking/mandates', { ...mandate, pin: p })
      .then(() => {
        setPin(false);
        view.reload();
          ...
          <div className="card">
              ...
              <div key={i.id} className="list-item">
                <div className="flex1">
                  ...
                  <div className="sub-text">
                    {i.currencies.join(', ')} · {i.features.join(', ')}
                    {i.provider === 'sandbox' ? ' · sandbox' : ''}
                  </div>
                </div>
            ...
            <Field label="CSV">
              <Textarea
                rows={6}
                value={statement.csv}
                onChange={(e) => setStatement({ ...statement, csv: e.target.value })}
                placeholder={'date,description,amount\n2026-08-28,ACME LTD SALARY,1500.00\n2026-08-01,RENT,-650.00'}
              />
            </Field>
              ...
              <div key={l.id} className="list-item" style={{ display: 'block' }}>
                <div className="row" style={{ justifyContent: 'space-between' }}>
                  <div className="main-text">
                    {l.institutionName} <span className="sub-text">{l.provider === 'statement_import' ? 'statement' : l.provider}</span>
                  </div>
                  <StatusBadge status={l.status} />
                </div>
                {l.accounts.map((a: any) => (
                  <KV key={a.id} k={`${a.name} ${a.masked}`} v={money(a.balanceMinor, a.currency)} />
                ))}
                <div className="sub-text">
                  {l.transactionCount} transactions{l.consentExpiresAt ? ` · consent until ${l.consentExpiresAt.slice(0, 10)}` : ''}
                  {l.lastSyncedAt ? ` · synced ${new Date(l.lastSyncedAt).toLocaleString()}` : ''}
                </div>
                <div className="row" style={{ gap: 6, marginTop: 6 }}>
                  {l.status === 'PENDING' && l.authUrl && (
                    <Button
                      size="sm"
                      onClick={() => {
      ...
      {tab === 'income' && (
        <div className="card">
          ...
          <Button
            ...
            onClick={() =>
              api
                .get('/api/open-banking/income?refresh=1')
                .then(() => view.reload())
                .catch(err)
            }
          >
          ...
          <div className="card">
            ...
            {(view.data?.mandates ?? []).map((m: any) => (
              <div key={m.id} className="list-item">
                <div className="flex1">
                  <div className="main-text">
                    {m.institutionName} · {m.purpose === 'billing' ? 'subscriptions' : 'top-ups'}
                  </div>
                  <div className="sub-text">
                    up to {money(m.maxPerPaymentMinor, m.currency)} per payment, {money(m.maxPerMonthMinor, m.currency)} a month · used {money(m.usedThisMonthMinor, m.currency)} this month
                  </div>
                </div>
                ...
                {m.status === 'ACTIVE' && (
                  <Button
                    ...
                    onClick={() =>
                      api
                        .del(`/api/open-banking/mandates/${m.id}`)
                        .then(() => view.reload())
                        .catch(err)
                    }
                  >
