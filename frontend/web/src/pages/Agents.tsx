    ...
    <div>
      ...
      <div className="grid cols-2">
            ...
            {agents.data?.items.map((a) => (
              <div key={a.id} className={`list-item clickable`} onClick={() => setSelected(a)}>
                ...
                <div className="flex1">
                  ...
                  <div className="sub-text">@{a.tag} {a.country ? `· ${a.country}` : ''} · commission {(a.commissionBps / 100).toFixed(2)}%</div>
                </div>
                <Button size="sm" variant={selected?.id === a.id ? undefined : 'secondary'}>Select</Button>
              </div>
            ))}
          </div>
        ...
        <div className="card">
          ...
          {selected ? (
            <div className="list-item card soft compact mb"><Avatar user={selected} /><div><div className="main-text">{selected.businessName || selected.fullName}</div><div className="sub-text">@{selected.tag}</div></div></div>
          ) : (
          ...
          <Button block disabled={!selected || !amount} onClick={() => setPinOpen(true)}>Create cash-out code</Button>
          ...
          <div className="list">
            {requests.data?.items.map((r) => (
              <div key={r.id} className="list-item">
                <div className="flex1">
                  <div className="main-text">{money(r.amount, r.currency)} · <span className="mono">{r.code}</span></div>
                  <div className="sub-text">{r.kind.replace('_', '-')} · {r.agent?.businessName || r.agent?.fullName} · {new Date(r.createdAt).toLocaleString()}</div>
                </div>
                <StatusBadge status={r.status} />
                {r.status === 'pending' && <Button size="sm" variant="ghost" onClick={() => api.post(`/api/agents/cash-requests/${r.code}/cancel`).then(requests.reload)}>{t('common.cancel')}</Button>}
              </div>
            ))}
          </div>
        </div>
      </div>
      <PinModal open={pinOpen} onClose={() => setPinOpen(false)} onSubmit={submit} loading={loading} summary={<KV k={`Cash out at ${selected?.businessName || selected?.fullName}`} v={`${amount} ${cur}`} />} />
      <Modal open={!!result} onClose={() => setResult(null)} title="Show this code to the agent">
        {result && (
          <div className="center">
            <div style={{ fontSize: '2.4rem', letterSpacing: '0.2em', fontWeight: 800 }} className="mono">{result.code}</div>
            <p className="muted small">{money(result.amount, result.currency)} + fee {money(result.fee, result.currency)} · expires {new Date(result.expiresAt).toLocaleTimeString()}</p>
