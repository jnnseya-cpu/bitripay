        ...
        actions={
          <>
            <Button
              variant="secondary"
              onClick={() => {
                setForm((f) => ({ ...f, amount: '', description: '', payer: '' }));
            ...
            <div key={r.id} className="list-item">
              ...
              <div className="flex1">
                ...
                <div className="sub-text">
                  {tab === 'incoming' ? `From ${r.requester?.fullName} (@${r.requester?.tag})` : r.payer ? `To ${r.payer.fullName} (@${r.payer.tag})` : r.kind} ·{' '}
                  {new Date(r.createdAt).toLocaleString()} · <span className="mono">{r.code}</span>
                </div>
              </div>
      ...
      <Modal open={!!created} onClose={() => setCreated(null)} title={created?.kind === 'request' ? tr('Money request') : tr('Payment link')}>
        {created && (
          <div className="center">
            ...
            <div className="card soft compact small mono" style={{ wordBreak: 'break-all' }}>
              {created.link}
            </div>
