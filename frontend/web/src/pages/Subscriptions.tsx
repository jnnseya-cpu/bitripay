...
export function Subscriptions() {
  ...
  const lookup = () =>
    api
      .get<any>(`/api/billing/plans/${code.trim()}`)
      .then((r) => setPlan(r.plan))
      .catch(err);
  const subscribe = (p: string) => {
    setBusy(true);
    api
      .post<any>('/api/billing/subscriptions', { plan: plan.code, pin: p })
      .then((r) => {
        setPin(false);
        setPlan(null);
        setCode('');
        view.reload();
        toast(r.invoice ? `Subscribed · ${money(r.invoice.totalMinor, r.invoice.currency)} paid` : 'Trial started', 'success');
      })
      .catch(err)
        ...
        <div className="card">
          ...
          {(view.data?.items ?? []).map((s: any) => (
            <div key={s.id} className="list-item" style={{ display: 'block' }}>
              ...
              <div className="sub-text">
                {money(s.plan.amountMinor, s.plan.currency)} every {every(s.plan)} · next {s.status === 'PAST_DUE' ? 'retry' : 'charge'} {new Date(s.nextChargeAt).toLocaleDateString()}
          ...
          {(view.data?.invoices ?? []).map((i: any) => (
            <div key={i.id} className="list-item">
              <div className="flex1">
                <div className="main-text">{i.number}</div>
                <div className="sub-text">
                  {i.periodStart.slice(0, 10)} → {i.periodEnd.slice(0, 10)}
                  {i.taxMinor ? ` · tax ${money(i.taxMinor, i.currency)}` : ''}
                  {i.usageQty ? ` · usage ${i.usageQty}` : ''}
                </div>
              </div>
