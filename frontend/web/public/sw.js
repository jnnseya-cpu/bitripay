...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/v1/')) {
    ...
    event.respondWith(fetch(req).then((res) => { const copy = res.clone(); caches.open(VERSION).then((c) => c.put(req, copy)); return res; }).catch(() => caches.match(req).then((hit) => hit || new Response(JSON.stringify({ error: { code: 'offline', bp: 'BP-4000', message: 'You are offline' } }), { status: 503, headers: { 'content-type': 'application/json' } }))));
    return;
  }
  // app shell and static assets: cache first, refresh in the background
  event.respondWith(caches.match(req).then((hit) => {
    const fetching = fetch(req).then((res) => { if (res.ok && (url.origin === self.location.origin)) caches.open(VERSION).then((c) => c.put(req, res.clone())); return res; }).catch(() => hit);
    return hit || fetching;
  }));
});
